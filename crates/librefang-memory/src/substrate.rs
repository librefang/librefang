...
fn remove_agent_inner(conn: &Connection, agent_id: AgentId) -> LibreFangResult<()> {
    ...
    // An ephemeral worker's run record is owned by the agent that spawned it
    // (#7752). Deleting the parent takes its workers with it in this same
    // transaction — "retention follows the parent, no orphans by
    // construction" is the entire claim `ephemeral_runs` makes.
