...
pub fn run_migrations(conn: &Connection) -> Result<(), rusqlite::Error> {
    ...
    // create this same table with byte-identical DDL — #8231 (the TUI view of
    // the same history) and `fix/schema-forward-compat-59` — so whichever
    // merges first keeps 58 and the rest renumber on rebase rather than
    // redefining the function. The migration itself is a no-op against a
