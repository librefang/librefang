...
mod tests {
    ...
    fn v54_marks_pre_existing_agent_rows_as_lineage_unknown() {
        ...
        // Every step above 50 fires from 50, so the fixture also needs each table those steps alter — `memories`, `group_roster`, `sessions` and `task_queue` — even though this test asserts nothing about them.
        // Their absence is not a v54 bug; a real database at user_version 50 has all four (`task_queue` since `migrate_v1`).
        let conn = Connection::open_in_memory().unwrap();
        conn.execute_batch(
            "
            CREATE TABLE agents (
