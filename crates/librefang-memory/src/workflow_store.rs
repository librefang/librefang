...
///
/// Shares the same `Arc<Mutex<Connection>>` as every other store in
/// `MemorySubstrate`. The `workflow_runs` table is created by
/// `migration::migrate_v35`, which runs before this store is constructed.
#[derive(Clone)]
...
impl WorkflowStore {
    ...
    pub fn upsert_run(&self, row: &WorkflowRunRow) -> Result<(), String> {
        ...
        .map_err(|e| format!("workflow upsert failed: {e}"))?;
        Ok(())
    }

    /// Get a single workflow run by ID.
    pub fn get_run(&self, id: &str) -> Result<Option<WorkflowRunRow>, String> {
        let c = self.conn.lock().unwrap();
        let mut stmt = c
            .prepare(
            ...
            .map_err(|e| format!("workflow get_run prepare failed: {e}"))?;
        let mut rows = stmt
            .query_map(rusqlite::params![id], row_from_sqlite)
            .map_err(|e| format!("workflow get_run query failed: {e}"))?;
        match rows.next() {
            Some(Ok(row)) => Ok(Some(row)),
            Some(Err(e)) => Err(format!("workflow get_run row read failed: {e}")),
            None => Ok(None),
        }
    }

    /// List workflow runs, optionally filtered by state.
    pub fn list_runs(&self, state_filter: Option<&str>) -> Result<Vec<WorkflowRunRow>, String> {
        let c = self.conn.lock().unwrap();
        ...
        let mut stmt = c
            .prepare(&sql)
            .map_err(|e| format!("workflow list_runs prepare failed: {e}"))?;
        let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|p| &**p).collect();
        let rows = stmt
            .query_map(param_refs.as_slice(), row_from_sqlite)
            .map_err(|e| format!("workflow list_runs query failed: {e}"))?;
        let mut result = Vec::new();
        for row in rows {
            result.push(row.map_err(|e| format!("workflow list_runs row read failed: {e}"))?);
        }
        Ok(result)
    }

    /// Delete a workflow run by ID. Returns true if a row was deleted.
    pub fn delete_run(&self, id: &str) -> Result<bool, String> {
        let c = self.conn.lock().unwrap();
        let affected = c
            .execute(
            ...
            .map_err(|e| format!("workflow delete_run failed: {e}"))?;
        Ok(affected > 0)
    }

    /// Load all workflow runs from the database. Used at boot time to
    /// populate the in-memory DashMap.
    pub fn load_all_runs(&self) -> Result<Vec<WorkflowRunRow>, String> {
        self.list_runs(None)
    }

    /// Count workflow runs, optionally filtered by state. Used by the
    /// JSON-to-SQLite migration to check whether the table is empty.
    pub fn count_runs(&self) -> Result<usize, String> {
        let c = self.conn.lock().unwrap();
        let count: i64 = c
    ...
    pub fn wal_checkpoint(&self) -> Result<(), String> {
        ...
        c.execute_batch("PRAGMA wal_checkpoint(PASSIVE);")
            .map_err(|e| format!("workflow wal_checkpoint failed: {e}"))?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn in_memory_store() -> WorkflowStore {
        ...
        WorkflowStore::new(Arc::new(Mutex::new(conn)))
    }
