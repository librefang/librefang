...
///
/// The three properties in one test, because they are the same claim seen from
/// three sides:
///
/// - the run is **reachable** under its parent, which it was not before — a
///   worker vanished completely, so "what did this agent delegate, and what did
///   it cost" had no answer at all;
/// - the mission workspace is still **gone**, because a run record and a
///   surviving workspace are different things and #7723's guarantee is not
///   being traded away to get the first one;
/// - an agent that spawned nothing is **untouched**, so the table cannot
///   attribute one agent's workers to another.
#[tokio::test(flavor = "multi_thread")]
async fn a_finished_ephemeral_run_is_recorded_under_its_parent() {
    ...
    // The stub driver's refusal is recovered by the agent loop into an
    // ordinary response, so this reaches the completion path. Whether it
    // returns Ok or Err, a record must exist either way — that is the point.
    let _ = kernel
        .spawn_ephemeral_worker(ephemeral_request(parent))
        .await;
    ...
    // The workspace is a separate guarantee and it still holds. Persisting a
    // run record is not the same as keeping the worker's scratch directory,
    // and #7723 says the directory goes.
    assert!(
        transient_entries(&kernel).is_empty(),
    ...
    // A parent that spawned nothing sees nothing, and in particular does not
    // see the other parent's worker.
    assert!(
        kernel
...
///
/// The ledger already bills a worker's spend to `billed_agent_id = parent`. If
/// the run record disagreed about the owner, the two views of one run would
/// answer "whose was this" differently, which is the orphan problem the record
/// exists to end, moved one table over.
#[tokio::test(flavor = "multi_thread")]
...
async fn a_failed_run_leaves_no_mission_workspace_behind() {
    ...
    // …and no run record either (#7752). This failure is driver resolution,
    // which happens before `run_agent_loop` is entered, so no turn ran and
    // there is nothing to report a cost or an answer for. That is the line the
    // record draws: a run record exists exactly when a run actually started.
