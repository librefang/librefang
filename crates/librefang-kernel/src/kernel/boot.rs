...
impl LibreFangKernel {
    ...
    pub fn boot_with_config_at(
        ...
        // #7911: bound the per-turn episodic memory row. The runtime's
        // per-turn writer reads this off the substrate it already holds, so
        // the value does not have to be threaded through `LoopOptions` at
        // every construction site. `[memory]` is restart-required in
        // `build_reload_plan`, so a value read once here cannot go stale
        // relative to the config on disk.
