...
//!
//! `reset_session` (`/new`) is async, but the summary write it triggers is
//! **detached**: `save_session_summary` does `Handle::try_current()` +
//! `handle.spawn(...)` so the aux-LLM digest happens off the reset's return
//! path. That is correct on the daemon, whose runtime outlives every request.
//!
//! The TUI drove `reset_session` with a per-call `tokio::runtime::Runtime::new()`
//! and dropped it as soon as `block_on` returned. Dropping a runtime aborts
//! its tasks, so a summary write still in flight was killed — no panic, no
//! warning, just a missing `session_<sid>` row. The `try_current()` branch was
//! even *taken*, which is why "no warning in the log" never surfaced it.
//!
//! **Whether the row is actually lost depends on config**, which is what makes
//! this bug so quiet. `build_session_summary` returns early with a trivial
//! digest when `[llm.auxiliary] session_summary` is unconfigured — that path
//! has no `.await` before the write, so the task finishes inside the scheduler
//! window `reset_session`'s remaining async work provides, and the row lands
//! even on a throwaway runtime. Configure an aux chain and the same task now
//! awaits an LLM round-trip, so the drop aborts it and the summary is gone.
//! The loss hits precisely the operators who paid for good summaries.
//!
//! A test that configures a real aux chain would need a live LLM, so what is
//! pinned here is the fix's guarantee — a runtime that outlives the call lets
//! the write complete regardless of which path it takes. The abort semantics
//! themselves are covered by `librefang-cli`'s
//! `block_on_tui_detached_tasks_survive_the_call`, which uses an explicit
//! await point and fails when the shared runtime is swapped for a throwaway.
...
/// Poll for the detached summary write to land. The write is async by design,
/// so a bounded wait is the honest check — a fixed sleep would either be flaky
/// or slow.
fn wait_for_summary(
...
/// The fix: driving `reset_session` on a runtime that outlives the call lets
/// the detached summary write finish.
///
/// This is the shape `tui::event::block_on_tui` produces — a process-lifetime
/// runtime shared by every TUI operation.
#[test]
