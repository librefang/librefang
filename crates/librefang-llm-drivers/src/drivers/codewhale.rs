...
mod tests {
    ...
    fn with_message_timeout_overrides_default() {
        let driver = CodeWhaleDriver::new(None, false).with_message_timeout(21);
        assert_eq!(driver.message_timeout_secs, 21);
    }
    ...
    async fn complete_honors_request_timeout() {
        ...
        let driver = CodeWhaleDriver::new(Some(cli.to_string_lossy().into_owned()), false);
        let request = CompletionRequest {
            model: "codewhale".to_string(),
            timeout_secs: Some(0),
            ..Default::default()
        };
