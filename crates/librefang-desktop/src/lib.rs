...
/// Desktop-only, for the same reason `ServerHandleHolder` is: its only caller
/// sits behind the same `cfg`. Without this attribute the function is still
/// compiled for iOS and Android, where nothing calls it, and `-D warnings`
/// turns the resulting `dead_code` into a hard error — which is how it broke
/// the iOS simulator build on `main` while every desktop lane stayed green.
#[cfg(not(any(target_os = "ios", target_os = "android")))]
...
mod tests {
    ...
    /// Gated with the helper it exercises: on iOS and Android `lock_server_handle`
    /// is not compiled, so an ungated test fails to resolve the symbol there.
    #[cfg(not(any(target_os = "ios", target_os = "android")))]
    #[test]
