...
pub trait KnowledgeGraph: Send + Sync {
    ...
    /// `agent_id` scopes the entity to its owning agent so the agent-scoped
    /// relations read and `delete_by_agent` see it (the empty string is the
    /// shared/unscoped sentinel). Callers thread the turn's `caller_agent_id`.
    ...
    /// `agent_id` scopes the relation to its owning agent (see
    /// [`knowledge_add_entity`](Self::knowledge_add_entity)).
