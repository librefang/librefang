...
pub(super) async fn tool_text_to_speech(
    ...
    if let Some(cache) = media_drivers {
        ...
        // Provider-specific config overrides: inject config defaults when the
        // tool call didn't supply an explicit value, so provider drivers get
        // the operator's chosen settings rather than their own hard-coded
        // fallbacks.
        let (
            effective_voice,
            effective_language,
            ...
            // ElevenLabs: when the tool call omits `format`, inject the
            // config's `output_format` (default `opus_48000_32`) so the
            // MediaDriver path produces Ogg/Opus for WhatsApp PTT.
