...
pub(super) async fn tool_workflow_run(
    ...
    // A nesting-depth refusal arrives as `CapabilityDenied` (refs #6659) and
    // must stay a policy error rather than becoming `Upstream`. Two reasons, the
    // same ones spelled out in `tool_agent_send`: `Upstream` lifts to a 5xx-class
    // `ToolExecution` that reads as a downstream crash to retry logic, and
    // `PermissionDenied` classifies as `ToolExecutionStatus::Denied` — a soft
    // failure — so a capped agent that keeps trying does not burn through
    // `MAX_CONSECUTIVE_ALL_FAILED` and lose the turn to an abort. Every other
    // kernel failure stays `Upstream`.
