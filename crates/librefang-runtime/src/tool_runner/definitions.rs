...
use instead of web_fetch + file_write (which round-trips the entire body through the model)."
            ...
            ToolDefinition {
                ...
                description: "Click an element on the current browser page by link marker, CSS selector, or visible text. Returns the resulting page state. Prefer the ⟨n⟩ marker from the page's link table: it identifies one exact link, where matching on visible text picks the first element merely containing it.".to_string(),
                input_schema: serde_json::json!({
                    "type": "object",
                    "properties": {
