            ...
            ToolDefinition {
                ...
                input_schema: serde_json::json!({
                    ...
                    "properties": {
                        ...
                        "output_format": { "type": "string", "enum": librefang_types::config::TTS_OUTPUT_FORMATS, "description": "Final output format. 'ogg_opus' converts to OGG Opus via ffmpeg (required for voice notes on WhatsApp and other messaging channels); falls back to provider format if ffmpeg is unavailable or conversion fails. Omit to use the operator's `[tts] output_format` setting, which itself defaults to 'mp3'." },
