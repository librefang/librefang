...
async fn run_full_mode_approval_case(
    ...
    let count = approval_requests.load(Ordering::SeqCst);
    (result, count)
}
...
async fn test_shell_exec_full_mode_skips_approval_by_default() {
    ...
    assert!(
        policy.full_mode_skips_approval,
        "the default must preserve the historical Full-waives-approval coupling"
    );

    let (result, approvals) =
        run_full_mode_approval_case(&policy, "echo full-default-ok", None, None).await;

    assert!(
        !result.content.contains("requires human approval"),
        "Full with the flag at its default must still bypass approval, got: {}",
        result.content
    );
...
async fn test_shell_exec_full_mode_honours_require_approval_when_flag_disabled() {
    ...
    let (result, approvals) =
        run_full_mode_approval_case(&policy, "echo full-flag-off", None, None).await;

    assert!(
        result.content.contains("requires human approval"),
        "with full_mode_skips_approval = false the global require_approval list \
         must be honoured, got: {}",
        result.content
    );
...
async fn test_shell_exec_full_mode_flag_disabled_skips_when_not_in_require_list() {
    ...
    let (result, approvals) =
        run_full_mode_approval_case(&policy, "echo full-not-required", None, Some(false)).await;

    assert!(
        !result.content.contains("requires human approval"),
        "nothing in require_approval means nothing to prompt for, got: {}",
        result.content
    );
    assert_eq!(approvals, 0);
}
...
async fn test_safe_bins_skip_approval_unaffected_by_full_mode_flag() {
    for full_mode_skips_approval in [true, false] {
        ...
        let (result, approvals) = run_full_mode_approval_case(&policy, "env", None, None).await;

        assert!(
            !result.content.contains("requires human approval"),
            "safe_bins_skip_approval must be unaffected by \
             full_mode_skips_approval = {full_mode_skips_approval}, got: {}",
            result.content
        );
        assert_eq!(
            approvals, 0,
            "safe_bins_skip_approval must be unaffected by \
             full_mode_skips_approval = {full_mode_skips_approval}"
        );
    }
}
...
async fn test_shell_exec_full_mode_respects_rbac_needs_approval() {
    ...
    assert!(policy.full_mode_skips_approval);
    ...
    assert!(
        result.content.contains("requires human approval"),
        "an RBAC NeedsApproval gate must override the Full waiver, got: {}",
        result.content
    );
    assert_eq!(approvals, 1);
}
