...
fn build_channel_section(
    ...
    // is available AND the channel is a real messaging adapter. System
    // channels deliver files and media through the normal response stream —
    // telling the agent to use channel_send to "webui" with a client IP as
    // recipient would fail (no adapter) and push the agent to fall back to
    // Telegram.
