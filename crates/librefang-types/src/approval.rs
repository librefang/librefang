...
pub struct ApprovalPolicy {
    ...
    /// When `true`, `apply_shorthands` clears `require_approval` so no tool is
    /// gated. Applied whenever the policy is installed into an
    /// `ApprovalManager` (boot, hot-reload, construction) — see `#6492`.
    #[serde(default)]
