...
mod tests {
    ...
    fn decision_rename_all() {
        ...
        assert!(json.contains("timed_out"), "expected snake_case: {json}");
        let json = serde_json::to_string(&ApprovalDecision::Skipped).unwrap();
        assert!(json.contains("skipped"), "expected snake_case: {json}");
    }
