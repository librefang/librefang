...
// ============================================================================
// Type Cast Functions (real[] <-> ruvector)
// ============================================================================

/// Cast real[] to ruvector type
///
/// Assignment cast from float arrays (e.g. `ruvector_embed()` output)
/// to the native ruvector type used in vector columns and HNSW indexes.
/// Fires automatically during INSERT/UPDATE; requires explicit `::ruvector`
/// in other expression contexts.
///
/// # Example
/// ```sql
/// -- Assignment context (automatic):
/// INSERT INTO docs(embedding) VALUES (ruvector_embed('hello'));
///
/// -- Explicit cast in expressions:
/// SELECT ruvector_embed('hello')::ruvector;
/// ```
#[pg_extern(immutable, parallel_safe)]
pub fn ruvector_from_real_array(arr: Vec<f32>) -> RuVector {
    if arr.is_empty() {
        pgrx::error!("Cannot create ruvector from empty array");
    }
    if arr.len() > crate::MAX_DIMENSIONS {
        pgrx::error!(
            "Array length {} exceeds maximum ruvector dimensions {}",
            arr.len(),
            crate::MAX_DIMENSIONS
        );
    }
    // Validate in place — from_slice will do the single copy
    for (i, &val) in arr.iter().enumerate() {
        if val.is_nan() {
            pgrx::error!("NaN not allowed at position {}", i);
        }
        if val.is_infinite() {
            pgrx::error!("Infinity not allowed at position {}", i);
        }
    }
    RuVector::from_slice(&arr)
}

/// Cast ruvector to real[] type
///
/// Enables conversion from native ruvector back to a float array
/// for use with array-based functions.
///
/// # Example
/// ```sql
/// SELECT ruvector_to_real_array(embedding) FROM docs LIMIT 1;
/// -- Or via cast:
/// SELECT embedding::real[] FROM docs LIMIT 1;
/// ```
#[pg_extern(immutable, parallel_safe)]
pub fn ruvector_to_real_array(v: RuVector) -> Vec<f32> {
    v.into_vec()
}
...
mod tests {
    ...
    #[pg_test]
    fn test_ruvector_from_real_array() {
        let v = ruvector_from_real_array(vec![1.0, 2.0, 3.0]);
        assert_eq!(v.dimensions(), 3);
        assert_eq!(v.as_slice(), &[1.0, 2.0, 3.0]);
    }

    #[pg_test]
    fn test_ruvector_from_real_array_large() {
        let arr: Vec<f32> = (0..384).map(|i| i as f32 / 384.0).collect();
        let v = ruvector_from_real_array(arr.clone());
        assert_eq!(v.dimensions(), 384);
        assert!((v.as_slice()[0] - 0.0).abs() < 1e-6);
        assert!((v.as_slice()[383] - 383.0 / 384.0).abs() < 1e-5);
    }

    #[pg_test]
    fn test_ruvector_to_real_array() {
        let v = RuVector::from_slice(&[4.0, 5.0, 6.0]);
        let arr = ruvector_to_real_array(v);
        assert_eq!(arr, vec![4.0, 5.0, 6.0]);
    }

    #[pg_test]
    fn test_cast_roundtrip() {
        let original = vec![1.5, 2.5, 3.5, 4.5];
        let v = ruvector_from_real_array(original.clone());
        let back = ruvector_to_real_array(v);
        assert_eq!(original, back);
    }

    #[pg_test]
    #[should_panic(expected = "Cannot create ruvector from empty array")]
    fn test_cast_rejects_empty() {
        ruvector_from_real_array(vec![]);
    }

    #[pg_test]
    #[should_panic(expected = "NaN not allowed")]
    fn test_cast_rejects_nan() {
        ruvector_from_real_array(vec![1.0, f32::NAN, 3.0]);
    }

    #[pg_test]
    #[should_panic(expected = "Infinity not allowed")]
    fn test_cast_rejects_pos_inf() {
        ruvector_from_real_array(vec![1.0, f32::INFINITY, 3.0]);
    }

    #[pg_test]
    #[should_panic(expected = "Infinity not allowed")]
    fn test_cast_rejects_neg_inf() {
        ruvector_from_real_array(vec![1.0, f32::NEG_INFINITY, 3.0]);
    }

    #[pg_test]
    #[should_panic(expected = "exceeds maximum ruvector dimensions")]
    fn test_cast_rejects_over_max_dimensions() {
        let oversized: Vec<f32> = vec![0.0; crate::MAX_DIMENSIONS + 1];
        ruvector_from_real_array(oversized);
    }
