...
-- ============================================================================
-- Embedding functions (require 'embeddings' feature flag at compile time)
-- Build with: cargo pgrx package --features embeddings
-- ============================================================================

-- Generate embedding for text using local ONNX models (no API call)
...
RETURNS TABLE(name text, dimensions integer, description text, speed integer, quality integer, memory_mb integer, loaded boolean)
...
-- Fires automatically in INSERT/UPDATE; requires explicit ::ruvector elsewhere
CREATE CAST (real[] AS ruvector)
    WITH FUNCTION ruvector_from_real_array(real[])
    AS ASSIGNMENT;

-- Explicit cast: ruvector -> real[] (requires ::real[])
CREATE CAST (ruvector AS real[])
    WITH FUNCTION ruvector_to_real_array(ruvector);
