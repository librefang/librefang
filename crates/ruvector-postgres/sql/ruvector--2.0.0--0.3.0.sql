...
-- ============================================================================
-- Type Casts (real[] <-> ruvector)
-- ============================================================================

-- Cast function: real[] -> ruvector
CREATE OR REPLACE FUNCTION ruvector_from_real_array(real[])
RETURNS ruvector
AS 'MODULE_PATHNAME', 'ruvector_from_real_array_wrapper'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

-- Cast function: ruvector -> real[]
CREATE OR REPLACE FUNCTION ruvector_to_real_array(ruvector)
RETURNS real[]
AS 'MODULE_PATHNAME', 'ruvector_to_real_array_wrapper'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

-- Assignment cast: real[] -> ruvector
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_cast WHERE castsource = 'real[]'::regtype AND casttarget = 'ruvector'::regtype) THEN
        CREATE CAST (real[] AS ruvector) WITH FUNCTION ruvector_from_real_array(real[]) AS ASSIGNMENT;
    END IF;
END $$;

-- Explicit cast: ruvector -> real[]
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_cast WHERE castsource = 'ruvector'::regtype AND casttarget = 'real[]'::regtype) THEN
        CREATE CAST (ruvector AS real[]) WITH FUNCTION ruvector_to_real_array(ruvector);
    END IF;
END $$;
