...
/// Sentinel returned by `describe_image` while the real provider HTTP
/// path is not yet wired. Callers that thread the description into an
/// LLM prompt (channel bridge enricher, agents.rs upload enricher)
/// match on this string and treat it as a provider failure (image
/// passes through unannotated). Public so the upstream enrichers can
/// import it without copying the literal.
...
impl MediaEngine {
    ...
    ///
    /// NOTE — current implementation is a stub: the real HTTP call to the
    /// chosen vision provider is not yet wired (tracked separately from
    /// PR #5239, which only landed the *caller* plumbing). Returning a
    /// placeholder description string here would silently pollute every
    /// inbound channel image's prompt with literal text like
    /// `"[Image description would be generated by anthropic provider]"`,
    /// which is strictly worse than not enriching at all. Until the HTTP
    /// path lands, this function returns `Err(NOT_IMPLEMENTED_SENTINEL)`,
    /// which both enrichers (channel bridge + agents.rs upload path) treat
    /// the same way as a provider 5xx — the image is passed through
    /// unannotated and the LLM still sees the inline `ImageFile`. The
    /// sentinel is also explicitly matched by the bridge / agents.rs
    /// helpers so the placeholder can never reach a prompt by accident
    /// even if a downstream caller decides to log the description text.
    pub async fn describe_image(
        ...
        // Determine which provider to use — we still resolve so a
        // misconfigured deployment surfaces the "no provider" Err shape
        // the same way as the real path will once implemented. Resolve
        // into an owned `String` because the explicit-config arm and the
        // env-var detect arm have different lifetimes (`&self`-bound vs
        // `'static`).
        let _provider: String = match self.config.image_provider.as_deref() {
            Some(p) => p.to_string(),
            None => detect_vision_provider()
                .ok_or("No vision-capable LLM provider configured. Set [media] image_provider in config.toml.")?
                .to_string(),
        };

        Err(NOT_IMPLEMENTED_SENTINEL.to_string())
    }
...
///
/// Currently unused — `describe_image` returns the `NOT_IMPLEMENTED_SENTINEL`
/// `Err` before reaching a model lookup. Kept (with `#[allow(dead_code)]`)
/// so the follow-up PR that wires the real HTTP path doesn't have to
/// re-derive the per-provider default model table.
#[allow(dead_code)]
...
mod tests {
    ...
    /// Pin the public `NOT_IMPLEMENTED_SENTINEL` value. The
    /// `librefang-api` upload-path enricher mirrors the same literal
    /// (it doesn't take a dep on this crate, only the kernel does) and
    /// has its own pin test against the same string. A drift here
    /// without a matching change there would let the stub placeholder
    /// leak into every inbound channel-image prompt — the original B1
    /// blocker on PR #5239.
    #[test]
    fn stub_sentinel_pin() {
        assert_eq!(
            NOT_IMPLEMENTED_SENTINEL,
            "describe_image: not yet implemented (stub)"
        );
    }

    /// `describe_image` must surface the sentinel via `Err`, not via the
    /// `Ok(MediaUnderstanding { description, .. })` payload. If the
    /// future returned `Ok(description = sentinel)` instead, the
    /// channel-bridge / agents.rs enrichers would happily wrap it in
    /// `<image_description>` tags and the stub text would leak to the
    /// LLM. Belt-and-braces guard on top of the upstream
    /// `is_describe_text_usable` predicate.
    ///
    /// Uses an explicit `image_provider` in `MediaConfig` to bypass
    /// the env-var auto-detection — no `set_var` in async tests, per
    /// project rule.
    #[tokio::test]
    async fn describe_image_returns_sentinel_via_err_not_ok() {
        let cfg = MediaConfig {
            image_provider: Some("gemini".into()),
            ..MediaConfig::default()
        };
        let engine = MediaEngine::new(cfg);
        // Tiny PNG header bytes encoded as base64 — `validate()` only
        // checks the MIME prefix + size cap, no decode, so any valid
        // base64 string is fine for this code path.
        let attachment = MediaAttachment {
            media_type: MediaType::Image,
            mime_type: "image/png".into(),
        ...
        match result {
            Err(reason) => assert_eq!(
                reason, NOT_IMPLEMENTED_SENTINEL,
                "stub must Err with the sentinel; got {reason:?}"
            ),
            Ok(_) => panic!(
                "stub describe_image must Err with NOT_IMPLEMENTED_SENTINEL, \
                 not Ok(...) — otherwise the placeholder leaks into prompts"
            ),
        }
    }
