...
impl CredentialVault {
    ...
    pub fn init(&mut self) -> ExtensionResult<()> {
        ...
        // BossFang fork: the env branch inside `resolve_master_key` uses
        // `vault_key_from_env()`, which checks BOSSFANG_VAULT_KEY first and
        // falls back to LIBREFANG_VAULT_KEY — that precedence is preserved.
