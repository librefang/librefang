...
//! or the `BOSSFANG_VAULT_KEY` env var for headless/CI environments
//! (`LIBREFANG_VAULT_KEY` accepted as a legacy fallback).
...
/// Primary env var for vault key (BossFang).
...
impl CredentialVault {
    ...
    fn resolve_master_key(&self) -> ExtensionResult<Zeroizing<[u8; 32]>> {
        ...
        // BossFang: check BOSSFANG_VAULT_KEY first, fall back to LIBREFANG_VAULT_KEY.
        let env_key = vault_key_from_env().ok().map(Zeroizing::new);
        ...
        match classify_master_key_sources(
            ...
            MasterKeySource::EnvOverridesDifferentKeyring => {
                ...
                tracing::warn!(
                    ...
                    env = %VAULT_KEY_ENV,
                    "both BOSSFANG_VAULT_KEY env var AND OS keyring are set, \
                     and they DISAGREE; env wins (current behaviour). \
                     If you intended the keyring value to take effect, unset \
                     BOSSFANG_VAULT_KEY (or LIBREFANG_VAULT_KEY) on the daemon process."
                );
            }
            MasterKeySource::EnvMatchesKeyring => {
                tracing::debug!(
                    target: "vault::keyring",
                    "env BOSSFANG_VAULT_KEY (or LIBREFANG_VAULT_KEY) matches OS keyring value; using env source"
                );
            }
            MasterKeySource::EnvOnly => {
                tracing::debug!(
                    target: "vault::keyring",
                    "vault master key sourced from BOSSFANG_VAULT_KEY env var \
                     (or LIBREFANG_VAULT_KEY fallback; no OS keyring entry present)"
                );
            }
            MasterKeySource::KeyringOnly => {
                tracing::debug!(
                    ...
                    "vault master key sourced from OS keyring \
                     (no BOSSFANG_VAULT_KEY or LIBREFANG_VAULT_KEY env var set)"
                );
            }
