...
pub fn record_http_request(path: &str, method: &str, status: u16, duration: Duration) {
    ...
    let counters = HTTP_REQUEST_COUNTS.entry(key.clone()).or_insert_with(|| Arc::new(AtomicU64::new(0)));
    counters.fetch_add(1, Ordering::Relaxed);
    
    let latencies = HTTP_REQUEST_LATENCIES.entry(key).or_insert_with(|| Arc::new(RwLock::new(Vec::with_capacity(1000))));
...
pub fn get_http_metrics_summary() -> String {
    ...
    output.push_str("# HELP librefang_http_requests_total Total HTTP requests by method, path, and status.\n");
    ...
    output.push('\n');
    output.push_str("# HELP librefang_http_request_duration_ms HTTP request duration in milliseconds.\n");
