...
- Per-adapter: `channel-discord`, `channel-slack`, `channel-webhook`, etc. (ntfy, telegram, gotify, mastodon, bluesky, reddit, and twitch migrated to sidecars — see `librefang.sidecar.adapters.{ntfy,telegram,gotify,mastodon,bluesky,reddit,twitch}` in the SDK.)
