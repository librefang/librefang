...
impl FeishuAdapter {
    ...
    async fn api_send_message(
        ...
        let label = self.region.label();
        let token = self.get_token().await?;
        let url = format!("{}?receive_id_type={}", self.send_url(), receive_id_type);
                    ...
                    move |body: axum::extract::Json<serde_json::Value>| {
                        ...
                        async move {
                            ...
                            if let Some(schema) = body.0["schema"].as_str() {
                                if schema == "2.0" {
                                    if let Some(mut msg) = parse_feishu_event(&body.0, region) {
                        ...
                        msg = ws_rx.next() => {
                            match msg {
                                Some(Ok(tokio_tungstenite::tungstenite::Message::Text(text))) => {
                                    ...
                                    if let Some(mut channel_msg) = parse_feishu_event(&payload, region) {
                                        if let Some(ref aid) = *account_id {
