    ...
    match crate::password_hash::verify_dashboard_password(
        ...
        crate::password_hash::VerifyResult::Ok {
            ...
            if let Some(ref hash) = upgrade_hash {
                ...
                match std::fs::write(&hint_path, body) {
                    Ok(()) => {
                        #[cfg(unix)]
                        {
                            ...
                            if let Err(e) = std::fs::set_permissions(
                                ...
                                tracing::warn!(
                                    ...
                                    error = %e,
                                    "Failed to chmod 0600 on dashboard upgrade-hint file — \
                                     the hash inside is operator-controlled; delete the file \
                                     after persisting the value to config.toml",
                                );
                            }
                        }
                        tracing::info!(
