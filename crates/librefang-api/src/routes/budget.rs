...
pub async fn update_agent_budget(
    ...
    if hourly.is_none() && daily.is_none() && monthly.is_none() && tokens.is_none() {
        return ApiErrorResponse::bad_request("Provide at least one of: max_cost_per_hour_usd, max_cost_per_day_usd, max_cost_per_month_usd, max_llm_tokens_per_hour").into_response();
    }
