...
//! * Auth is **not** public. Every request goes through the authenticated middleware path, and every mutating call under `/api/groups*` is forced to `Owner` by `middleware::is_owner_only_write` — the same gate as `/api/users*`, for the same reason: group membership will confer roles (#7746), so an Admin API key that could `POST /api/groups` with `roles = ["owner"]` and add itself as a member would be a self-promotion path. `GET` stays on the middleware's generic GET rule, which admits every authenticated role including `Viewer`, so the dashboard's group list works for whatever role opens it — what it returns is the group roster and the roles each group confers, not credentials.
//!
