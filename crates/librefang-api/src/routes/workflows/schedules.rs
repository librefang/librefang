...
pub async fn create_schedule(
    ...
    // Primary output destination. Omitted or null keeps the historical
    // `None` (fire and forget) rather than inheriting the `LastChannel`
    // default `cron_create` applies: an existing client that posts no
    // `delivery` must not start delivering output it never asked for.
    // Shape errors return 400 here for the same reason `delivery_targets`
    // does — `add_job` would otherwise reject the SSRF-blocked hosts deeper
    // in, where the caller cannot tell a bad URL from a server fault.
    ...
    // `session_mode` accepts only the serde spellings of `SessionMode`
    // (`"persistent"` / `"new"`). A misspelling used to fall through
    // `.ok()` to `None`, silently giving the job the agent's default
    // instead of the isolation the caller asked for.
    ...
    // `peer:{user_id}:KEY` memory lookups resolve (`cron_tick` reads it).
    // An empty string is rejected rather than stored: `peer_scoped_key`
    // refuses an empty peer as ambiguous with global scope, so it would fail
    // at fire time instead of at the boundary that can still report it.
    ...
    match state.kernel.cron().add_job(job.clone(), false) {
        ...
        // `add_job` funnels every `validate_with_home` rejection — including
        // the SSRF host checks that `delivery` reaches now that it is
        // settable here — through `InvalidInput`. Those are caller errors, so
        // they must map to 400; `internal_scrub` alone turned a refused
        // webhook host into a 500 and hid the reason. Mirrors the same split
        // in `update_schedule` below (#4732).
...
pub async fn update_schedule(
    ...
    // Primary output destination. Same null-vs-omitted contract as
    // `delivery_targets`: omitted or null leaves the existing value alone
    // (the kernel's `update_job` skips a null patch), so clearing delivery
    // means sending the explicit `{"kind": "none"}` variant.
    //
    // The shape is deserialized here rather than left to the kernel because
    // `update_job` reports a serde failure as `LibreFangError::Internal`,
    // which the match below turns into a 404 "Schedule not found" — a
    // misleading status for a payload the caller can fix. Validating at the
    // boundary yields the 400 that `create_schedule` returns for the same
    // body. SSRF host rejection stays in the kernel, where it surfaces as
    // `InvalidInput` and is already mapped to 400.
    ...
    // `peer_id` and `session_mode` cannot be patched: `update_job` has no
    // branch for either, and every branch it does have follows an
    // omitted-or-null-means-untouched convention that cannot express
    // "clear this optional field" — so supporting them needs presence-based
    // semantics inside the kernel, not a wider `updates` map here.
    //
    // Rather than drop such a request on the floor, say so. Silently
    // accepting a field that has no effect is the same class of defect as
    // the read gap this issue reported, and a 200 would tell the caller
    // their change landed. A value equal to what is already stored is
    // accepted as the no-op it is, so the natural GET-mutate-PUT round trip
    // — which now echoes both fields back because the read half of #6611
    // added them — keeps working.
    //
    // Malformed shapes are rejected regardless of whether the job exists,
    // but the "cannot be changed" comparison only runs when it does: for an
    // unknown id the request has to fall through to `update_job` so the
    // response is the 404 the caller needs, not a 400 about a field.
    ...
    if let Some(requested) = req.get("session_mode") {
        ...
        if let Some(job) = current.as_ref() {
            // Compare *effective* modes, not the raw `Option`. `None` and
            // `Some(Persistent)` resolve identically — `cron_fire_session_override`
            // only diverges on `Some(New)` — so a caller that spells the default
            // out is asking for no change and must not be refused. The stored
            // value is `None` far more often than `Some(Persistent)` (that is
            // what `skip_serializing_if` on the field implies), which makes this
            // the likely shape of a hand-written round trip rather than an edge
            // case.
