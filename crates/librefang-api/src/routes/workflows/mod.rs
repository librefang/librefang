...
fn cron_job_to_schedule_json(job: &librefang_types::scheduler::CronJob) -> serde_json::Value {
    ...
    // `/api/cron/jobs` serializes the whole `CronJob`, so it carries `delivery`,
    // `peer_id`, and `session_mode`; this view is the alternate presentation of
    // the same store and had fallen behind on all three (#6611).
    // `peer_id` selects the `SenderContext.user_id` the fire runs under, so
    // peer-keyed memory resolves; `session_mode` decides whether every fire
    // shares one persistent session or gets an isolated one; `delivery` is the
    // primary output destination and is still read at fire time
    // (`kernel::cron_tick` clones it alongside `delivery_targets`, which
    // fan out in addition to it rather than replacing it).
    let delivery =
        serde_json::to_value(&job.delivery).unwrap_or_else(|_| serde_json::json!({"kind": "none"}));
    // `CronJob::session_mode` carries `skip_serializing_if = "Option::is_none"`,
    // which is why an unset value is simply absent from `/api/cron/jobs` rather
    // than null. This view emits an explicit `null` instead, matching how it
    // already treats `tz`, `last_run`, and `next_run`: a read surface with a
    // stable key set lets a client distinguish "not configured" from "the
