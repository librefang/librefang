...
pub(crate) fn enrich_agent_json(
    ...
    serde_json::json!({
        ...
        // #7930: `parent_agent_id` used to be `null` for every agent after a
        // daemon restart, because nothing persisted the link. It now round-trips
        // through `agents.parent_id`. `parent_unknown` distinguishes the two
        // reasons the field can be `null`: `false` means the agent really has no
        // parent, `true` means the row predates schema v54 and its lineage was
        // never recorded — a client must not render the latter as a root agent.
