...
//!
//! BossFang note: this SQLite `mcp_server_configs` path is the legacy
//! (`sqlite-backend`) write target. Under the fork's default `surreal-backend`,
//! `/api/mcp/servers` writes are routed to the SurrealDB config store
//! (`config_store_overlay::write_mcp_servers`, key `mcp_servers`) regardless of
//! `mcp_runtime_store`, so the assertions below about the SQLite table do not
//! hold there. The SurrealDB equivalent (read-only `config.toml` →
//! DB-backed runtime writes) is covered by `config_store_overlay_test`. Gate the
//! whole file off under `surreal-backend`.
