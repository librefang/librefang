...
/// #6629: the two store-scanning fallbacks are gone, so an unproven request is
/// a 400 — pinned exactly, not as "some 4xx".
///
/// Each body below used to succeed at *selecting a session*: `{provider}` hit
/// `find_by_provider` (an arbitrary entry for that provider) and `{}` hit
/// `find_any_with_refresh` (literally the first entry in the map with a refresh
/// token). Either would refresh a different local user's upstream session and
/// hand the caller their access and rotated refresh tokens.
///
/// 400 rather than 502 is also the assertion that no outbound call fired: the
/// provider's `token_url` is `example.invalid`, so a handler that reached the
/// network would surface 502 (or hang).
#[tokio::test(flavor = "multi_thread")]
