    ...
    <div className="flex flex-col gap-6 transition-colors duration-300">
      ...
      {modelsQuery.isLoading ? (
        ...
        <div className="rounded-2xl border border-border-subtle overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[1fr_90px_70px_70px_70px_40px_40px_40px] gap-2 px-4 py-2.5 bg-main text-[9px] font-bold text-text-dim/60 uppercase">
            ...
            <span className="text-center" title="Tools"><Wrench className="w-3 h-3 inline" /></span>
            <span className="text-center" title="Vision"><Eye className="w-3 h-3 inline" /></span>
            <span className="text-center" title="Streaming"><Zap className="w-3 h-3 inline" /></span>
          </div>

          {paged.map((m, i) => (
            <div key={m.id}
              className={`grid grid-cols-[1fr_90px_70px_70px_70px_40px_40px_40px] gap-2 px-4 py-2.5 items-center border-t border-border-subtle/50 hover:bg-surface transition-colors ${
              ...
              <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md w-fit ${tierColor(m.tier)}`}>{m.tier || "-"}</span>
              ...
              <span className="text-center">{m.supports_tools ? <Check className="w-3 h-3 text-success inline" /> : <X className="w-3 h-3 text-text-dim/15 inline" />}</span>
              <span className="text-center">{m.supports_vision ? <Check className="w-3 h-3 text-success inline" /> : <X className="w-3 h-3 text-text-dim/15 inline" />}</span>
              <span className="text-center">{m.supports_streaming ? <Check className="w-3 h-3 text-success inline" /> : <X className="w-3 h-3 text-text-dim/15 inline" />}</span>
            </div>
          ))}
        </div>
      )}
