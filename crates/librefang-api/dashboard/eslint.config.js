...
export default [
  ...
  // React + React Hooks for .ts/.tsx — applied to source only.
  {
    ...
    rules: {
      ...
      // The API is undefined outside a secure context, so on a dashboard
      // served over plain HTTP on a LAN / VPN address the property access
      // throws before the promise exists — the copy button does nothing and
      // says nothing. `lib/clipboard.ts` already falls back to
      // `document.execCommand('copy')`; this rule exists because six call
      // sites reintroduced the raw API after that helper landed (#6611).
      //
      // Coverage, measured against this config rather than inferred from it.
      // Caught: `navigator.clipboard…`, `navigator?.clipboard?.…`, and
      // `const { clipboard } = navigator` — `no-restricted-properties`
      // checks destructuring patterns as well as member reads.
      // `no-restricted-syntax` adds the `<obj>.navigator.clipboard` spelling
      // (`window.` / `globalThis.` prefixed), which `no-restricted-properties`
      // cannot see because its `object` option only matches a bare identifier
      // and here the object is itself a member expression.
      // Not caught: an aliased receiver (`const n = navigator; n.clipboard`)
      // and `const { clipboard } = window.navigator`. Both need type-aware
      // alias tracking; these are syntactic rules with no notion of what an
      // identifier was assigned. Nobody writes either shape in this codebase,
      // and the review that catches a hand-rolled copy path at all catches
      // those too.
  ...
  // The clipboard helper is the one place allowed to touch the raw API —
  // it is the fallback the `no-restricted-properties` rule above points
  // everyone else at. Only that rule is disabled: the helper reads a bare
  // `navigator.clipboard`, so the `window.navigator.clipboard` spelling that
  // `no-restricted-syntax` covers stays banned even here.
  {
