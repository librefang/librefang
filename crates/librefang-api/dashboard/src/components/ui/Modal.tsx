...
///
/// Anything that closes on Escape and can be stacked belongs here rather than owning a `keydown` listener of its own.
/// Two competing listeners cannot agree on who is on top: `handleModalEscape` calls `stopImmediatePropagation`, so whichever registered first silently wins the key and the other's handler never runs.
/// That is how Escape on a `ConfirmDialog` opened over a `Modal` used to close both (#8336).
...
///
/// Exported so a layer can gate its *other* key handling on being on top — `ConfirmDialog`'s Enter-confirms shortcut must not fire while something is stacked above it, for the same reason Escape must not reach through.
...
///
/// The single `keydown` listener is attached while the stack is non-empty and removed when it drains, so exactly one handler ever sees the key and the topmost layer always wins regardless of mount order.
