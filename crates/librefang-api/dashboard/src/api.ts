...
export interface ChannelItem {
  ...
  fields?: ChannelField[];
  /** Set on an unconfigured sidecar row when `--describe` failed at
   *  daemon boot and there is no static fallback — i.e. `fields` is
   *  empty and the configure form would otherwise be a blank drawer.
   *  Carries the actionable reason (typically: the Python sidecar SDK
   *  is not installed), surfaced in the configure form so the operator
   *  knows why the form is empty and how to fix it. */
  schema_error?: string;
