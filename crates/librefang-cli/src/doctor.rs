...
mod tests {
    ...
    /// Holds the shared env lock (`crate::test_env`) for the entire body so
    /// concurrent vault-key tests (and any other env-var test in this binary)
    /// don't race. The original value is restored before the lock is released.
