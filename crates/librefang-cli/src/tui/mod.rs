    ...
    fn handle_key(&mut self, key: ratatui::crossterm::event::KeyEvent) {
        ...
        if matches!(self.phase, Phase::Main) {
            ...
            match key.code {
                ...
                // F(8) was the `Channels` tab shortcut before that tab was
                // retired; `Goals` takes the freed slot rather than renumbering
                // F(1)-F(7) and breaking the existing bindings.
                KeyCode::F(8) => {
            ...
            if key.modifiers.contains(KeyModifiers::ALT) {
                match key.code {
                    ...
                    // Char('8') was the Alt-8 `Channels` tab shortcut before
                    // that tab was retired; `Goals` takes the freed slot, in
                    // step with F(8) above.
                    KeyCode::Char('8') => {
                        self.switch_tab(Tab::Goals);
                        return;
