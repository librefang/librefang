...
pub fn run_acp_server(config: Option<PathBuf>, agent: Option<String>) {
    ...
    if let Some(sock) = locate_acp_socket() {
        eprintln!("{}", crate::i18n::t_args("acp-attached-uds", &[("path", &sock.to_string_lossy())]));
    ...
    let kernel = match LibreFangKernel::boot(config.as_deref()) {
        Ok(k) => Arc::new(k),
        Err(e) => {
            eprintln!("{}", crate::i18n::t_args("acp-error-boot-kernel", &[("error", &e.to_string())]));
            std::process::exit(1);
        }
    };
    ...
    let exit_code = rt.block_on(async {
        ...
        let agent_id = match adapter.resolve_agent(agent_name).await {
            Ok(id) => id,
            Err(e) => {
                eprintln!("{}", crate::i18n::t_args("acp-error-resolve-agent", &[("name", agent_name), ("error", &e.to_string())]));
                return 1;
            }
        };
        ...
        match librefang_acp::run(Arc::new(adapter), agent_id).await {
            Ok(()) => 0,
            Err(e) => {
                eprintln!("{}", crate::i18n::t_args("acp-error-server", &[("error", &e.to_string())]));
                1
            }
        }
    });
...
fn run_uds_proxy(sock_path: &std::path::Path) -> i32 {
    ...
    rt.block_on(async {
        let stream = match UnixStream::connect(sock_path).await {
            Ok(s) => s,
            Err(e) => {
                eprintln!("{}", crate::i18n::t_args("acp-error-uds-connect", &[("path", &sock_path.to_string_lossy()), ("error", &e.to_string())]));
                return 1;
            }
        };
...
fn run_pipe_proxy() -> i32 {
    ...
    rt.block_on(async {
        let stream = match ClientOptions::new().open(PIPE_NAME) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("{}", crate::i18n::t_args("acp-error-pipe-connect", &[("name", PIPE_NAME), ("error", &e.to_string())]));
                return 1;
            }
        };
