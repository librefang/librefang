...
fn render_status_inprocess(config: Option<PathBuf>, json: bool, quiet: bool) -> i32 {
    ...
    let _agent_count = count_agent_dirs(&cfg.home_dir);
...
fn render_status_unreachable(home_dir: &std::path::Path, json: bool, quiet: bool) -> i32 {
    let info = match read_daemon_info(home_dir) {
        Some(i) => i,
        None => return 1,
    };
    let pid = info.pid;
    let addr = &info.listen_addr;
    let started = &info.started_at;

    if quiet {
        println!("librefang starting_or_unreachable pid={pid} addr={addr}");
        return 2;
    }
