...
tui-agents-workspaces-help = a 添加 · d 删除 · Enter 编辑 · Tab/Shift+Tab 字段 · s 保存 · Esc 返回
tui-agents-workspaces-empty = 还没有共享文件夹。按 a 添加一个。
tui-agents-workspaces-loading = 正在加载共享文件夹…
tui-agents-workspaces-row-dropped = 名称或路径为空的行将不会保存。再次按 s 保存其余行。
tui-agents-workspaces-row-invalid = 共享文件夹 "{ $name }" 未保存：名称只能使用字母、数字、"-" 和 "_"，路径必须是相对路径且不含 ".."。
tui-agents-workspaces-mode-invalid = "{ $value }" 不是文件夹模式；已保留原值。请使用 "readonly" 或 "readwrite"。
...
tui-event-workspaces-not-available-in-process = 编辑共享文件夹需要正在运行的守护进程。
