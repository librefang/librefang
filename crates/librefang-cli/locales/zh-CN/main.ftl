...
tui-event-promote-missing-pr-url = 响应中缺少 PR URL
tui-event-promote-http-error = { $status }: { $detail }
tui-event-promote-not-available-in-process = 仅在连接到守护进程时才能提升
tui-event-kv-save-failed = 保存键值对失败
tui-event-kv-not-available-in-process = 进程内模式下内存 KV 不可用
tui-event-kv-delete-failed = 删除键值对失败
tui-event-daemon-http-status = HTTP { $status }
tui-event-daemon-failure-detail = { $summary }：{ $detail }
tui-event-dashboard-status-fetch-failed = 加载守护进程状态失败
tui-event-dashboard-audit-fetch-failed = 加载近期活动失败
tui-event-workflows-fetch-failed = 加载工作流失败
tui-event-triggers-fetch-failed = 加载触发器失败
tui-event-sessions-fetch-failed = 加载会话失败
tui-event-agents-fetch-failed = 加载智能体失败
tui-event-kv-fetch-failed = 加载智能体 { $agent_id } 的记忆失败
tui-event-clawhub-search-failed = ClawHub 搜索失败
tui-event-clawhub-browse-failed = 浏览 ClawHub 失败
tui-event-templates-fetch-failed = 加载智能体模板失败
tui-event-providers-fetch-failed = 加载提供商失败
tui-event-security-fetch-failed = 加载安全状态失败
tui-event-audit-fetch-failed = 加载审计记录失败
tui-event-usage-summary-fetch-failed = 加载用量汇总失败
tui-event-usage-by-model-fetch-failed = 加载按模型统计的用量失败
tui-event-usage-by-agent-fetch-failed = 加载按智能体统计的用量失败
tui-event-tools-fetch-failed = 加载工具失败
tui-event-groups-fetch-failed = 加载用户组失败
tui-event-peers-fetch-failed = 加载对等节点失败
tui-event-logs-fetch-failed = 加载日志失败
tui-event-goals-fetch-failed = 加载目标失败
tui-event-hands-fetch-failed = 加载 Hands 失败
tui-event-active-hands-fetch-failed = 加载活动 Hands 失败
tui-event-extensions-fetch-failed = 加载扩展失败
tui-event-extension-health-fetch-failed = 加载扩展健康状态失败
tui-event-comms-topology-fetch-failed = 加载智能体拓扑失败
tui-event-comms-events-fetch-failed = 加载通信事件失败
tui-event-agent-model-label-fetch-failed = 加载智能体 { $agent_id } 的模型失败
...
tui-event-backup-delete-failed = 删除备份 { $filename } 失败
tui-event-backup-restore-failed = 从 { $filename } 恢复失败
...
tui-mod-vault-key-saved = 已存储 { $key }
tui-mod-vault-key-deleted = 已清除 { $key }
tui-mod-vault-key-saved-env-override = 已存储 { $key }，但守护进程从自身环境变量读取该值，存储的值不会被使用
tui-mod-vault-key-deleted-env-override = 已从保险库清除 { $key }，但守护进程仍从自身环境变量读取该值，实际上未撤销任何凭据
tui-mod-key-deleted-for = 已删除 { $name } 的密钥
tui-mod-backup-created = 已创建备份：{ $filename }
tui-mod-backup-deleted = 已删除备份：{ $filename }
tui-mod-backup-restored = 已从 { $filename } 恢复 { $files } 个文件。请重启守护进程以使所有更改生效。
tui-mod-backup-restored-with-errors = 已从 { $filename } 恢复 { $files } 个文件，{ $errors } 个失败。请重启守护进程以使所有更改生效。
...
tui-templates-hints =   [↑↓] 导航  [Enter] 生成智能体  [f] 过滤分类  [r] 刷新  [R] 还原  [v] 版本历史
tui-templates-promote-only-custom = 只有自定义智能体类型才能被提升到注册表
tui-templates-promoting = 正在将 '{ $name }' 提升到注册表…
tui-templates-promoted = 注册表 PR 已打开: { $url }
tui-templates-promote-failed = 无法提升 '{ $name }': { $error }
tui-templates-provider-not-configured = 提供商 '{ $provider }' 未配置。请先在“设置”中设置 API 密钥。
tui-templates-manifest-unavailable = 无法加载 '{ $name }' 的清单。请检查守护进程日志。
tui-templates-restore-invalid-name = 无效的模板名称
...
tui-templates-restore-failed = 无法从注册表还原
tui-templates-history-failed = 无法加载版本历史
tui-templates-history-loading = 正在加载 { $name } 的版本历史…
tui-templates-history-daemon-only = 版本历史需要守护进程模式
tui-templates-history-title = { $name } 的版本历史
tui-templates-history-col-id = 版本 ID
tui-templates-history-col-created = 创建时间
tui-templates-history-col-source = 来源
tui-templates-history-error = 无法加载版本历史：{ $detail }
tui-templates-history-shape = 守护进程的版本历史响应格式异常
tui-templates-history-empty = 暂无版本历史
tui-templates-history-manifest-only = 仅自定义模板支持查看版本历史
tui-templates-version-restoring = 正在将 { $name } 还原到所选版本…
tui-templates-version-restore-failed = 无法还原版本
tui-templates-history-hints = [Esc] 返回  [↑↓] 导航  [Enter] 还原
...
tui-settings-tab-backups = 4 备份
tui-settings-hints-backups =   [↑↓] 导航  [c] 创建  [Enter] 恢复  [d] 删除  [r] 刷新
tui-settings-hints-restore =   [↑↓] 移动  [Space] 切换选择  [Enter] 恢复  [Esc] 取消
tui-settings-backups-header-filename = 归档
tui-settings-backups-header-size = 大小
tui-settings-backups-header-created = 创建时间
tui-settings-backups-header-components = 组件
tui-settings-backups-loading = 正在加载备份…
tui-settings-backups-empty = 还没有备份。按 [c] 创建一个。
tui-settings-backups-creating = 正在创建备份…
tui-settings-backups-delete-confirm =   删除此备份？[y] 是  [任意键] 取消
tui-settings-tab-vault = 7 保险库
tui-settings-hints-vault =   [↑↓] 导航  [e] 设置值  [d] 清除  [r] 刷新
tui-settings-vault-header-key = 键
tui-settings-vault-header-status = 状态
tui-settings-vault-loading = 正在加载保险库键...
tui-settings-vault-empty = 此守护进程未提供任何可写的保险库键
tui-settings-vault-status-set = 已设置
tui-settings-vault-status-notset = 未设置
tui-settings-vault-status-env = 被环境变量覆盖
tui-settings-vault-input-prompt = 输入 { $key } 的新值
tui-settings-vault-note = 已存储的值不会再次显示。[e] 替换，[d] 清除。
tui-settings-vault-note-env-override = { $keys } 来自守护进程自身的环境变量，其优先级高于保险库。[e] 和 [d] 只会更改已存储的副本，不会更改守护进程实际使用的凭据。
tui-settings-vault-delete-confirm =   清除此保险库键？[y] 是  [任意键] 取消
tui-settings-backups-restore-title = 恢复 { $filename }
tui-settings-backups-restore-warning = 恢复会覆盖现有状态。完成后请重启守护进程。
tui-settings-backups-restore-keep-config = 保留本机的 config.toml（克隆模式）
tui-settings-backups-restore-all = 已选中归档中的全部内容。
tui-settings-backups-restore-none = 请至少选择一个组件，或按 [Esc] 取消。
tui-settings-tab-auxiliary = 5 辅助任务
tui-settings-hints-auxiliary =   [↑↓] 导航  [Enter] 编辑链  [r] 刷新  [Esc] 取消
tui-settings-auxiliary-header-task = 任务
tui-settings-auxiliary-header-chain = 回退链
tui-settings-auxiliary-loading = 正在加载辅助任务配置…
tui-settings-auxiliary-empty = 尚未配置辅助任务。
tui-settings-auxiliary-not-configured = (未配置)
tui-settings-auxiliary-editing = 正在编辑 { $task } 的链（以逗号分隔的 provider:model）：
# chat.rs
...
tui-workflows-hints-runs =   [↑↓] 导航  [a] 自动刷新  [r] 刷新  [Esc] 返回
...
tui-workflows-hint-steps = 智能体来源决定值指代什么：运行中智能体的 ID、智能体名称，或智能体类型（模板名称，当该类型没有运行中的智能体时会自动创建）。会话选“每次运行新建”时，该步骤每次运行都使用全新会话。
tui-workflows-label-steps = 步骤:
tui-workflows-steps-empty = 还没有步骤。按 [a] 添加。
tui-workflows-step-field-name = 步骤名称
tui-workflows-step-field-source = 智能体来源
tui-workflows-step-field-prompt = 提示词
tui-workflows-step-field-session = 会话
tui-workflows-source-id = 智能体 ID
tui-workflows-source-name = 智能体名称
tui-workflows-source-type = 智能体类型
tui-workflows-session-persistent = 持久
tui-workflows-session-new = 每次运行新建
tui-workflows-placeholder-agent-id = 运行中智能体的 UUID
tui-workflows-placeholder-agent-name = 已有智能体的名称
tui-workflows-placeholder-agent-type = 模板名称，无运行实例时自动创建
tui-workflows-placeholder-prompt = 此步骤的请求内容；{"{{input}}"} 为工作流输入
tui-workflows-steps-none = 请按 [a] 至少添加一个步骤。
tui-workflows-step-agent-required = 第 { $step } 步：{ $source } 为必填项
tui-workflows-step-prompt-required = 第 { $step } 步：提示词为必填项
tui-workflows-label-review = 确认 — 按 Enter 创建
tui-workflows-review-name =   名称:  
tui-workflows-review-desc =   描述:  
tui-workflows-review-steps =   步骤:  
tui-workflows-hints-create-submit =   [Enter] 创建  [Esc] 返回
tui-workflows-hints-create-next =   [Enter] 下一步  [Esc] 返回
tui-workflows-hints-create-steps-list =   [a] 添加  [d] 删除  [↑↓] 选择步骤  [Tab] 字段  [Enter] 下一步  [Esc] 返回
tui-workflows-hints-create-steps-text =   [Tab/Shift-Tab] 字段  [↑↓] 选择步骤  [Enter] 下一步  [Esc] 返回
tui-workflows-hints-create-steps-select =   [Space/←→] 切换  [Tab/Shift-Tab] 字段  [↑↓] 选择步骤  [Enter] 下一步  [Esc] 返回
...
tui-memory-config-saved = 已保存
tui-memory-config-save-failed = 保存失败：{ $error }
tui-memory-config-unsaved = 有未保存的更改 ── 按 s 保存，Esc 放弃
tui-memory-config-save-partial = 已写入磁盘，但实时重新加载失败 —— 需要重启
tui-memory-config-hint =   j/k 选择 · space/Enter 切换或编辑 · s 保存 · r 刷新 · Esc 返回
...
channel-prompt-secret-retype =   { $label } ({ $key }) [已设置，必填 — 需重新输入才能保存]:
...
agent-set-unknown-field = 未知字段：{ $field }。支持的字段：model, temperature, max_tokens, top_p, frequency_penalty, presence_penalty, top_k, min_p, repeat_penalty, context_window, max_output_tokens
...
message-invalid-session-id = 不是有效的会话 UUID：{ $value }
message-invalid-session-id-fix = 请传入该智能体现有会话的 UUID，或省略 --session-id 以使用其规范会话。可用 `librefang sessions <agent>` 查看列表。
label-source = 来源
label-name = 名称
label-captured = 已捕获
label-module = 模块
...
maintenance-service-install-system-hint = 若需要开机（而非登录后）即启动的服务，请使用：sudo librefang service install --system
maintenance-service-system-needs-root = --system 需要写入 /Library/LaunchDaemons，必须具备 root 权限。
maintenance-service-system-sudo-hint = 请用 sudo 重新执行：sudo librefang service install --system
maintenance-service-system-macos-only = --system 安装的是 macOS LaunchDaemon，当前平台尚未实现该模式。
maintenance-service-system-linux-hint = 在 NixOS 上请使用 services.librefang 模块；其他 Linux 发行版可将系统级 unit 写入 /etc/systemd/system/，或继续使用 `librefang service install` 创建的用户级服务。
maintenance-service-system-no-sudo-user = 无法确定守护进程应以哪个账户运行：SUDO_USER 未设置，说明这是直接以 root 登录执行的，而非通过 sudo。
maintenance-service-system-chown-failed = 未能将 { $path } 的所有权移交给 { $user }：{ $error }
maintenance-service-system-agent-conflict = { $path } 已安装用户级 LaunchAgent。它与开机启动的 LaunchDaemon 使用相同的 ai.librefang.daemon 标签和相同的状态目录，同时安装会导致该 agent 在每次登录后反复启动失败并被拉起。
maintenance-service-system-agent-conflict-fix = 请先以本人账户（而非 sudo）移除：librefang service uninstall
maintenance-service-system-chmod-failed = 设置 { $path } 的权限失败：{ $error }
maintenance-launchdaemon-loaded = LaunchDaemon 已加载 — 开机即启动，并以 { $user } 身份运行
maintenance-launchdaemon-removed = 已移除 LaunchDaemon
maintenance-launchdaemon-remove-failed = 移除 LaunchDaemon plist 失败：{ $error }
maintenance-launchdaemon-not-found = 未找到 LaunchDaemon — 无需移除。
maintenance-launchdaemon-status-registered = 已注册开机启动的 LaunchDaemon
maintenance-launchdaemon-status-not-registered = 未注册开机启动的 LaunchDaemon（守护进程仅在登录后启动）。
maintenance-status-label-daemon-loaded =   LaunchDaemon 已加载
...
everyapi-connect-skip-no-context-window = { $model }：已知该模型的输出上限，但上下文窗口未知，而文本模型缺少其中任一项都会在目录加载时被丢弃。
everyapi-connect-skip-no-output-limit = { $model }：已知该模型的上下文窗口，但输出上限未知，而文本模型缺少其中任一项都会在目录加载时被丢弃。
everyapi-connect-token-limits-borrowed = 网关没有公布以下模型的上下文窗口和/或输出上限，缺失的数字取自内置的 OpenRouter 快照，描述的是该快照中的模型副本，而非本网关的实际部署：{ $models }
everyapi-connect-models-unpriced = 以下模型没有按 token 计费的价格（不在网关计费列表中，或按次计费），其花费会记为未知，而不是免费：{ $models }
...
doctor-audit-desktop-deps-ok = 桌面 GTK/WebKit 组件齐备 — pkg-config 可解析 { $module }。
...
doctor-audit-desktop-deps-hint-apt = 在 { $distro } 上，用 `apt-cache search webkit2gtk` 找出提供 { $modules } 的软件包，然后用 `sudo apt install <package>` 安装。软件包名称在不同版本之间存在差异，请搜索确认而不要凭猜测。
doctor-audit-desktop-deps-hint-pacman = 在 { $distro } 上，用 `pacman -Ss webkit2gtk` 找出提供 { $modules } 的软件包，然后用 `sudo pacman -S <package>` 安装。软件包名称在不同版本之间存在差异，请搜索确认而不要凭猜测。
doctor-audit-desktop-deps-hint-dnf = 在 { $distro } 上，用 `dnf search webkit2gtk` 找出提供 { $modules } 的软件包，然后用 `sudo dnf install <package>` 安装。软件包名称在不同版本之间存在差异，请搜索确认而不要凭猜测。
doctor-audit-desktop-deps-hint-nix = 在 { $distro } 上，以命令式方式安装开发库并不可行 — 请使用 flake 的 devShell 或 librefang-desktop 包，而不要手动安装 { $modules }。
doctor-audit-desktop-deps-hint-generic = 在 { $distro } 上，请安装你的包管理器提供的 GTK 3 与 WebKitGTK 开发包，直到 pkg-config 能够解析 { $modules }。
...
tui-goals-label-status = 状态：
tui-goals-label-description = 描述：
tui-goals-label-agent = 代理：
tui-goals-label-progress = 进度：
tui-goals-label-phase = 运行阶段：
tui-goals-label-iteration = 迭代：
tui-goals-label-title = 标题：
tui-goals-label-loop-engineering = 循环工程：
tui-goals-label-verifier = 验证代理：
tui-goals-label-evaluator = 评估模型：
tui-goals-label-verify-rounds = 验证轮数：
tui-goals-loop-engineering-on = 已启用
tui-goals-loop-engineering-off = 已停用
tui-goals-verifier-none = 无
tui-goals-evaluator-none = 无，由代理自己的标记决定
tui-goals-hint-stop = [s] 停止运行
tui-goals-hint-start = [s] 开始运行
tui-goals-hint-rounds = [+/-] 轮数（verify_max_retries）
tui-goals-detail-hints = [q/Esc] 关闭 { $run_hint } [r] 刷新
tui-goals-verify-rounds-default = 默认值
tui-goals-verify-rounds-next = { $current }（下次：{ $next }）
tui-goals-new-title = 新建目标
tui-goals-step = 第 { $n }/{ $total } 步
tui-goals-title-hint = 目标的简短名称，例如“修复登录超时”。
tui-goals-description-hint = 代理需要完成的内容，将直接作为它的提示词。
tui-goals-agent-hint = 执行该目标的代理名称或 UUID。必填——没有代理的目标无法运行。
tui-goals-loop-engineering-hint = 让第二个代理对每次迭代给出裁决，而不是采信代理自称的完成。关闭时与普通目标循环完全一致。[space] 切换，[y]/[n] 直接设置。字段：loop_engineering。
tui-goals-verifier-hint = 评判工作成果的代理 UUID。留空表示不设验证关卡。代理不能评判自己的工作。字段：verify_agent_id。
tui-goals-evaluator-hint = 独立于代理的 GOAL_DONE 标记来判断目标是否达成的模型。建议使用小而便宜的模型。留空则仅依赖该标记。字段：evaluator_model。
tui-goals-nav-next = [Enter] 下一步  [Esc] 返回
tui-goals-nav-submit = [Enter] 创建  [Esc] 返回
tui-goals-create-incomplete = 标题和代理都是必填项。
tui-goals-label-tick = 循环间隔：
tui-goals-tick-hint = 运行器在两次循环迭代之间等待的秒数（tick_interval_secs）。留空则使用默认值 { $default }。允许范围为 { $min }–{ $max }。
tui-goals-tick-value = { $secs } 秒
tui-goals-tick-default = { $secs } 秒（默认）
tui-goals-tick-out-of-range = 循环间隔必须在 { $min } 到 { $max } 秒之间。
tui-goals-phase-actv = ● 活动
tui-goals-phase-done = ○ 完成
tui-goals-phase-canc = ○ 取消
tui-goals-phase-fail = ● 失败
tui-goals-phase-pend = ○ 待定
tui-goals-phase-paused = ⏸ 暂停
tui-goals-phase-rate-limited = ⚡ 限流
tui-goals-phase-max-iterations = ⏱ 迭代上限
tui-goals-run-running = 运行中
tui-goals-run-paused = 已暂停
tui-goals-run-finished = 已完成
tui-goals-run-max-iterations = 已达迭代上限
tui-goals-run-rate-limited = 因限流而停止
tui-goals-run-stopped = 已被操作员停止
...
# --- goal command (CLI) ---
cmd-goal-agent-required = 目标需要一个代理来执行。
cmd-goal-agent-required-fix = 请传入 --agent <名称或 UUID>。运行 `librefang agents list` 查看可用代理。
cmd-goal-agent-unknown = 没有与 '{ $agent }' 匹配的代理。
cmd-goal-created = 目标已创建: { $id }
cmd-goal-create-error = 创建目标出错：{ $error }
cmd-goal-start-error = 启动运行出错：{ $error }
cmd-goal-watching = 正在观察运行。Ctrl+C 只停止观察，运行本身会继续。
cmd-goal-progress = [{ $current }/{ $total }] { $phase } — 已完成 { $progress }%
...
tui-channels-header-instance = 实例
tui-channels-header-agent = 智能体
tui-channels-header-status = 状态
tui-channels-header-traffic = 收/发
tui-channels-loading = 正在加载通道实例…
tui-channels-empty-state = 尚未配置任何通道实例。按 [a] 添加。
tui-channels-agent-unbound = （未绑定）
tui-channels-group-count = { " " }（{ $count }）
tui-channels-add-new-option =   + 添加通道实例
tui-channels-status-online = ● 在线
tui-channels-status-degraded = ◐ 降级
tui-channels-status-stopped = ○ 已停止
tui-channels-status-unsupervised = ○ 未启动
tui-channels-confirm-delete =   删除通道实例 { $name }？[y/n]
tui-channels-hints-list =   [↑↓] 导航  [Enter/e] 编辑  [a] 添加  [d] 删除  [r] 刷新  [R] 重载适配器
tui-channels-title-picker = 选择新实例运行的适配器
tui-channels-picker-empty = 没有可用的 sidecar 适配器。请安装 Python SDK 后重新加载。
tui-channels-no-schema = 无字段架构 — 需手动配置
tui-channels-hints-picker =   [↑↓] 导航  [Enter] 选择  [Esc] 返回
tui-channels-title-create = 添加通道实例
tui-channels-title-edit = 编辑通道实例
tui-channels-label-instance-name = 实例名称
tui-channels-label-instance-name-locked = 实例名称（不可修改）
tui-channels-label-agent = 默认智能体
tui-channels-placeholder-agent = （未绑定智能体）
tui-channels-secret-stored = （已保存 — 需重新输入才能保存）
tui-channels-hints-form =   [↑↓] 字段  [Space/←→] 切换  [Enter] 保存  [Esc] 取消
tui-channels-error-no-adapter = 保存前请先选择适配器。
tui-channels-error-name-required = 实例名称不能为空。
tui-channels-error-name-invalid = { $name } 不是可用的实例名称 — 只能使用字母、数字以及 `-`、`_`、`.`。
tui-channels-error-name-taken = 名为 { $name } 的实例已存在。
tui-channels-error-name-secret-collision = { $name } 与现有的 { $other } 都会映射到密钥前缀 { $prefix }__，因而会共用彼此的密钥。请选择一个不仅仅是标点符号不同的名称。
tui-channels-error-field-required = { $label } 为必填项。
tui-channels-error-secret-required = { $label } 已保存但无法读回 — 请重新输入后保存。
tui-mod-channel-saved = 通道实例 { $name } 已保存。
tui-mod-channel-saved-restart-required = 通道实例 { $name } 已保存 — 需重启守护进程才能生效。
tui-mod-channel-saved-shadowed = 通道实例 { $name } 已保存，但守护进程环境中已设置 { $keys } 并优先生效 — 请取消该变量后重启。
tui-mod-channel-deleted = 通道实例 { $name } 已删除。
tui-mod-channels-reloaded = 通道已重载（已启动 { $started } 个）。
tui-event-channel-instances-fetch-failed = 加载通道实例失败：{ $error }
tui-event-aux-fetch-failed = 加载辅助 LLM 链失败
tui-event-aux-save-failed = 保存 { $task } 的辅助链失败
tui-event-aux-save-not-available-in-process = 进程内模式下辅助链编辑不可用
tui-event-aux-tasks-fetch-failed = 加载辅助任务列表失败：{ $error }
tui-event-aux-tasks-schema-missing = 守护进程的配置架构未包含辅助任务列表；仅显示已配置的任务。
tui-event-channels-not-available-in-process = 通道管理需要正在运行的守护进程。
...
tui-agents-label-routing-stable-inert = 内核运行在稳定模式：不运行任何路由器，该智能体使用 { $model }。此处保存的设置将在内核退出稳定模式后生效。
tui-agents-label-routing-stable-inert-manifest = 内核运行在稳定模式：不运行任何路由器，该智能体使用其清单中的模型。此处保存的设置将在内核退出稳定模式后生效。
...
agent-routing-label-stable = 稳定模式
agent-routing-stable-inert = 不运行任何路由器；在内核退出稳定模式之前，该智能体使用 { $model }
agent-routing-stable-inert-manifest = 不运行任何路由器；在内核退出稳定模式之前，该智能体使用其清单中的模型
...
tui-agents-param-top-k = Top-K
tui-agents-param-top-k-hint = top_k — 只考虑可能性最高的 K 个词元。仅限 Anthropic、Gemini、Ollama、OpenRouter 和本地服务器。
tui-agents-param-min-p = 最小概率
tui-agents-param-min-p-hint = min_p — 丢弃远低于最高词元概率的词元。仅限 Ollama、vLLM、llama.cpp 和 OpenRouter。
tui-agents-param-repeat-penalty = 重复惩罚
tui-agents-param-repeat-penalty-hint = repeat_penalty — 抑制最近重复的词元；1.0 为关闭。仅限 Ollama、OpenRouter 和本地服务器。
