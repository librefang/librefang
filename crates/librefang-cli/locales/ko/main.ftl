...
tui-agents-workspaces-row-dropped = 이름 또는 경로가 비어 있는 행은 저장되지 않습니다. 나머지를 저장하려면 s를 다시 누르세요.
tui-agents-workspaces-row-invalid = 공유 폴더 "{ $name }"을(를) 저장하지 않았습니다: 이름에는 문자, 숫자, "-", "_"만 쓸 수 있고 경로는 ".." 없는 상대 경로여야 합니다.
tui-agents-workspaces-mode-invalid = "{ $value }"은(는) 폴더 모드가 아닙니다. 이전 값을 유지했습니다. "readonly" 또는 "readwrite"를 사용하세요.
