...
tui-agents-param-top-k-hint = top_k — consider only the K likeliest tokens. Anthropic, Gemini, Ollama and local servers only.
tui-agents-param-min-p = Minimum probability
tui-agents-param-min-p-hint = min_p — drop tokens far less likely than the top one. Ollama, vLLM and llama.cpp only.
tui-agents-param-repeat-penalty = Repetition penalty
tui-agents-param-repeat-penalty-hint = repeat_penalty — discourages recently repeated tokens; 1.0 is off. Ollama and local servers only.
