...
# The hook's secret scan runs `gitleaks protect --config .gitleaks.toml`, whose
# path is relative to the repo it is invoked in. Without this copy the throwaway
# repo has no config, gitleaks exits non-zero on "unable to load gitleaks
# config", and the test fails on any machine where gitleaks is installed — i.e.
# it only passed where the hook was soft-warning past that step entirely.
