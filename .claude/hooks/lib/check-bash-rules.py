...
Available rules:
  ...
  gh-body-attribution       -> banned (gh pr/issue/release --body / --body-file carrying the same)
...
_ATTRIBUTION_RE = re.compile(
    ...
    # "with" and "by" are both in circulation and the difference is not
    # meaningful, so match either rather than picking one. The repo learned
    # this the expensive way: the harness footer says "Generated by", the
    # rule said "Generated with", and 285 PRs / issues carry the watermark.
    r"|Generated (with|by) .{0,40}Claude"
    r"|🤖.*Claude\s*Code"
    # The harness footer, matched by shape rather than by bare domain so a
    # legitimate link to a claude.ai artifact — different link text, and a
    # /code/artifact/… path — is not caught with it.
    r"|\[Claude Code\]\(https?://claude\.ai/code)",
    re.IGNORECASE,
)
        ...
        while i < len(rest):
            ...
            if msg and _ATTRIBUTION_RE.search(msg):
                return (
                    ...
                    "(Co-Authored-By / Generated with-or-by Claude / 🤖 Claude "
                    "Code / the [Claude Code](https://claude.ai/code) footer). "
                    "CLAUDE.md forbids these. Remove the line and retry."
                )
    return None


# `gh` subcommands whose body text becomes public prose on GitHub. CLAUDE.md
# forbids attribution in every one of them, not just in commit messages.
_GH_BODY_VERBS = {
...
# A body-file large enough to be a mistake is not worth reading into the hook.
_GH_BODY_FILE_READ_CAP = 256 * 1024


def _read_body_file(path: str, cwd: str) -> str | None:
    """Read a --body-file argument, or None when it cannot be read.

    Unreadable is not the same as clean, but a hook that raises is a hook that
    blocks every command, so the caller treats None as "nothing to scan".
    """
    if not path or path == "-":  # stdin — not inspectable before the command runs
        return None
    p = path if os.path.isabs(path) else os.path.join(cwd or ".", path)
    try:
        if os.path.getsize(p) > _GH_BODY_FILE_READ_CAP:
            return None
...
def _gh_body_values(rest: list[str], cwd: str):
    """Yield (origin, text) for every body this gh invocation would publish.

    Handles the inline forms (--body/-b, --body=) and the file forms
    (--body-file/-F, --body-file=). The file forms are the load-bearing half:
    CLAUDE.md *mandates* `--body-file` for any body containing markdown, so a
    rule that inspected only inline flags would be bypassed by every
    convention-following command — which is exactly how the watermark reached
    285 PRs and issues unchallenged.
    """
    i = 0
    while i < len(rest):
        x = rest[i]
        ...
        if x.startswith("--body="):
            yield ("--body", x[len("--body="):])
            i += 1
            continue
        if x in ("--body-file", "-F"):
            if i + 1 < len(rest):
                text = _read_body_file(rest[i + 1], cwd)
                if text is not None:
                    yield (f"--body-file {rest[i + 1]}", text)
            i += 2
            continue
        if x.startswith("--body-file="):
            path = x[len("--body-file="):]
            text = _read_body_file(path, cwd)
            if text is not None:
                yield (f"--body-file {path}", text)
            i += 1
            continue
        i += 1


def rule_gh_body_attribution(toks, ctx):
    """Inspect the body a `gh pr` / `gh issue` / `gh release` command publishes.

    rule_claude_attribution covers `git commit -m`; the git-side commit-msg
    hook covers editor and heredoc messages. Neither can see a PR body, so
    before this rule the "no attribution in PR bodies or comments" half of the
    CLAUDE.md rule had no mechanical enforcement at all.
    """
    cwd = ctx.get("cwd") or ""
    for i, t in enumerate(toks):
        if is_quoted_content(t):
        ...
        if i + 2 >= len(toks):
            continue
        noun = strip_parens(toks[i + 1])
        verb = strip_parens(toks[i + 2])
        if verb not in _GH_BODY_VERBS.get(noun, ()):
            continue
        for origin, text in _gh_body_values(toks[i + 3:], cwd):
            if text and _ATTRIBUTION_RE.search(text):
                return (
                    f"`gh {noun} {verb}` would publish Claude / Anthropic "
                    f"attribution (found in {origin}). CLAUDE.md forbids AI "
                    f"attribution in PR bodies, issues and review comments, "
                    f"not just commit messages. Remove the line and retry."
                )
    return None
...
RULES = {
    ...
    "gh-body-attribution":       rule_gh_body_attribution,
