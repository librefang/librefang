Groups are now a first-class entity, so a permission or an ownership decision can name a team rather than an individual.
  ...
  Deleting or renaming a user now updates every group that named them, in the same config write, so a removed person cannot keep the roles their membership granted. (#PR) (@houko)
