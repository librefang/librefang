...
`librefang-skills` now detects a markup response before handing it to `serde_json` and returns a distinct `SkillError::MarketplaceUnavailable` with an actionable message; the API layer maps it to `503 Service Unavailable` instead of `502`/`404`.
The fix lives at the shared `ClawHubClient` parsing boundary too, so `get_skill` (used by both ClawHub and Skillhub) benefits without duplicating the check.
...
Local skill install/search on FangHub and ClawHub are unaffected (#7387) (@DaBlitzStein)
