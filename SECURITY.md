...
- **Threat model — what the anchor does and does not buy**: the anchor file lives next to the database by default. An attacker with full write access to `~/.librefang/data/` can still corrupt both files in lockstep — the anchor is meaningfully stronger only when operators sync `audit.anchor` to an append-only store they control (offsite cron rsync, signed systemd-journald mirror, transparency log). Tier-2 (journald mirror) and Tier-3 (Ed25519-signed offsite mirror, transparency log) are tracked as follow-up work in #3339. Until then, treat the audit log as tamper-evident against single-file tampering and cooperative against multi-file tampering by an attacker with full filesystem write.
  Independently of operator filesystem permissions, the WASM skill sandbox now refuses any `fs_write` whose canonical target is the resolved audit anchor, regardless of the `FileWrite` capability a skill manifest declares (including `FileWrite("*")`) — a deny-list enforced above the capability check in `host_fs_write` so a sandboxed skill cannot truncate the anchor and silently break the Merkle chain (#6182).
  This protects the in-process skill threat surface; it does not change the OS-level multi-file-tamper analysis above.
- **Recovery without evidence loss**: when a chain has already broken, `librefang security audit-reanchor` repairs it by archiving every row at or after the break to `<data_dir>/audit-archive/` as JSON Lines, deleting only those rows, and appending a `ChainReanchored` marker linked to the last row that still verified. The archive's SHA-256 is committed into that marker, so the preserved rows stay tamper-evident after the repair. `librefang security audit-reset` — which empties the table — remains available for development machines whose audit history has no value; it is not an appropriate recovery in a compliance environment. See `docs/operations/audit-chain-recovery.md`.
