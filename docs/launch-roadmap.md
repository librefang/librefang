...
**Status: COMPLETE** — Created `sdk/javascript/` (@librefang/sdk) with full REST client: agent CRUD, streaming via SSE, sessions, workflows, skills, channels, memory KV, triggers, schedules + TypeScript declarations. Created `sdk/python/librefang_client.py` (zero-dependency stdlib urllib) with same coverage. Both include basic + streaming examples. Python `setup.py` for pip install.
...
1. Create `sdks/javascript/` — thin wrapper around REST API
   ...
   - Publish to npm as `@librefang/sdk`
2. Create `sdks/python/` — thin wrapper with httpx
   - Same operations
   - Publish to PyPI as `librefang`
3. Include usage examples in README

**Done when:**
- `npm install @librefang/sdk` works
- `pip install librefang` works
...
**Status: COMPLETE** — Added `GET /api/metrics` endpoint returning Prometheus text format. Metrics: `librefang_uptime_seconds`, `librefang_agents_active`, `librefang_agents_total`, `librefang_tokens_total{agent,provider,model}`, `librefang_tool_calls_total{agent}`, `librefang_panics_total`, `librefang_restarts_total`, `librefang_info{version}`.

**Problem (was):** No way to monitor LibreFang in production (no Prometheus, no OpenTelemetry).

**What to do:**
1. Add `/api/metrics` endpoint with Prometheus format
   - `librefang_agents_active` gauge
   - `librefang_messages_total` counter (by agent, by channel)
   - `librefang_tokens_total` counter (by provider, by model)
   - `librefang_request_duration_seconds` histogram
   - `librefang_tool_calls_total` counter (by tool name)
   - `librefang_errors_total` counter (by type)
2. Optional: OTLP export for tracing spans

**Files:** `crates/librefang-api/src/routes.rs`, new `metrics.rs` module
...
**Problem (was):** Both LibreFang and OpenClaw define workflows in TOML/config only. No visual builder exists in either. First to ship this wins.
...
**Files:** `crates/librefang-kernel/src/kernel.rs`, `routes.rs`, `ws.rs`, `index_body.html`
...
**What to do:**
1. Watch `~/.librefang/config.toml` for changes (notify crate)
2. On change: re-parse, diff, apply only changed sections
3. Log what was reloaded
4. UI notification: "Config reloaded"

**Files:** `crates/librefang-api/src/server.rs`, `crates/librefang-types/src/config.rs`
...
| Feature | OpenClaw | LibreFang | Winner |
|---------|----------|----------|--------|
| Language/Performance | Node.js (~200MB) | Rust (~30MB single binary) | **LibreFang** |
| Channels | ~15 | **40** | **LibreFang** |
| Built-in Tools | ~19 | **41** | **LibreFang** |
| Security Systems | Token + sandbox | **16 defense systems** | **LibreFang** |
| Agent Templates | Manual config | **30 pre-configured** | **LibreFang** |
| Hands (autonomous) | None | **7 packages** | **LibreFang** |
| Workflow Engine | Cron + webhooks | **Full DAG with parallel/loops** | **LibreFang** |
| Knowledge Graph | Flat vector store | **Entity-relation graph** | **LibreFang** |
| P2P Networking | None | **OFP wire protocol** | **LibreFang** |
| WASM Sandbox | Docker only | **Dual-metered WASM** | **LibreFang** |
| Desktop App | Electron (~200MB) | **Tauri (~30MB)** | **LibreFang** |
| Migration | N/A | **`migrate --from openclaw`** | **LibreFang** |
| Skills | 54 bundled | **60 bundled** | **LibreFang** |
...
| Visual Workflow Builder | None | **Drag-and-drop builder** | **LibreFang** |

**LibreFang wins 15/18 categories.** The remaining gaps are: mobile apps (OpenClaw), voice wake word (OpenClaw slight edge).
