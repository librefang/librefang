...
<Note>
All channel credentials can be stored using any of the three methods:
- **Environment variable**: `export TELEGRAM_BOT_TOKEN=...`
- **Encrypted vault** (recommended): `librefang vault set TELEGRAM_BOT_TOKEN`
- **.env file**: `librefang config set-key` or edit `~/.librefang/.env` directly

The examples below use `export` for brevity. In production, use the vault.
</Note>
