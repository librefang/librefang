...
```bash
export GOOGLE_CHAT_SA_KEY=/path/to/service-account-key.json
...
```bash
export FEISHU_APP_ID=cli_abcdef123456
...
```bash
export ROCKETCHAT_TOKEN=your-personal-access-token
...
```bash
export ZULIP_EMAIL=my-bot@example.zulipchat.com
...
```bash
export FLOCK_BOT_TOKEN=your-flock-bot-token
```
...
```bash
export TWIST_TOKEN=your-twist-token
```
...
```bash
export PUMBLE_BOT_TOKEN=your-pumble-bot-token
```
...
```bash
export GUILDED_BOT_TOKEN=your-guilded-bot-token
```
...
export LINE_ACCESS_TOKEN=your-channel-access-token
```
...
```bash
export VIBER_AUTH_TOKEN=your-viber-auth-token
```
...
export MESSENGER_VERIFY_TOKEN=my-verify-token
```
...
```bash
export TWITCH_OAUTH_TOKEN=oauth:your-token-here
...
```bash
export XMPP_JID=bot@example.com
...
```bash
export REVOLT_TOKEN=your-revolt-bot-token
```
...
```bash
export NOSTR_PRIVATE_KEY=nsec1your-private-key-here
...
```bash
export DINGTALK_ACCESS_TOKEN=your-access-token
...
- A WeCom (WeChat Work) enterprise account with a self-built application (from the [WeCom Developer Portal](https://developer.work.weixin.qq.com/document/))

### Setup

1. In the WeCom admin console, go to **App Management > Self-Built Apps** and create a new application.
2. Note the Corp ID, Agent ID, and Secret.
3. Configure the receiving message API callback URL in the app settings.
4. Set environment variables:
...
The WeCom adapter uses the WeCom Enterprise API webhook protocol to receive message callbacks. It obtains an access token using the corp credentials and sends responses via the WeCom message sending API. Message payloads are encrypted and decrypted using the configured encoding AES key.
...
```bash
export MQTT_BROKER_URL=mqtt://broker.example.com:1883
...
# Optional: if the topic requires authentication
# export NTFY_TOKEN=your-access-token
```
...
export NEXTCLOUD_TOKEN=your-app-password-or-token
```
...
```bash
export MUMBLE_SERVER=mumble.example.com:64738
export MUMBLE_USERNAME=librefang-bot
# Optional: server password
# export MUMBLE_PASSWORD=your-password
```
...
```bash
export WEBHOOK_URL=https://your-service.example.com/hook
