...
The install script auto-detects platform and architecture, downloads the correct binary, verifies SHA256 checksums, configures PATH, and starts the daemon.
...
Or via Homebrew Cask: `brew install --cask librefang`
...
Deploy on GCP always-free tier (e2-micro):
...
LibreFang provides 10 agent templates (assistant pre-installed, others available from dashboard). Spawn the assistant agent:
