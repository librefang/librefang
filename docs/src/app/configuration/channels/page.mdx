...
An unset (`null`) field is projected onto no override at all, so setting one knob here never materializes a policy you did not write — the footgun fixed in #6445. Agent-level `[channel_overrides]` in `agent.toml` still takes precedence over these per-channel values.
