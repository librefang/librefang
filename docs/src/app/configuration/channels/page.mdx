...
Every channel ships with a set of built-in slash commands (`/agent`, `/new`, `/reboot`, `/model`, `/usage`, etc.) that let users switch agents, reset sessions, inspect usage, and trigger workflows. On **public-facing bots** (customer support, community assistants), exposing these to arbitrary users is a security hole — anyone can type `/agent admin` to switch to an internal agent, `/new` to wipe the session, or `/model` to change the model.
...
Command names are the bare tokens used internally (no leading `/`). The full list of built-in commands is in `/help`.
