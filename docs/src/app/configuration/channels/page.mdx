...
| `dm_policy`, `group_policy` | Moved to the agent: set them under `agent.toml [channel_overrides]` — the channel bridge enforces them on every dispatch, and `allowed_only` user gating is backed by `[[users]]` channel bindings (RBAC). They are not sidecar env knobs: only the WhatsApp sidecar reads `WHATSAPP_DM_POLICY` (Cloud API webhook mode only), `WHATSAPP_GROUP_POLICY` is read but currently has no effect, and the Telegram / Discord / Slack / Teams sidecars read no `*_DM_POLICY` / `*_GROUP_POLICY` at all. |
