...
| `dm_policy`, `group_policy` | Not a sidecar env knob — DM / group authorization is enforced kernel-side by `[[users]]` channel bindings (`authorize_channel_user`). Only the WhatsApp sidecar reads `WHATSAPP_DM_POLICY`; `WHATSAPP_GROUP_POLICY` is read but currently has no effect, and the Telegram / Discord / Slack / Teams sidecars read no `*_DM_POLICY` / `*_GROUP_POLICY` at all. |
