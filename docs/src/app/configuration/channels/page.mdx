...
# WHATSAPP_DM_POLICY = "respond"          # respond / allowed_only / ignore (Cloud API webhook mode only)
# WHATSAPP_GROUP_POLICY = "all"           # read but currently has no effect in either mode
```
...
| `dm_policy`, `group_policy` | Settable per-channel on `[[sidecar_channels]]` directly (#6445), or on the agent under `agent.toml [channel_overrides]` (which takes precedence). The channel bridge enforces them on every dispatch, and `allowed_only` user gating is backed by `[[users]]` channel bindings (RBAC). Distinct from the sidecar env knobs: only the WhatsApp sidecar reads `WHATSAPP_DM_POLICY` (Cloud API webhook mode only), `WHATSAPP_GROUP_POLICY` is read but currently has no effect, and the Telegram / Discord / Slack / Teams sidecars read no `*_DM_POLICY` / `*_GROUP_POLICY` at all. |
...
| `output_format` | Settable per-channel on `[[sidecar_channels]]` (#6445), or on the agent side; the agent value takes precedence. |
...
| `threading` | Settable per-channel on `[[sidecar_channels]]` (#6445), or on the agent side; the agent value takes precedence. |
| `usage_footer`, `typing_mode` | Sidecar defaults (no usage footer; instant typing). Not configurable per-channel today. |
...
| `dm_policy` | string or null | `null` | How the bot handles direct messages. See below. `null` (unset) applies no DM gating — DMs flow — and, unlike a written value, does not affect any other override (#6445). |
| `group_policy` | string or null | `null` | How the bot handles group messages. See below. `null` (unset) applies no group gating — every group message is processed. Before #6445, an unset value silently behaved as `mention_only` whenever any other override field was written. |
...
| `dm_policy` | string or null | `null` | Per-channel DM policy (`respond` / `allowed_only` / `ignore`), scoped to this sidecar instance (#6445). `null` inherits the bridge default (DMs flow) and, unlike a written value, leaves every other override untouched. |
| `group_policy` | string or null | `null` | Per-channel group policy (`all` / `mention_only` / `commands_only` / `ignore`), scoped to this sidecar instance (#6445). `null` inherits the bridge default (process all group messages). |
| `threading` | bool or null | `null` | Enable thread replies for this sidecar instance (#6445). `null` inherits the bridge default (off). |
| `output_format` | string or null | `null` | Output-format override (`markdown` / `telegram_html` / `slack_mrkdwn` / `plain_text`) for this sidecar instance (#6445). `null` inherits the channel default. |

An unset (`null`) field is projected onto no override at all, so setting one knob here never materializes a policy you did not write — the footgun fixed in #6445.
Agent-level `[channel_overrides]` in `agent.toml` still takes precedence over these per-channel values, and that precedence is whole-struct, not per-field: if the agent manifest declares a `[channel_overrides]` table at all, it replaces this sidecar's `dm_policy` / `group_policy` / `threading` / `output_format` entirely, even for the fields the agent table itself leaves unset.
An agent whose `[channel_overrides]` table sets only `model` (nothing else) will silently drop a sidecar-level `group_policy = "ignore"` back to unset (process all group messages) — set the same policy under the agent's `[channel_overrides]` too if it must hold regardless of what else the agent overrides.
