...
| `reasoning_mode` | string | unset | Explicit reasoning effort: `"none"`, `"low"`, `"high"`, or `"max"`. When unset, the effort is derived from `budget_tokens`. |
...
The Anthropic and Ollama drivers keep reading `budget_tokens` for the graded rungs — their wire control is a token budget, not an effort enum — but they do honour `none`, because the mode has to travel on a materialised thinking config and reading that config as a bare budget would enable reasoning on a turn that asked for the opposite.
