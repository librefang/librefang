...
A dream fires for an agent when all gates hold: `enabled = true`, the agent's manifest has `auto_dream_enabled = true`, `min_hours` have elapsed since its last dream, `min_sessions` have been touched since then, and the per-agent lock can be acquired. Manual triggers via `POST /api/auto-dream/agents/{id}/trigger` bypass the time and session gates but still respect the lock.
