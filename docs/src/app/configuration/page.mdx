# BossFang Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the BossFang Agent OS.
...
BossFang reads its configuration from a single TOML file:
...
BossFang supports splitting configuration across multiple files using the `include` field. This enables modular configs (e.g., separate files for channels, MCP servers, or environment-specific overrides).
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, BossFang boots with Anthropic as the default provider:
...
```toml
# ============================================================
# BossFang Agent OS -- Complete Configuration Reference
# ============================================================
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.librefang` | BossFang home directory. Stores config, agents, skills. |
...
Configures the OFP (BossFang Protocol) peer-to-peer networking layer. Authentication has two layers: a `shared_secret` HMAC admission gate plus per-node Ed25519 identity with TOFU pinning (#3873). The Ed25519 keypair and trust pins live in `<data_dir>/peer_keypair.json` and `<data_dir>/trusted_peers.json`.
...
LINE is provided by the Python sidecar adapter (`librefang.sidecar.adapters.line`). The in-process `[channels.line]` config block was removed in the sidecar migration. Declare LINE as a `[[sidecar_channels]]` entry instead — the sidecar runs its **own** HTTP webhook server (no longer mounted on the BossFang API port), so the URL you register at the LINE Developers Console is now `https://<your-sidecar-host>:<LINE_WEBHOOK_PORT><LINE_WEBHOOK_PATH>`:
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across BossFang instances.
...
| `name` | string | `"BossFang Agent OS"` | Service-level display name shown in the well-known agent card. |
...
Configures device pairing for the BossFang mobile companion app and push notifications.
...
Configures prompt versioning and A/B experiment support. When enabled, BossFang automatically tracks prompt version history and supports running A/B experiments to compare prompt variants. See the [Prompt Intelligence guide](/agent/prompt-intelligence) for full documentation.
