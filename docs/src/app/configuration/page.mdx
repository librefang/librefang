...
| `transcription_timeout_secs` | u64 | `60` | Timeout for one audio transcription provider request in seconds. Requires a daemon restart. |
| `ffmpeg_timeout_secs` | u64 | `30` | Timeout for one ffmpeg subprocess in seconds. Requires a daemon restart. |
...
| `audio_model` | string or null | `null` | Preferred model for audio transcription. Provider default if `null`. |
| `audio_language` | string or null | `null` | Fallback ISO-639-1 language hint for `media_transcribe` / `speech_to_text` calls that omit their own `language`. The per-call value always wins. |
| `audio_prompt` | string or null | `null` | Fallback transcription prompt (domain vocabulary, proper nouns) for calls that omit their own `prompt`. The per-call value always wins. |
...
| `max_content_chars` | usize | `50000` | Maximum characters of extracted page content returned by `browser_navigate` / `browser_read_page`. A real ceiling: the truncation marker is counted against it, so content plus marker never exceeds this value. Roughly 4 characters per token, so the default is about 12–15k tokens — comfortable against a 200k-context model, hostile against a 32k local one. |
...
| `safe_bins_skip_approval` | bool | `false` | Opt-in: in `allowlist` mode, a `shell_exec` whose every base command is a declared `safe_bin` also skips the human-approval prompt. A chained command with any non-safe base still routes through approval. |
| `full_mode_skips_approval` | bool | `true` | Whether `mode = "full"` also waives the global `approval.require_approval` list for `shell_exec`. Set `false` for unrestricted commands that still prompt. |
...
`mode` and approval are separate decisions.
Historically `full` implied both "skip allowlist validation" and "skip the approval prompt"; `full_mode_skips_approval = false` decouples them, so `require_approval` is honoured under `full` exactly as it is under `allowlist`.
The default stays `true` because `approval.require_approval` ships with `shell_exec` in it and a standalone agent that declares `shell_exec` is promoted to `full` at spawn, so flipping the default would make a stock install prompt on every command.

Like every other `exec_policy` field, this one is copied into each agent's manifest when the agent is spawned or restored, so `POST /api/config/reload` does not retrofit it onto agents that are already running -- kill the agent and let it respawn, or restart the daemon.
Without that step, setting it to `false` looks like a silent no-op: no new prompts appear, and the natural reading is that nothing needed approval.

The dangerous-command denylist is independent of `mode` and screens every command under `deny`, `allowlist`, and `full` alike -- `full` disables allowlist *validation*, not the denylist.
It covers, among others, LibreFang daemon lifecycle commands (`librefang stop` / `start` / `restart`, including the `gateway` alias and path-qualified invocations), which bounce every agent and channel adapter sharing the daemon, and mutating SQL or output redirection against the daemon's own `librefang.db`.
Read-only inspection of that database (`select`, `.schema`, `.dump`) and `librefang status` are deliberately not matched.
...
budget_tokens = 10000
stream_thinking = false
reasoning_mode = "high"        # optional: none | low | high | max
```
...
| `reasoning_mode` | string | unset | Explicit reasoning effort: `"none"`, `"low"`, `"high"`, or `"max"`. When unset, the effort is derived from `budget_tokens` on the drivers whose wire still takes a budget — see the per-provider mapping below. |

#### `reasoning_mode` (#7946)

`budget_tokens` cannot say *do not reason at all* — a sub-1024 budget merely omits the reasoning opt-in, and a model that reasons by default (every DeepSeek V4 id) keeps reasoning.
It also cannot reach the top rung of providers that have one, because the derived bucket stops at `high`.
`reasoning_mode` is the direct control, and each driver translates it into whatever its wire actually accepts.

Set it in three places, resolved **per-call > per-agent > global > compiled default**:

- per call — `reasoning_mode` on the body of `POST /api/agents/{id}/message` (and `/message/stream`). Wins over the older boolean `thinking` field when both are sent.
- per agent — `[thinking] reasoning_mode` in that agent's `agent.toml`. Per-agent settings do not work from `config.toml`.
- globally — `[thinking] reasoning_mode` here.

Per-provider wire mapping:

| Mode | DeepSeek V4 (direct) | OpenRouter-routed | Other OpenAI-compatible |
|------|----------------------|-------------------|-------------------------|
| `none` | `thinking: {"type":"disabled"}` | `reasoning: {"effort":"none"}` | field omitted |
| `low` | `thinking: enabled` + `reasoning_effort: "low"` | `reasoning: {"effort":"low"}` | `reasoning_effort: "low"` |
| `high` | `thinking: enabled` + `reasoning_effort: "high"` | `reasoning: {"effort":"high"}` | `reasoning_effort: "high"` |
| `max` | `thinking: enabled` + `reasoning_effort: "max"` | `reasoning: {"effort":"xhigh"}` | `reasoning_effort: "high"` |

A payload never carries both `reasoning_effort` and `reasoning.effort` — OpenRouter rejects that combination with HTTP 400.
Kimi/Moonshot models always send `thinking: disabled` (a multi-turn tool-call requirement, not a preference), and DeepSeek R1 has no wire toggle.
The Ollama driver keeps reading `budget_tokens` for the graded rungs — its wire control is a token budget, not an effort enum — but it does honour `none`, because the mode has to travel on a materialised thinking config and reading that config as a bare budget would enable reasoning on a turn that asked for the opposite.
The Anthropic driver splits on the model: Haiku 4.5 and older Claude models behave the same way, while Opus 4.6 and newer have no token budget left to send and take the mode as their only rung — `low` / `high` / `max` map straight onto `output_config.effort`, and with no mode set the field is omitted so the API's own default applies rather than a rung derived from a budget the operator may never have chosen.
Gemini does not read the thinking config at all.
Full detail, including why `medium` is not a settable mode: `docs/architecture/reasoning-mode-resolution.md`.
...
| `output_format` | string or null | `null` (→ `"mp3"`) | Default for the `text_to_speech` tool's `output_format` argument, applied to every provider: `"mp3"` or `"ogg_opus"`. `"mp3"` performs no conversion — the provider's own format is kept. Set `"ogg_opus"` when the audio is sent to a messaging channel as a voice note. A value passed in the tool call wins. |
...
| `output_format` | string | `"opus_48000_32"` | The `?output_format=` query parameter sent to ElevenLabs — a provider-side setting, distinct from the `[tts] output_format` above. Others: `mp3_44100_128`, `mp3_22050_32`, `opus_24000_32`, `pcm_16000`, `pcm_44100`, `ulaw_8000`. |
...
| `container_prefix` | string | `"librefang-sandbox"` | Prefix for container names. The full name is `{prefix}-{8 hex agent digest}-{8 hex instance digest}`, and Docker caps a container name at 63 characters, so a prefix longer than **45 characters** makes every sandbox creation fail with "Container name too long". |
...
| `reuse_cool_secs` | u64 | `300` | Settling time in seconds before a container released by one agent may be handed to a *different* agent. Applies to `scope = "shared"` only — `session` / `agent` scope pin the container to the one workload that released it, and even under `shared` an agent re-acquiring the container it just released does not wait. |
| `idle_timeout_secs` | u64 | `86400` | Destroy pooled containers after this many seconds of inactivity (24 hours default). `0` disables idle reaping. Together with `max_age_secs` this is the only thing that shrinks the pool — see the note on accumulation below. |
| `max_age_secs` | u64 | `604800` | Maximum pooled container age before forced destruction (7 days default), measured from `docker run` rather than from the last time the container was used. `0` disables age reaping. |
...
| Value | Description |
|-------|-------------|
| `session` | One container per (agent, session), reused by every tool call in that session (default). |
| `agent` | One container per agent, reused across that agent's sessions. |
| `shared` | One container per (config, workspace), reusable by any agent that mounts the same workspace. |

A container is reused only when the whole key matches: the scope owner above, the container-shaping `[docker]` fields (`image`, `network`, `memory_limit`, `cpu_limit`, `workdir`, `read_only_root`, `cap_add`, `tmpfs`, `pids_limit`, `blocked_mounts`), and the host workspace path bind-mounted into the container. The workspace is part of the key for every scope, including `shared`: a container carries the workspace it was created with, so sharing it with a caller that mounts a different path would expose one agent's workspace to another.

Containers are destroyed by the reaper, which runs every 60 seconds while `enabled = true` and applies `idle_timeout_secs` and `max_age_secs`, and by daemon shutdown, which destroys every pooled container. There is no separate "session ended" signal in LibreFang, so a `session`-scoped container outlives its last tool call until one of those two reaps it — set `idle_timeout_secs` to the window you want that to be.

A call site with no LibreFang session — the `POST /api/tools/{name}` bridge, and other out-of-band invocations — cannot honour `scope = "session"`, so it falls back to creating a container for the call and destroying it afterwards rather than widening the container to the whole agent.

`shared` is a trust boundary, not only a pooling strategy. A container is handed over whole: everything a previous call wrote to a writable path or to the `/tmp` tmpfs is still there, as is any process that call left running. `reuse_cool_secs` delays a handover between two agents; it does not sanitize anything, and nothing else does either. Use `shared` only where every agent mounting that workspace already trusts the others, and reach for `agent` or `session` otherwise.

`reuse_cool_secs` delays a handover rather than queueing one. A call that finds only a container still cooling down creates an additional container instead of waiting, and that container joins the pool when it is released. So a workspace several agents hit concurrently under `shared`, or any key hit by concurrent calls under any scope, holds more than one container.

Nothing returns a container to the host except the reaper, so the idle containers a key holds settle at that key's peak concurrency and stay there for `idle_timeout_secs` — 24 hours by default. That is the number to size for when the sandbox is enabled: lower `idle_timeout_secs` rather than assuming load will drop off and take the containers with it.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_depth_per_agent` | u32 | `0` | Maximum *pending* tasks assigned to one agent. A post that would exceed it is refused with `429 Too Many Requests`. Counted per assignee, so one agent filling its share does not block another; tasks with no assignee are in the shared pool and are not counted here. `0` = unlimited. |
| `max_depth_global` | u32 | `0` | Maximum pending tasks across every agent and the unassigned pool. A post that would exceed it is refused with `429 Too Many Requests`. `0` = unlimited. |
| `task_ttl_secs` | u64 | `3600` | A task still `pending` this many seconds after it was posted is moved to `cancelled`, with a `result` naming this setting. It is not deleted: the row stays visible until `task_queue_retention_days` reclaims it, so an expiry can be seen rather than inferred from a task that vanished. A task that has been claimed is in flight and is never expired here — `task_reset_stuck` is the separate mechanism for a worker that stalled after claiming. `0` = unlimited. |
...
| `max_memories_per_agent` | usize | `1000` | Maximum memories per agent. When exceeded, raw dialogue rows are evicted before extracted facts, lowest-confidence first within each class. `0` = no cap. |
| `session_scoped_recall` | bool | `true` | (#7605) Stamp each auto-memorized memory with the session that produced it, and refuse to auto-retrieve a memory stamped for a different session. Keeps one visitor's content out of another visitor's turn on an agent that serves several people. Memories written before this shipped carry no stamp and stay recallable everywhere. Set `false` for a single agent-wide pool — the pre-#7605 behaviour, and what an agent with `session_mode = "new"` wants if its runs are meant to build on each other. |

#### Per-agent overrides

Each agent's `agent.toml` may carry a `[proactive_memory]` block that overrides selected fields of the kernel-global config above for that agent only. Unset fields inherit the global. All five override fields are optional:
...
# session_scoped_recall = false         # let this agent recall its memories across all of its sessions
```
...
| `session_scoped_recall` | (#7605) `Some(false)` gives this agent one memory pool across all of its sessions; `Some(true)` isolates it when the deployment default is off. Unrelated to `enabled` — a disabled agent does no automatic recall at all, so there is nothing left to scope. |
...
| `heartbeat_channel` | `null` | Where this agent's unresponsive alert goes. `"telegram"` uses the owner's `channel_bindings` recipient for that channel; `"telegram:123456"` addresses a recipient directly. Consulted after a matching `[[notification.agent_rules]]` entry and before `[notification] alert_channels`. |
