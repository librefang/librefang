...
This page covers internet-hosted model providers that authenticate with an API key and connect through BossFang's native Anthropic/Gemini drivers or the shared OpenAI-compatible driver.
...
- Vision is **not** wired today: image content blocks are dropped before the request because Bedrock Converse rejects the BossFang `Image` / `ImageFile` shapes.
