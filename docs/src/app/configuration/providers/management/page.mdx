...
The store is what decides discovery for any provider it has an entry for: once you have used the switch for a provider, editing `discover_models` in that provider's own file no longer changes anything.
Change it through the Dashboard or the API instead, or edit the store file directly; deleting that file drops every recorded preference at once.
