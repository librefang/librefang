...
EveryAPI is a managed provider with no fixed model list. The available models are a property of your account rather than of LibreFang. LibreFang asks the EveryAPI CLI credential process for the current gateway and relay credential, then loads the account's live catalog.
...
1. Install the public [EveryAPI CLI](https://github.com/everyapi-ai/everyapi-ai) and sign in:
   ...
   everyapi auth login
   ```
2. Start LibreFang. It detects the authenticated EveryAPI CLI and registers the managed provider automatically:
   ```bash
   librefang start
   ```
...
For managed discovery, EveryAPI remains the credential authority. LibreFang invokes `everyapi auth credential --format=json` when it needs a credential and never persists the returned secret. Older compatible CLI credentials can still be read as a fallback. Explicit endpoint or environment-variable configuration remains available and takes precedence over managed discovery.
