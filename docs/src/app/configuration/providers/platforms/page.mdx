...
1. Install the [EveryAPI CLI](https://github.com/everyapi-ai/everyapi) and sign in, which writes `~/.config/everyapi/credentials.json`:
   ...
   everyapi login
   ```
2. Register the gateway with LibreFang:
   ```bash
...
Model metadata is resolved from LibreFang's builtin catalog, because the gateway's `/v1/models` response carries ids but no pricing and mostly no context window. A text model whose context window and output limit cannot be resolved is skipped and named in the command output rather than registered — a text entry missing either field is discarded when the catalog loads, so registering it would silently lose the model with nothing but a log line. Where pricing cannot be resolved the entry is marked as having unknown pricing, so budget figures do not silently treat it as free.
