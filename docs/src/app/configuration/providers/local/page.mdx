...
When the embeddings driver is set to `provider = "auto"`, LibreFang walks a priority list to pick the first reachable embeddings backend. Cloud API keys win first; local servers come after. The current order:
