...
This page covers providers that run on your machine, inside your network, or behind a self-hosted inference endpoint that BossFang can reach directly.
...
**Notes:** BossFang auto-discovers models from a running Ollama instance and merges them into the catalog with `Local` tier and zero cost. Any model you pull becomes usable immediately.
...
When the embeddings driver is set to `provider = "auto"`, BossFang walks a priority list to pick the first reachable embeddings backend. Cloud API keys win first; local servers come after. The current order (verified against `librefang-runtime::embedding::detect_embedding_provider`):
...
For Ollama, BossFang probes `/api/tags` at startup and on the periodic provider refresh, then derives each model's capabilities from the metadata it returns:
