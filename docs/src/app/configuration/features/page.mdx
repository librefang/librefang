...
| `output_format` | string or null | `null` (→ `"mp3"`) | Default for the `text_to_speech` tool's `output_format` argument, applied to every provider: `"mp3"` or `"ogg_opus"`. A value passed in the tool call wins. |
