...
| `allow_remote` | bool | `false` | Allows unauthenticated access from remote or proxied connections when no auth is configured. Default behavior is local-only access without auth. |
...
| `trust_proxy_headers` | bool | `false` | When `true`, loopback connections without proxy headers (X-Forwarded-For, X-Real-IP) are rejected. Enable only when running behind a reverse proxy that injects these headers. |
