...
The keys are values of the token's `roles` claim, and the values are LibreFang roles in the same `viewer` < `user` < `admin` < `owner` ladder that `[[users]] role` and `[channel_role_mapping]` use.
