...
Core configuration sections for BossFang: top-level fields, default model, memory, networking, web search, media processing, and link understanding.
...
Self-hosted SearXNG provider. Disabled by default — set `url` to opt in. No API key is required; BossFang talks to the instance over the public `/search?format=json` endpoint and discovers categories via `/config`.
...
Browser tool configuration. By default BossFang launches a local Chromium instance. Set `cdp_endpoint` to attach to an already-running browser (Browserless, headful Chrome with `--remote-debugging-port`, etc.) instead.
...
Reach the dashboard from a non-loopback address? Set a username/password and BossFang will hash the password to Argon2id automatically on first boot.
...
| `[a2a]` | A2A protocol listen path + accepted-issuer allowlist for cross-instance auth. | Set when federating multiple BossFang instances. |
