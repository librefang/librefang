...
| `max_agent_call_depth` | u32 | `5` | Inter-agent call depth (prevents recursion bombs across `agent_send`); nested `workflow_run` calls are charged to the same budget. |
...
# The same form routes the night-* agents' unresponsive alerts to an on-call thread
[[notification.agent_rules]]
agent_pattern = "night-*"
...
| Field | Type | Default | Description |
|---|---|---|---|
| `approval_channels` | `Vec<NotificationTarget>` | `[]` | Where pending approvals fan out. Each target is `{ channel_type, recipient }` — `channel_type` is `telegram`, `slack`, `discord`, `email`, `webhook`, `feishu`, `dingtalk`, …, `recipient` is the chat ID / channel ID / email address / URL, and an optional `thread_id` picks a thread or topic on adapters that have them. |
| `alert_channels` | `Vec<NotificationTarget>` | `[]` | Where task completion / failure alerts go. Same target shape as `approval_channels`. |
| `agent_rules` | `Vec<AgentNotificationRule>` | `[]` | Per-agent overrides, replacing both global lists for the events they name. All three fields are required: `agent_pattern` (glob over agent names, `*` for every agent), `events` (any of `approval_requested`, `task_completed`, `task_failed`, `tool_failure`, `content_filtered`, `health_check_failed`, `model_migrated`), and `channels` (same target shape as above). `content_filtered` has no global list, so a rule is the only way to route it. |
...
Unresponsive-agent alerts travel this path too, under the event type `health_check_failed` — that is the string to list in an `agent_rules` entry's `events`.
For that one event there is a third layer between the two above: an agent's own `[autonomous] heartbeat_channel` in `agent.toml`.
Precedence, most specific first: a matching `agent_rules` entry, then the agent's `heartbeat_channel`, then `alert_channels`.
The manifest shorthand accepts `"telegram"` (recipient taken from the `owner` user's `channel_bindings` entry for that channel) or `"telegram:123456"` (recipient given directly); a value that resolves to neither is logged and ignored, so the alert still reaches whatever `[notification]` routes it to.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `cooldown_secs` | u64 | `5` | Minimum seconds between consecutive firings of the same window. Prevents chatty triggers from saturating the agent loop. Patterns that name a subject — task board, memory key, agent lifecycle — hold one window per subject, so two different tasks completing a second apart both fire. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `claim_ttl_secs` | u64 | `600` | How long an `in_progress` task may stay claimed before the sweeper resets it. `0` disables the *global* clock only — a task that declared its own `timeout_secs` still expires. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `config_version` | u32 | `1` | Configuration schema version. Used by the auto-migrator to know whether your config needs upgrading on daemon boot. **Do not set manually** — `librefang upgrade` writes it. The exception is a config assembled with `include = [...]`: the migration runs in memory but is never written back for those files, because the write-back serialises the *merged* configuration and would inline every included file into `config.toml` and delete the `include` directive. For those the daemon logs a warning naming the includes and asks you to apply the schema change and stamp this key by hand. |
