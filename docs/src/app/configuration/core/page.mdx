...
A dream fires for an agent when all gates hold: `enabled = true`, the agent's manifest has `auto_dream_enabled = true`, `min_hours` have elapsed since its last dream, `min_sessions` have been touched since then, and the per-agent lock can be acquired. Manual triggers via `POST /api/auto-dream/agents/{id}/trigger` bypass the time and session gates but still respect the lock. Live status and abort control are exposed at `GET /api/auto-dream/status` and `POST /api/auto-dream/agents/{id}/abort`, and surfaced on the web dashboard Settings page and the CLI TUI Dashboard tab.
