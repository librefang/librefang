...
| `agent_name` | `agent` | `StepAgent::ByName` | -- | Reference an agent by its name (first match). Mutually exclusive with `agent_id` and `agent_type`. |
| `agent_id` | `agent` | `StepAgent::ById` | -- | Reference an agent by its UUID. Mutually exclusive with `agent_name` and `agent_type`. |
| `agent_type` | `agent` | `StepAgent::ByType` | -- | Reference an agent *type* — the registered agent of that name if one exists, otherwise spawned from the template of that name. Mutually exclusive with `agent_id` and `agent_name`. |
...
Every step must specify **exactly one** of `agent_name`, `agent_id` or `agent_type`. The `StepAgent` enum is:

```rust
pub enum StepAgent {
    ById { id: String },       // UUID of an existing agent
    ByName { name: String },   // Name match (first agent with this name)
    ByType { template: String }, // Agent type — find-or-spawn from a template
}
```

A step carrying more than one routing key is rejected at the API boundary and by the TOML/JSON deserializer, rather than resolved by key precedence.
Ambiguity resolved silently binds the step to a different agent than the one the author is reading, and nothing in the run output says which one won.

If the agent cannot be resolved at execution time, the workflow fails with `"Agent not found for step '<name>'"`.

#### Agent types (find-or-spawn)

`agent_type` lets a workflow express "use the researcher agent type" without the operator pre-registering an instance.
Resolution is find-or-spawn: the registered agent of that name is reused if there is one; otherwise the template manifest of that name is loaded and spawned top-level under the canonical name-derived UUID, so the type keeps one identity — and one session history — across daemon restarts.

Templates are read from `<home>/workspaces/agents/<type>/agent.toml`, falling back to the read-only registry checkout at `<home>/registry/agents/<type>/agent.toml`.
A local template shadows the registry copy of the same name.
The manifest's `name` must match the directory it lives in (a manifest that omits `name` inherits the directory name); a manifest declaring a different name is refused rather than spawned, because it would register under the declared name and the type would never converge on one instance.

Loading distinguishes the failure modes in the daemon log — a missing template, an unreadable file, a manifest that does not parse, and a name mismatch each report their own reason and path, so a typo in the workflow is never confused with a corrupt manifest on disk.

A **dry run never spawns**: `POST /api/workflows/{id}/dry-run` previews the agent an `agent_type` step would use without creating it.
