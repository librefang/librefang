...
Lazy loading is opt-in per agent via `lazy_tools = true` in the manifest. Disable to restore the legacy "all tools always loaded" behaviour. This is what the **Lazy Tool Loading** mention on [Agent Types](/agent/templates) refers to.
...
| `task_claim` | optional `priority_filter` | Claim ownership of the highest-priority pending task. Reclaimed after `[task_board] claim_ttl_secs` (default 600s), or after the task's own `timeout_secs` override if one was set when it was posted — `task_post` never sets that override, so it only shows up on tasks posted through the API or dashboard. |
