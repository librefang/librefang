...
| `speech_to_text(audio_or_video, language?, prompt?)` | Same providers and video-container support as `media_transcribe`; falls back to a permissive best-guess MIME type instead of rejecting an unrecognised extension. |
...
A window starting at `0` begins a new file; later windows append to it, so the calls assemble one transcript without the agent holding any of it.
