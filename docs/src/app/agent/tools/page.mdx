...
The same two scopes gate **automatic** memory as of #7605, with one difference: there a declared-but-empty list (`memory_read = []`) denies, where the tool gate treats it as undeclared. They are also withheld entirely when `[proactive_memory] enabled = false`, since the store is then never built.
