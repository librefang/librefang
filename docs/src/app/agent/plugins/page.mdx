...
BossFang loads the manifest, wires the declared hook scripts into the context engine, and runs them as subprocesses at well-defined lifecycle points. Each hook invocation is a fresh subprocess: one JSON object in on stdin, one JSON object out on stdout.
...
Return the trimmed / reordered message list. If you return an empty list or the hook fails, BossFang automatically falls back to the default trimming strategy.
...
Exit with a non-zero code and write an error to stderr. BossFang logs the stderr at `warn` level and falls back to the default context engine result. The turn still proceeds.
...
**Order of precedence** (highest wins): `[env]` values override BossFang's baseline set (`LIBREFANG_AGENT_ID`, `PATH`, `HOME`, runtime passthrough vars) for any key that appears in both. Variables in the agent's `allowed_env_vars` list are applied after `[env]` and therefore take final precedence.
...
Every hook is just a script that reads JSON from stdin and writes JSON to stdout — you can test it without BossFang running:
