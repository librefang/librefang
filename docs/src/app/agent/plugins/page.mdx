...
> **Log volume** — every non-empty stderr line emits an `info!` event. High-frequency hooks (per-step ML inference progress, packet-by-packet network probes, byte-by-byte file scanners) can flood `journalctl`. Either down-rate by routing only milestones to stderr and keeping per-iteration noise on stdout / a side log file, or filter at subscriber level with `RUST_LOG=plugin_stderr=warn` (silences streaming while keeping the post-exit `debug!` summary intact).

> **Memory safety** — the post-exit summary that gets `debug!`-logged is capped at 1 MiB to prevent a runaway hook from OOM-ing the daemon. Output past the cap is silently dropped from the summary with a single truncation marker; the live `*_stderr` tracing channel keeps streaming uninterrupted, so the full output is still observable in real time — just not in the post-exit dump.
