...
LibreFang provides **32 agent types** from the [registry](https://github.com/librefang/librefang-registry/tree/main/agents). The `assistant` agent type is pre-installed; others can be installed from the dashboard. Each agent type is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Agent types cover software engineering, research, writing, business operations, personal productivity, and system tasks.

All agent types use `provider = "default"` and `model = "default"`, which means every agent inherits whichever model you have configured as your default in `~/.librefang/config.toml`. You can override the model at spawn time if needed.

## Quick Start

Spawn any agent type from the CLI:
...
Spawn from a built-in agent type:
...
Agent types are organized by use case. Every agent type uses the default model provider. The table below lists each agent's description, tags, temperature, and max output tokens. Click an agent name to view its full `agent.toml` manifest in the registry.

### Software Engineering

| Agent Type | Description | Tags | Temp | Max Tokens |
|----------|-------------|------|------|------------|
...
The `temperature` setting controls how deterministic or creative an agent's responses are. Agent types set this value based on what each agent does:
...
The `agents/custom/` directory is reserved for your own agent types. Create a new `agent.toml` file following the manifest format below.
...
# `memory_read` / `memory_write` also gate automatic memory (#7605): leaving a
# key out means "unrestricted", but writing `memory_read = []` denies — the
# agent then gets no memories injected into its prompt, and `memory_write = []`
# stops the turn being auto-memorized.
...
2. **Write a clear system prompt**. The system prompt is the most important part of the agent type. Be specific about the agent's role, methodology, output format, and limitations.
...
│   ├── IDENTITY.md     ← personality (archetype, vibe, greeting style)
...
**Using the alias**: the file tools accept either form — `@library/notes/today.md` or the absolute path it points at.
The alias is expanded before the sandbox check, so it grants nothing the absolute path would not: an unknown alias is rejected with the list of aliases the agent actually has, `..` inside the remainder is refused, and a write through a `mode = "r"` alias is denied exactly as a write to its absolute path is.
Aliases are expanded in the `path` argument of `file_read`, `file_write`, `file_list`, `code_search`, `image_analyze`, `media_describe` and `media_transcribe`, in `channel_send`'s `file_path` and in `web_fetch_to_file`'s `dest_path`.
`apply_patch` is the exception: its targets live inside the patch text rather than in a path argument, so write them as absolute paths.
...
The full catalog is still rendered as a one-line-per-tool hint list in the system prompt, so the model knows what exists without paying for full schemas. **Tools the runtime classifies as "always native"** (`tool_search` / `tool_load` themselves, the `file_*` and `memory_*` groups, `web_search`, `web_fetch`, `code_search`, `agent_send`, `agent_list`, `channel_send`, `notify_owner`, `read_artifact` and the `skill_evolve_*` group) keep their full schema in every request — there's no point making the model `tool_load` something it almost certainly will use.
`shell_exec` and the cron tools are *not* in that list; like everything else in the catalog they stay behind `tool_search` / `tool_load` while `lazy_tools` is on.
...
All agent types use `provider = "default"` and `model = "default"`. The model that agents actually use depends on which provider and model you configure in `~/.librefang/config.toml`.
...
`agent.toml` accepts a number of top-level fields that aren't shown in the standard quick-start agent types. Listing the full set so you can find the field name when you need it. All fields are optional unless marked otherwise.
