...
Prompt Intelligence provides **prompt version management** and **A/B experiments** for BossFang agents. Track prompt changes over time, compare variants side-by-side, and let the system automatically activate the best-performing prompt.
...
When `enabled = true`, BossFang computes a hash of each agent's system prompt at the start of every request. If the prompt content has changed since the last tracked version, a new version is created automatically and marked as active.
...
Prompt Intelligence uses SQLite (same database as the rest of BossFang). Migration v13 creates four tables:
