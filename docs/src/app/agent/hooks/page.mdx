...
agent code. When an event fires, BossFang spawns the configured executable, passes
...
| `gateway:startup` | The BossFang daemon finishes its boot sequence |
...
BossFang passes two variables to every hook process:
...
| Field | Type | Description |
|---|---|---|
| `version` | string | BossFang daemon version, e.g. `"2026.4.21-beta1"` |
...
- If a hook exits with a non-zero code, BossFang logs a `WARN` entry with the hook name,
...
- **No secrets injection.** BossFang does not expose vault secrets to hook processes.
