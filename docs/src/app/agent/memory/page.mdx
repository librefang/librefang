...
BossFang's memory system provides persistent storage, semantic search, and knowledge graph functionality.
...
The BossFang memory system includes:
...
BossFang supports semantic search with vector embeddings:
...
The `MemoryProvider` trait allows integrating external memory backends — such as vector databases, knowledge graphs, or remote storage services — alongside BossFang's built-in SQLite memory.
