...
After configuring the official repository above:

```bash
sudo pacman -S librefang-desktop-bin
```

The desktop package is available on x86_64 only. On aarch64, install
`librefang-bin` and use the web dashboard at `http://127.0.0.1:4545/`.
