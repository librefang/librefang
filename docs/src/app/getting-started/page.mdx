# Getting Started with BossFang

This guide walks you through installing BossFang, configuring your first LLM provider, spawning an agent, and chatting with it.
...
SDKs for integrating BossFang API into your applications.
...
BossFang needs at least one LLM provider API key. Set it as an environment variable:
...
BossFang supports three layers of key storage (highest priority first):
...
BossFang provides 32 agent templates from the [registry](https://github.com/librefang/librefang-registry/tree/main/agents) (assistant pre-installed, others available from dashboard). Spawn the assistant agent:
...
Now that you have BossFang running:
