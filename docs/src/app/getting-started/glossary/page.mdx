...
Definitions of BossFang-related terms and concepts.
...
Application programming interface that allows external programs to communicate with BossFang.
...
Command-line interface for interacting with BossFang through the terminal.
...
### Daemon
BossFang daemon running in the background.
...
### Hand
BossFang's autonomous capability package that can run independently and execute tasks.
...
### Kernel
BossFang's core component responsible for coordinating all subsystems.
...
### LLM Driver
Adapter layer that BossFang uses to interact with different LLM providers.
...
### Memory
BossFang's persistent storage system including sessions, vector search, and knowledge graphs.
...
BossFang's peer-to-peer communication protocol.
