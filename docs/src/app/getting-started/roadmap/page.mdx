...
BossFang v0.1.0 release roadmap, organized into 4 sprint phases.
...
BossFang compared against similar open-source solutions.

| Feature | Comparable Solutions | BossFang | Advantage |
|---------|---------------------|-----------|-----------|
| Language / Performance | Node.js (~200MB) | Rust (~30MB single binary) | **BossFang** |
| Channel Integrations | ~15 | 44 | **BossFang** |
| Built-in Tools | ~19 | 41 | **BossFang** |
| Security Systems | Token + Sandbox | 16-layer defense-in-depth | **BossFang** |
| Agent Templates | Manual config | 32 pre-configured templates | **BossFang** |
| Autonomous Capabilities | None | 15 Hands packages | **BossFang** |
| Workflow Engine | Cron + Webhooks | Full DAG (parallel / loops) | **BossFang** |
| Knowledge Graph | Flat vector store | Entity-relation graph | **BossFang** |
| P2P Networking | None | OFP protocol | **BossFang** |
| WASM Sandbox | Docker only | Dual-metered WASM | **BossFang** |
| Desktop App | Electron (~200MB) | Tauri (~30MB) | **BossFang** |
| Migration Tools | None | `migrate --from openfang/openclaw` | **BossFang** |
| Skill Ecosystem | 54 bundled | 60 bundled | **BossFang** |
| LLM Providers | ~15 | 49 drivers / 130+ models | **BossFang** |
| SDKs | TypeScript | JS + Python | Tie |
| Native Mobile | iOS + Android + macOS | In Progress ([#3351](https://github.com/librefang/librefang/issues/3351)) | **BossFang** (soon) |
| Voice Mode | Wake word + TTS | Mic + TTS playback | Comparable (slight) |
| Visual Workflows | None | Drag-and-drop builder | **BossFang** |

<Note>
  BossFang wins 15 out of 18 categories. Remaining gaps: native mobile apps and voice wake word.
</Note>
