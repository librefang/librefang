...
This page is organized around BossFang's core capabilities: agents, hands, channels, workflows, scheduling, webhooks, memory, and API usage. The examples below follow the current CLI, REST API, and config shapes in the source tree.
...
## What BossFang Can Do

Start with the capability map, then move to concrete usage.

In the current source tree, BossFang is not just a chat wrapper. It is an Agent OS with several layers:
...
1. What can BossFang be used for?
...
The fastest way to use BossFang is to spawn a built-in template and send it a message.
...
This is the base layer of BossFang: run an agent and make it complete a concrete task. If your use case is code generation, review, writing, research, or support, this is usually the first layer to reach for.
...
This is BossFang's external interface layer: the agent is no longer only a local CLI object, but something users can interact with from Telegram, Discord, Slack, WeChat, and similar systems.
...
This is the part of BossFang that is closest to “productized capabilities”. For research, OSINT, lead generation, browser automation, content workflows, and similar tasks, Hands are often the right starting point instead of assembling raw agents from scratch.
...
- Channel config belongs under `[channels.<name>]`, for example `[channels.discord]`. Telegram is sidecar-only and uses `[[sidecar_channels]]` instead of `[channels.telegram]`.
