...
The model catalog also supports **dynamic loading** — official provider definitions are pre-installed to `~/.librefang/providers/` from the registry on first boot. You can add or override providers by placing custom TOML files in the same directory. All `~/.librefang/providers/*.toml` files are merged into the catalog at boot, allowing you to add private endpoints, on-premises deployments, or new providers without modifying the core configuration.
