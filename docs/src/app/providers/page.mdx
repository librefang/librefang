...
LibreFang ships with a comprehensive model catalog covering **3 native LLM drivers**, **28 providers**, **51 builtin models**, and **23 aliases**. Every provider uses one of three battle-tested drivers: the native **Anthropic** driver, the native **Gemini** driver, or the universal **OpenAI-compatible** driver. This guide is the single source of truth for configuring, selecting, and managing LLM providers in LibreFang.
...
| **Key Required** | Yes (service account) |
| **Free Tier** | No |
| **Auth** | OAuth2 service account |
| **Models** | Gemini models via Vertex AI enterprise endpoint |
...
Files in `~/.librefang/providers/` are loaded at startup and merged into the catalog alongside the 28 builtin providers.
...
Returns all 28 providers with auth status and model counts.
...
Lists all 28 providers with their authentication status.
