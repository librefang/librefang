...
- **Block** at 5 identical tool calls, or at 3 identical *results* for the same parameters — a tool that keeps answering the same way is refused on its fourth call rather than its fifth
- **Circuit breaker** at 30 total tool calls (stops the agent)
...
**Platform-specific**:
- **Linux**: Requires a StatusNotifierItem host/system tray (e.g., AppIndicator Support extension on GNOME)
