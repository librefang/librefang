...
Common issues, diagnostics, and answers to frequently asked questions about BossFang.
...
BossFang uses `tracing` for structured logging. Set the log level via environment:
...
**Automatic protection**: BossFang has a built-in loop guard:
...
**Cause**: usually the plugin's declared runtime isn't installed on this host. The hook fails with `LauncherNotFound`, BossFang logs a warning, and falls back to the default context engine — so nothing crashes, nothing visible changes, and memories just never come back.
...
3. Look at BossFang logs for `Timeout` entries naming your hook.

### Hook runs but memories don't show up

**Cause**: output isn't valid JSON, or the last line of stdout isn't the JSON response.

BossFang scans stdout from bottom to top and uses the first JSON-parseable line. Debug logs printed *before* the JSON are fine; logs printed *after* are fine; but if the last line is something like `done.` or a traceback, the response is lost.
...
Restart BossFang after editing config.
...
### Can I run BossFang without an internet connection?
...
### What's the difference between BossFang and OpenClaw?

| Aspect | BossFang | OpenClaw |
|--------|----------|----------|
...
BossFang can import OpenFang configs: `librefang migrate --from openfang`

BossFang can also import OpenClaw configs: `librefang migrate --from openclaw`
...
### Can I use BossFang as a library?
