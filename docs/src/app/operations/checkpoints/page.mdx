...
Before every `file_write` or `apply_patch` tool call, BossFang automatically snapshots the
...
only the files that BossFang has written — it does not mirror the entire project tree on
every snapshot.
...
BossFang skips snapshotting under two conditions:
...
repo. BossFang logs a `DEBUG` entry when a file is skipped for this reason. The `file_write`
...
directory that BossFang operates on.
