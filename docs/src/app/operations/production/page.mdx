...
No severity is suppressed and `--ignore-unfixed` is never used, so the report contains every finding at every severity, fixed and unfixed alike.

**Enforcement threshold.** The gate ships **report-only** and is *not* currently enforced.
The runtime image carries a large pre-existing backlog (10 critical and 95 high findings as of 2026-08-01, issue #6694), so an armed gate would fail `main` and every release on its first run.
The threshold lives in exactly one place — the `fail-on` input of `.github/actions/trivy-image-scan/action.yml` — and neither workflow overrides it, so a maintainer arms both by changing that single default:
...
**Where the reports are retained.** Three places, for 30 days of workflow-artifact retention:
