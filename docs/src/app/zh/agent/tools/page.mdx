...
BossFang 自带的工具目录远不止标准 agent 模板里看到的 file / shell / web 工具。本页覆盖那些一直只活在源码里的家族 —— 浏览器自动化、子进程生命周期、运行时调度、知识图谱、媒体生成、agent 间消息。
...
不同 BossFang 实例上的 agent 可以通过 A2A 协议互发任务。
...
跟**同一 BossFang daemon 里**别的 agent 通话(无 A2A 协议开销) —— 直接 spawn、list、find、kill、发消息。
