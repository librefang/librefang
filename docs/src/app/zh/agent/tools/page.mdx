...
| `task_claim` | 可选 `priority_filter` | 认领最高优先级 pending 任务。超过 `[task_board] claim_ttl_secs`(默认 600s)后被重置;如果任务发布时带了自己的 `timeout_secs` 覆盖值,则以那个为准——`task_post` 从不设置这个覆盖值,所以只有通过 API 或 dashboard 发布的任务才会有。 |
