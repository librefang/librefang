...
`[metadata] frequency` 声明 Hand 的 Agent 是否自行唤醒，以及唤醒频率。

```toml
[metadata]
# continuous | hourly | daily | periodic | on-demand
# 默认值是 "on-demand"；"reactive" 是它的别名。
frequency = "continuous"
```

只有当 Hand 声明了唤醒节奏，**并且**该角色自身声明了 `[autonomous]` 护栏（扁平格式下由 `max_iterations` 合成）时，这个角色才会自行唤醒。
后半个条件的作用是让多 Agent Hand 中被派发的子 Agent 保持休眠：把护栏加在真正跑循环的角色上，协调者派发的角色不要加。
`max_iterations` 本身只是 Agent 循环的迭代上限，不代表任何调度意图。

各节奏对应的间隔：`continuous` 按角色自己的 `heartbeat_interval_secs`（默认 30 秒），`hourly` 为 3600 秒，`daily` 为 86400 秒，`periodic` 为 300 秒。
`on-demand` 永不自行唤醒。

角色可以自己指定 `schedule`，它覆盖由 `frequency` 推导出的节奏，也是取得 cron（`periodic`）或条件驱动（`proactive`）调度的唯一途径。
`ScheduleMode` 使用外部标签（externally tagged）序列化，因此结构体变体必须写成子表——裸的 `schedule = "continuous"` 字符串无法解析。
...
这两项声明都在激活时解析，因此修改会在下一次激活该 Hand 时生效——或者在下一次守护进程启动时生效，届时所有持久化的 Hand 都会重新激活。
