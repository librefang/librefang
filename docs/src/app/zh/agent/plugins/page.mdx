...
└── hooks/
    ├── ingest.py        # （或 .js / .go / .rb / ...）
    ├── after_turn.py
    ├── assemble.py      # 可选：控制 LLM 看到什么
    └── compact.py       # 可选：自定义压缩策略
```
...
插件默认沙箱化 —— 环境变量被洗成最小集合，工作目录受控，30 秒超时会 kill 掉跑飞的 hook。
...
| `hooks.bootstrap` | string | 否 | 引擎初始化时运行一次。 |
