...
> **日志量提醒** —— 每行非空 stderr 都会发一条 `info!` 事件。高频输出的 hook（按步推进的 ML 推理进度、按包探测的网络脚本、按字节扫描的文件分析等）很容易把 `journalctl` 撑爆。两种缓解：要么把每轮细节路由到 stdout 或者侧写日志,只留里程碑到 stderr;要么在 subscriber 级别过滤 `RUST_LOG=plugin_stderr=warn`(屏蔽实时流但保留退出后的 `debug!` 汇总)。

> **内存安全** —— 退出时打到 `debug!` 的汇总缓冲被限制在 1 MiB,防止失控的 hook 把 daemon 打到 OOM。超出部分会从汇总里静默丢弃,只留一条截断标记;实时的 `*_stderr` tracing 通道继续流式输出不受影响,完整输出仍然能实时观察到 —— 只是不会出现在退出后的汇总里。
