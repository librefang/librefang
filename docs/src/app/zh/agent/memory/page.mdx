...
- [外部向量后端](#external-vector-backend)
- [使用追踪](#usage-tracking)
...
近似最近邻检索是目前记忆子系统中唯一接受外部实现的部分，接缝就是 `VectorStore` trait。
LibreFang 仍然拥有 SQLite 行存储、抽取流水线和全部隔离守卫；你接入的后端只决定哪些记忆 ID 距离查询向量最近。

### 启用 HTTP 后端

```toml
[memory]
vector_backend = "http"                                        # "sqlite"（默认）表示全部在进程内完成
vector_store_url = "http://127.0.0.1:6333/collections/memories"
```

`vector_backend = "http"` 而没有 `vector_store_url` 会让启动失败，而不是静默回退到 SQLite；除 `"http"` / `"sqlite"` 以外的取值会被按名字拒绝。
base URL 会去掉尾部斜杠后原样使用，路径直接拼接在它后面。

### HTTP 契约

你的服务需要在该 base URL 下提供四个端点。
响应体上限 64 MiB，连接超时 10 秒，单次请求总超时 30 秒。

| 方法 | 路径 | 请求体 | 响应体 |
|---|---|---|---|
| POST | `/insert` | `{ id, embedding, payload, metadata }` | 忽略；任意 2xx 即视为写入成功 |
| POST | `/search` | `{ query_embedding, limit, filter? }` | `[{ id, payload, score, metadata }]` |
| DELETE | `/delete` | `{ id }` | 忽略；任意 2xx 即视为删除成功 |
| POST | `/get_embeddings` | `{ ids }` | `{ "<id>": [f32, …], … }` |

`filter` 是调用方的 `MemoryFilter` 原样序列化的结果——agent、source、scope、最低置信度、创建时间上下界、metadata 等值对以及 peer。
遵守它可以让后端在排序前先剪枝，这正是它被发送过去的原因。

### 后端无权决定的部分

`/search` 返回的一切都被当作不可信数据，因为搜索结果是一个来自 LibreFang 无法控制的进程的 ID 列表。

- **ID 从 SQLite 水合，绝不从后端的 payload 水合。** 后端返回的 `payload` 和 `metadata` 不会进入 prompt，进入 prompt 的是 LibreFang 本地那条 ID 对应的行。
- **水合之后会重新应用调用方的 `MemoryFilter`。** 忽略了所收到 filter 的后端无法扩大调用方的范围：agent、scope、source、置信度下限、创建时间上下界和 metadata 等值都会在水合后的行上重新校验，而 `peer_id` 因为不由 `MemoryFragment` 承载，会在水合查询中作为 SQL 谓词强制执行。
- **跨会话（`chat_scope`）与跨 session（`session_scope`）隔离在其之后运行**，走的是与进程内后端相同的 recall 后置路径，所以接入后端不会重新打开这两道守卫中的任何一道。

### 失败行为

- 不是 UUID 的 ID 会被丢弃并打 `WARN`，结果集中的其余部分照常水合。一条畸形记录只损失它自己，而不是整次 recall。
- 水合不到任何行的 ID——已删除、未知、或属于另一个 peer——以同样方式被丢弃。
- 传输层失败（连接被拒、超时、非 2xx、无法解析的响应体）会作为该次 recall 的错误返回。不同调用方的处理不同：prompt 组装路径会记录日志并用手上已有的内容继续，而直接的 API 搜索会把错误返回给调用者。

### 在进程内实现该 trait

`vector_backend = "http"` 覆盖了任何你能放到一个小 HTTP 服务背后的后端，这是受支持的路径。
如果你更希望直接链接原生客户端，可以实现 `VectorStore` 并在启动时挂到 substrate 上：
...
`search` 是在 recall 路径上通过阻塞桥接调用的，因此它不能长时间阻塞执行器——上面那些超时正是为此存在的，原生客户端需要自备一套。
