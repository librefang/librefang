...
EveryAPI 是托管 provider，没有固定的模型清单。
可用模型取决于你的账户而非 LibreFang。
LibreFang 通过 EveryAPI CLI 的凭据进程获取当前网关与 relay 凭据，再加载账户的实时模型目录。
...
托管发现模式下，EveryAPI 始终是凭据权威。
LibreFang 在需要凭据时调用 `everyapi auth credential --format=json`，且不会持久化返回的密钥。
旧版兼容 CLI 的凭据仍可作为回退读取。
显式端点或环境变量配置仍然可用，并优先于托管发现。
