...
模型元数据取自 LibreFang 的内置目录，因为网关 `/v1/models` 的响应只带模型 id，没有价格，多数条目也没有上下文窗口。凡是上下文窗口与输出上限都无法解析的文本模型会被跳过并在命令输出中点名，而不是注册进去 —— 文本条目缺少其中任一字段会在目录加载时被丢弃，注册它只会让模型悄悄消失，仅留下一行日志。价格无法解析时，条目会标记为价格未知，避免预算统计把它当成免费。
