...
本页覆盖需要 API Key、并通过 BossFang 原生 Anthropic/Gemini 驱动或共享 OpenAI-compatible 驱动接入的云端模型提供商。
...
- 视觉**未接入**：image content block 在请求前被丢弃，因为 Bedrock Converse 不接受 BossFang 的 `Image` / `ImageFile` 形状。
