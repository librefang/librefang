...
| `dm_policy`、`group_policy` | 不是 sidecar 环境变量 —— DM / 群组授权由内核侧的 `[[users]]` channel bindings（`authorize_channel_user`）强制执行。只有 WhatsApp sidecar 读 `WHATSAPP_DM_POLICY`；`WHATSAPP_GROUP_POLICY` 会被读取但目前不生效，Telegram / Discord / Slack / Teams 四个 sidecar 完全不读 `*_DM_POLICY` / `*_GROUP_POLICY`。 |
