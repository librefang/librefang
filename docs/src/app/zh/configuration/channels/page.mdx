...
每个通道都自带一组内置斜杠命令（`/agent`、`/new`、`/reboot`、`/model`、`/usage` 等），可用于切换 Agent、重置会话、查看用量、触发工作流。在**面向公众的 bot**（客服、社区助手）上暴露这些命令是安全隐患——任何人都可以打 `/agent admin` 切到内部 Agent，`/new` 清空会话，或者 `/model` 换模型。
...
命令名是内部使用的裸 token（不带 `/`）。完整命令列表见 `/help`。
