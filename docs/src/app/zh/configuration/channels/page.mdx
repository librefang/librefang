...
Baileys 网关（`@librefang/whatsapp-gateway`，Node.js，监听 `WHATSAPP_GATEWAY_URL`）**需要单独启动** —— kernel 不再内嵌或自动 spawn 它。入站消息由网关直接 POST 到 BossFang REST API（`POST /api/agents/{id}/message`），绕过 sidecar；出站回复经 sidecar 路由到 `{gateway_url}/message/send`。
...
LINE 由 Python sidecar 适配器（`librefang.sidecar.adapters.line`）提供，进程内 `[channels.line]` 配置块已在 sidecar 迁移中移除。请改用 `[[sidecar_channels]]` 形式声明 —— sidecar 自身运行 HTTP Webhook 服务器（不再挂载在 BossFang API 端口上），因此你在 LINE Developers 控制台注册的 Webhook URL 现在为 `https://<sidecar-host>:<LINE_WEBHOOK_PORT><LINE_WEBHOOK_PATH>`：
...
| `dm_policy`、`group_policy` | 移到了 agent 上：写在 `agent.toml [channel_overrides]` 里 —— channel bridge 在每次派发时强制执行，`allowed_only` 的用户白名单由 `[[users]]` channel bindings（RBAC）背书。它们不是 sidecar 环境变量：只有 WhatsApp sidecar 读 `WHATSAPP_DM_POLICY`（仅 Cloud API webhook 模式生效），`WHATSAPP_GROUP_POLICY` 会被读取但目前不生效，Telegram / Discord / Slack / Teams 四个 sidecar 完全不读 `*_DM_POLICY` / `*_GROUP_POLICY`。 |
