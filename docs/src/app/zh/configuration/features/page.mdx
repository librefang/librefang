...
| `totp_issuer` | string | `"BossFang"` | 注册时显示在验证器应用中的发行者名称。 |
...
压缩算法实现于 `ContextEngine` trait 之后。当前实现使用基于 LLM 的摘要器。BossFang 未来版本将支持配置替代压缩算法——包括基于 DAG 的压缩和 LCM（Latent Context Modeling）——等实现稳定后开放配置。
...
BossFang 自带可选的可观测性栈，一条 `docker compose` 起栈。捕获 HTTP 请求**和** BossFang 自己的工作 — agent 回合、tool 调用、channel 发送、cron 触发 — 作为可搜索的 Tempo trace。
...
**自动启动是 opt-in 的。** 可观测性栈**不**会跟着 `librefang start` 自动起 —— 想让 daemon 帮你管理生命周期就在 `config.toml` 设 `[telemetry] auto_start = true`。否则用 `librefang observability up` / `down` 显式管理。捆绑的容器按 `home_dir` 加 Docker label，同一台机器上多个 BossFang 安装不会抢同名容器；拆栈用 RAII 风格清理，daemon 异常退出时也能回收。
