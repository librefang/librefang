...
| `allow_remote` | bool | `false` | 在未配置认证时，允许来自远程或反向代理连接的未认证访问。默认行为是仅允许本地无认证访问。 |
...
| `trust_proxy_headers` | bool | `false` | 当为 `true` 时，没有代理头（X-Forwarded-For、X-Real-IP）的环回连接将被拒绝。仅在使用注入这些头的反向代理时启用。 |
