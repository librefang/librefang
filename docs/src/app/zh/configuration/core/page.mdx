...
当所有闸门同时满足时才会触发：`enabled = true`、agent manifest 中 `auto_dream_enabled = true`、距离上次梦境至少 `min_hours` 小时、至少触及 `min_sessions` 个会话、以及能获取到该 agent 的文件锁。通过 `POST /api/auto-dream/agents/{id}/trigger` 手动触发可绕过时间和会话闸门，但仍须获取锁。实时状态与中止控制通过 `GET /api/auto-dream/status` 和 `POST /api/auto-dream/agents/{id}/abort` 暴露，并在 Web Dashboard 设置页及 CLI TUI Dashboard 标签页呈现。
