...
`config.toml` 的完整参考文档，涵盖 BossFang Agent OS 中每一个可配置字段。
...
BossFang 支持通过 `include` 字段将配置拆分到多个文件中。这使得可以实现模块化配置（例如将通道配置、MCP 服务器配置或特定环境的覆盖配置分开存放）。
...
最简单的可用配置只需将 LLM 提供商的 API Key 设置为环境变量。如果没有配置文件，BossFang 将以 Anthropic 作为默认提供商启动：
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `home_dir` | path | `~/.librefang` | BossFang 主目录，存储配置、Agent、技能等。 |
...
配置 OFP（BossFang 协议）点对点网络层，使用 HMAC-SHA256 双向认证。
...
LINE 由 Python sidecar 适配器（`librefang.sidecar.adapters.line`）提供，进程内 `[channels.line]` 配置块已在 sidecar 迁移中移除。请改用 `[[sidecar_channels]]` 形式声明 —— sidecar 自身运行 HTTP Webhook 服务器（不再挂载在 BossFang API 端口上），因此 LINE Developers 控制台注册的 URL 现在为 `https://<sidecar-host>:<LINE_WEBHOOK_PORT><LINE_WEBHOOK_PATH>`：
...
Agent 间协议（A2A）配置，实现跨 BossFang 实例的 Agent 间通信。
...
| `name` | string | `"BossFang Agent OS"` | 在 well-known Agent 名片中展示的服务级显示名称。 |
...
配置 Prompt 版本管理和 A/B 实验功能。启用后，BossFang 自动追踪 prompt 版本历史，支持运行 A/B 实验对比不同 prompt 变体。详细文档见 [Prompt 智能指南](/zh/agent/prompt-intelligence)。
