...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `access_token_env` | string | `"DINGTALK_ACCESS_TOKEN"` | 存放钉钉 Webhook 访问令牌的环境变量。 |
| `secret_env` | string | `"DINGTALK_SECRET"` | 存放钉钉签名密钥的环境变量。 |
| `webhook_port` | u16 | `8457` | 入站 Webhook 端口。 |
| `default_agent` | string 或 null | `null` | 消息路由的目标 Agent 名称。 |
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `corp_id` | string | `""` | 企业微信企业 ID。 |
| `agent_id` | string | `""` | 企业微信应用 Agent ID。 |
| `secret_env` | string | `"WECOM_SECRET"` | 存放企业微信应用密钥的环境变量。 |
| `webhook_port` | u16 | `8454` | 入站 Webhook 端口。 |
| `token_env` | string 或 null | `null` | 存放回调验证令牌的环境变量。验证入站消息时必填。 |
| `encoding_aes_key_env` | string 或 null | `null` | 存放消息加解密 AES Key 的环境变量。 |
| `default_agent` | string 或 null | `null` | 消息路由的目标 Agent 名称。 |
| `account_id` | string 或 null | `null` | 用于多 Bot 路由的唯一 Bot 实例标识符。 |
| `overrides` | object | （默认值） | 每个通道的行为覆盖配置。详见[通道行为覆盖](#通道行为覆盖)。 |
...
| `hooks.ingest` | string 或 null | `null` | `ingest` 钩子的 Python 脚本路径（在收到新用户消息时调用）。 |
| `hooks.after_turn` | string 或 null | `null` | `after_turn` 钩子的 Python 脚本路径（在每轮对话完成后调用）。 |
...
配置 Prompt 版本管理和 A/B 实验功能。启用后，LibreFang 自动追踪 prompt 版本历史，支持运行 A/B 实验对比不同 prompt 变体。详细文档见 [Prompt 智能指南](/zh/prompt-intelligence)。
...
| `DINGTALK_ACCESS_TOKEN` | DingTalk | 钉钉 Webhook 访问令牌。 |
| `DINGTALK_SECRET` | DingTalk | 钉钉签名密钥。 |
