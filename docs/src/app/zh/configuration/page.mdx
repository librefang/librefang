...
1. 使用现有的 `dashboard_pass` 明文密码正常登录 Dashboard
2. 登录成功后查看服务日志，会输出可用的哈希值：
   ```
   Set `dashboard_pass_hash = "$argon2id$v=19$m=19456,t=2,p=1$..."` in config.toml ...
   ```
3. 将哈希值写入 `config.toml`，并删除明文密码：
   ```toml
   dashboard_user      = "admin"
   dashboard_pass_hash = "$argon2id$v=19$m=19456,t=2,p=1$<salt>$<hash>"
   ```
