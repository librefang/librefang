...
当所有闸门同时满足时才会触发：`enabled = true`、agent manifest 中 `auto_dream_enabled = true`、距离上次梦境至少 `min_hours` 小时、至少触及 `min_sessions` 个会话、以及能获取到该 agent 的文件锁。通过 `POST /api/auto-dream/agents/{id}/trigger` 手动触发可绕过时间和会话闸门，但仍须获取锁。
