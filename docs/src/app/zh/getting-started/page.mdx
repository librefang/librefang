...
在 macOS 上，发行版二进制使用 Developer ID 签名，完全磁盘访问权限和自动化授权会在更新后保留；从旧的 ad-hoc 签名版本升级时，需要重新授予一次（参见 [`librefang update`](/zh/integrations/cli#librefang-update)）。
...
它会链接完整的 GTK / webview 闭包（`gtk3`、`libsoup_3`、`webkitgtk_4_1`），因此构建耗时远高于 CLI，而且会走到 CLI 软件包根本不会触及的代码路径。
