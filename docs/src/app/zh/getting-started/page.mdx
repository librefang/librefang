...
本指南将带你安装 BossFang、配置第一个 LLM 供应商、生成 Agent 并与其对话。
...
<Note>
AUR 账户注册目前暂时不可用，因此 BossFang 目前通过官方 pacman 仓库发布已签名的 Arch 软件包。
</Note>

导入并在本地信任 BossFang 软件包签名密钥：
...
用于将 BossFang API 集成到你自己应用中的 SDK。
...
BossFang 至少需要一个 LLM 供应商的 API Key。作为环境变量设置：
...
BossFang 在 [registry](https://github.com/librefang/librefang-registry/tree/main/agents) 中提供了 32 个 Agent 模板（`assistant` 预装，其他可从仪表盘安装）。生成 hello-world Agent：
