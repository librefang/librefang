...
LibreFang 暴露 REST API、WebSocket 端点和 SSE 流式传输。默认监听地址 `http://127.0.0.1:4545`。
...
跨提供商的媒体生成 — 图像、语音合成、视频、音乐。LibreFang 会自动检测每种能力对应的首个已配置提供商，也可通过 `provider` 字段显式指定。
...
### ws://127.0.0.1:4545/api/agents/\{id\}/ws

WebSocket 实时聊天。

```javascript
const ws = new WebSocket('ws://127.0.0.1:4545/api/agents/\{id\}/ws');
...
```bash
curl -N http://127.0.0.1:4545/api/agents/\{id\}/stream \
...
```bash
curl http://127.0.0.1:4545/v1/models
```
