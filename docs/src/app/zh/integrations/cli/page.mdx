...
`librefang` 命令行工具的完整参考，用于管理 BossFang Agent OS。

## 概述

`librefang` 二进制是管理 BossFang Agent OS 的主要接口。支持两种操作模式：
...
初始化 BossFang 工作空间。创建 `~/.librefang/` 目录，包含子目录（`data/`、`workspaces/agents/`）和默认的 `config.toml`。
...
- 启动 BossFang 内核（加载配置、初始化 SQLite 数据库、加载 Agent、连接 MCP 服务器、启动后台任务）。
...
BossFang 支持 Apple 风格的更新频道。在 `config.toml` 中设置默认频道：
...
BossFang 在每次 CLI 调用时将 `~/.librefang/.env` 加载到进程环境中。系统环境变量优先于 `.env` 中的值。
