...
- 将源框架的 Agent 配置、YAML 清单和设置转换为 BossFang 格式。
- 将导入的数据保存到 `~/.librefang/`。
...
- 通过 stdin/stdout 上的 JSON-RPC 2.0 协议（Content-Length 分帧）将运行中的 BossFang Agent 暴露为 MCP 工具。
