...
channel adapter 暴露的命令（`/agents`、`/new`、`/usage`、`/triggers`、`/budget` 等）见 [Provider 管理 — Channel 命令](/zh/configuration/providers/management)。未知命令名只会得到一个 "unknown command" 错误，不会被传给模型。
