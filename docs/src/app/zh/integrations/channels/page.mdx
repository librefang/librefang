...
自 #6445 起，`[[sidecar_channels]]` 已暴露 `output_format`、`dm_policy`、`group_policy`、`threading` 的逐通道覆盖。每个字段都是可选的 —— 未设的字段继承 bridge 默认，且不会产生你没写的策略。agent 级 `[channel_overrides]` 仍优先于这些按通道设置的值。
