...
`ChannelOverrides` 用来让 agent 按通道自定义行为。
Sidecar 迁移移除的是旧的 `[channels.<name>.overrides]` 子表本身 —— 它挂在已删的进程内 `ChannelsConfig` 上 —— 但通道级这一层依然存在，只是改写成 sidecar 条目上的一等字段。
详见[频道配置指南里的迁移对照表](/zh/configuration/channels#通道行为覆盖旧版--已移除)。

- **Agent 级** 在 `agent.toml` —— 只对该 agent 在所有通道上生效。加顶层 `[channel_overrides]` 段。
- **通道级** 在 `[[sidecar_channels]]` —— 对该通道上的所有 agent 生效。
  直接写在条目的键上（`dm_policy`、`group_policy`、`threading`、`output_format`、`command_policy`、`message_coalesce_window_ms`），**不是**写在 `[sidecar_channels.env]` 里。
  只有至少设了其中一个，bridge 才会据此构造 `ChannelOverrides`；什么都没设的普通 sidecar 条目不贡献任何覆盖。

**解析顺序**：agent 级 → 通道级（`[[sidecar_channels]]`）→ 内置默认。

优先级是**整个结构体级别的，不是逐字段的**：只要 agent 有 `[channel_overrides]` 段，就整体采用它，通道级结构体不再参与；因此 agent 没设的字段会落到**内置默认**，而不是落到通道级的值上。
用哪一层，就把在意的字段在那一层全部写齐。
