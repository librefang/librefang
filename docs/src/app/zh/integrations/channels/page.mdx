...
BossFang 通过 **45 个通道适配器** 连接各类消息平台，让用户可以在所有主流通信平台上与 Agent 交互。适配器覆盖消费级即时通讯、企业协作、社交媒体、社区平台、隐私优先协议、通用 Webhook 以及可扩展的 Sidecar 进程。
...
- `bot_token_env` / `token_env` -- 保存 Bot/访问令牌的环境变量名。BossFang 在启动时从该环境变量读取令牌。所有密钥以 `Zeroizing<String>` 存储，释放时自动清零。
