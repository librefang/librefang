...
# SLACK_RESOLVE_DISPLAY_NAMES = "false"       # 设为 "true" 显示真实姓名而非原始 U… id
# SLACK_DISPLAY_NAME_TTL = "21600"            # 已解析姓名的缓存时长(秒)
```
...
默认情况下,adapter 把发送者的原始 Slack id(`U09…`)当作显示名上报,agent 看到的、以及群名册里存下的都是它。
设置 `SLACK_RESOLVE_DISPLAY_NAMES = "true"` 即可改为通过 Slack 的 `users.info` 解析真实姓名,同时顺带补上 `@handle`。
这需要 `users:read` 这个 bot scope;没有它每次查询都会失败,adapter 会继续上报 id。

查询结果按 user id 缓存 `SLACK_DISPLAY_NAME_TTL` 秒(默认六小时),所以繁忙频道一天只花掉少量 API 调用,而不是每条消息一次 —— `users.info` 有按方法计的速率限制,不缓存会很快耗尽。
"查不到"同样会被缓存:一个已删除的用户,或一个缺少 scope 的机器人,只花一次请求,而不是每条消息一次。
临时性失败(限流、连接中断)只抑制一分钟,工作区恢复后很快就会重新开始解析姓名。

**默认关闭,是因为它改变的是 LibreFang 存下来的内容,而不只是展示的内容。**
daemon 会为每个在群里发过言的人持久化一行名册记录(`channel`、`chat_id`、`user_id`、显示名、handle、首次与最近出现时间),这行记录里存的就是本 adapter 上报的值。
开关关着时存的是一个不透明的平台 id;打开后,真实姓名会写入 `~/.librefang/memory.db`,直到该行被删除为止。
目前这些行没有自动过期机制,所以请把启用它当作一个关于个人数据留存的决定,而不是一个展示偏好。
只想给少数几个人配人类可读的名字、又不想引入任何查询与存储变化的运维者,可以继续使用 `[users]` 条目,它始终具有最高优先级。
