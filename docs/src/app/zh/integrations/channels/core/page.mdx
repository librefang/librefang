...
Web/QR 模式下，网关直接把入站 POST 到 BossFang REST API（`/api/agents/{id}/message`），绕过 sidecar；出站回复经 sidecar 路由到 `{gateway_url}/message/send`。**kernel 不再自动 spawn 网关**（原嵌入的 `whatsapp_gateway.rs` 模块在本次迁移中已移除）—— 需要你自己单独启动它。
...
2. 启动（或重启）BossFang 守护进程。sidecar 会以 INFO 级日志输出二维码字符串 —— 用微信扫码确认。首次登录后，把 `bot_token` 填到 `WECHAT_BOT_TOKEN`（放在 `~/.librefang/secrets.env`）就能跳过后续扫码。
...
消息平台对出站消息有严格的字符数限制，以 **UTF-16 编码单元**（而非字节或 Unicode 码位）计量。BossFang 以 UTF-16 单元数衡量消息长度，并自动处理超长消息。
...
`split_to_utf16_chunks(text, limit)` 将消息分割为多个连续块，每块的 UTF-16 单元数均在 `limit` 以内。BossFang 按顺序发送各块，保留所有内容。拆分边界遵守 Unicode 标量边界——不会在字符中间结束一个块。
