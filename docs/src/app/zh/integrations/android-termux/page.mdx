# 在 Android 上运行 BossFang (Termux)

BossFang 在每个版本中提供预构建的 `aarch64-linux-android` 二进制文件，可以通过 [Termux](https://termux.dev/) 在 Android 设备上运行完整的 Agent OS。
...
Android 会积极终止后台进程。使用 `termux-wake-lock` 或在 `tmux`/`screen` 会话中运行 BossFang：
