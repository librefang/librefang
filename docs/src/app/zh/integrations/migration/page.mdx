...
OpenFang 和 BossFang 共享相同的目录结构和配置格式（BossFang 是 OpenFang 的社区分支）。迁移过程是将 `~/.openfang` 递归复制到 `~/.librefang`，同时自动重写 `.toml` 和 `.env` 文件中的引用。
...
> 以下指南展示如何手动将 LangChain 配置转换为 BossFang 格式。

### Agent 配置转换

将 LangChain agent 配置转换为 BossFang 格式：
...
> 以下指南展示如何手动将 AutoGPT 配置转换为 BossFang 格式。
