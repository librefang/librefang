...
- `"unassigned"` — 仅对无人认领的工作触发。事件里两种"无归属"的写法都能匹配：assignee 缺失，以及 assignee 为空字符串 —— `task_post` 工具和 `POST /api/tasks` 都不会规范化这个字段。
- `"any"`（默认） — 旧行为；任何 `task_posted` 事件都触发，不看 assignee。

注意 `"unassigned"` 响应的是任务**发布**时就没有归属，而不是之后变成无归属。卡住任务清扫器释放认领时不发布任何事件，所以不会唤醒这个触发器 —— 见 #6728。
