...
brew install --cask librefang       # 桌面应用 (stable)
# Beta/RC 频道:
...
|------|------|
| `--check` | 仅检查是否有新版本，不安装 |
| `--version <TAG>` | 安装指定的 GitHub Release 标签 |
| `--channel <CHANNEL>` | 临时覆盖更新频道 (`stable`、`beta`、`rc`) |

**更新频道:**

在 `config.toml` 中设置默认频道：
...
# 列出 agent
curl http://127.0.0.1:4545/api/agents

# 发送消息
curl -X POST http://127.0.0.1:4545/api/agents/<id>/message \
