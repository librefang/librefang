...
当 `dashboard_user` 和 `dashboard_pass` 同时配置时，Dashboard 会显示登录页面，用户必须输入用户名和密码才能访问。
...
| 3 | 配置文件明文 | `dashboard_pass = "my-password"`（不推荐） |
...
Vault 使用 AES-256-GCM 加密，主密钥存储在操作系统密钥链中（macOS Keychain / Windows 凭据管理器 / Linux Secret Service），或通过 `LIBREFANG_VAULT_KEY` 环境变量指定。

#### 安全特性

- **恒定时间比较**：使用 `subtle::ConstantTimeEq` 验证凭据，防止时序攻击
- **HMAC 会话令牌**：登录成功后，使用 HMAC-SHA256 从凭据派生确定性令牌，作为后续 API 请求的 Bearer token
- **无服务端会话状态**：令牌是确定性的，服务端无需存储会话，重启后令牌仍然有效
