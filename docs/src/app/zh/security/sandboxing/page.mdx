...
pub fn sandbox_command(
    ...
    // Operator-declared vars (exec_policy.allowed_env_vars): refuse only the
    // daemon's own reserved secrets (vault key, provider API keys)
    for var in operator_env_vars { /* ... */ }

    // Untrusted passthrough (hand-declared lists): full secret-name heuristic
    for var in untrusted_env_vars { /* ... */ }

    refused // names that were dropped, surfaced back to the agent
}
```
...
不在这些列表中且不在任一 allowlist 中的变量**永远不会**传递给子进程。
两个 allowlist 的信任级别不同（#6458）：operator 自己的 `exec_policy.allowed_env_vars` 可以携带形如密钥的名称（`MYTOOL_KEYRING_PASSWORD`、`FOO_API_TOKEN` 等），只有 daemon 自身的保留密钥（`LIBREFANG_VAULT_KEY`、provider API key）会被无条件拒绝。
不可信的透传列表（marketplace hand 的 `[[requires]]` / settings 名称）还会额外经过凭证词启发式过滤，在装配时与 spawn 时各检查一次。
被拒绝的名称会返回给调用方并透出到工具输出中。
...
**源码：** `librefang-runtime/src/dangerous_command.rs`

在执行任何 shell 命令之前，LibreFang 会将命令字符串与 **42 个正则表达式规则**进行匹配，这些规则涵盖 13 类危险操作。此检测在能力检查和污点检查的基础上提供额外的语义安全层。

该扫描与 `exec_policy.mode` 无关，在 `deny`、`allowlist`、`full` 下都会执行——`full` 关闭的是白名单*校验*，而不是这份黑名单。
...
| **守护进程数据库写入** | 针对守护进程自身 `librefang.db` 的写入——该文件保存了主机上所有 Agent、会话与审批记录 | `sqlite3 librefang.db "DELETE FROM …"`、`> librefang.db` |
| **系统文件覆写** | 直接写入关键 OS 路径 | `> /etc/passwd`、`> /etc/shadow`、`> /boot/` |
| **服务管理** | 停止或禁用系统服务，以及重启共享的 LibreFang 守护进程 | `systemctl stop`、`systemctl disable`、`librefang stop`、`librefang gateway restart` |
...
| **容器权限提升** | 进入容器以在 Agent 自身沙箱之外执行操作 | `docker exec` |
