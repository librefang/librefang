...
BossFang 线路协议（OFP）在 TCP 连接上使用基于 HMAC-SHA256 的 nonce
双向认证。
...
BossFang 通过 `governor` crate 使用通用信元速率算法（GCRA）实现
成本感知的 API 速率限制。
...
BossFang 支持为 Web Dashboard 配置可选的用户名/密码认证。
