...
- 相同工具调用达 3 次时**警告**
- 相同工具调用达 5 次时**阻止**；同参数连续 3 次返回相同*结果*时同样**阻止**——一直给出相同答案的工具在第四次调用就被拒绝，而不是第五次
- 总工具调用次数达 30 次时触发**熔断器**（停止 Agent）
...
**平台相关说明**：
- **Linux**：需要 StatusNotifierItem 支持的系统托盘（如 GNOME 上的 AppIndicator Support 扩展）
- **macOS**：开箱即用
