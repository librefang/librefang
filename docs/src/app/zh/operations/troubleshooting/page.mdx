...
BossFang 使用 `tracing` 进行结构化日志记录。通过环境变量设置日志级别：
...
**原因**：通常是插件声明的 runtime 在这台机器上没装。hook 会以 `LauncherNotFound` 失败，BossFang 打一条 warning 日志，回退到默认 context engine —— 所以不会崩、表面上没变化、但召回的 memories 永远不回来。
...
3. 看 BossFang 日志里提到你 hook 的 `Timeout` 条目。

### Hook 跑了但 memories 没出来

**原因**：输出不是合法 JSON，或者 stdout 最后一行不是 JSON 响应。

BossFang 从 stdout 最后一行往前扫，取第一个能 parse 的 JSON 行。在 JSON **前面**打的 debug log 不影响；**后面**打的也不影响；但如果最后一行是 `done.` 或者 traceback 之类的东西，响应就丢了。
...
### BossFang 和 OpenClaw 有什么区别？

| 维度 | BossFang | OpenClaw |
|--------|----------|----------|
| 编程语言 | Rust | Python |
| 通道数 | 40 | 38 |
| 技能数 | 60 | 57 |
...
BossFang 可以导入 OpenFang 配置：`librefang migrate --from openfang`

BossFang 也可以导入 OpenClaw 配置：`librefang migrate --from openclaw`
