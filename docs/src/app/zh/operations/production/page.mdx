...
运行时镜像存在大量历史遗留问题（截至 2026-08-01 有 10 个 critical 和 95 个 high，见 issue #6694），因此一旦启用强制执行，`main` 和每次发布在第一次运行时就会失败。
阈值只存在于一个地方 —— `.github/actions/trivy-image-scan/action.yml` 的 `fail-on` 输入 —— 且两个工作流都不覆盖它，因此维护者只需修改这一个默认值即可同时启用两者：
