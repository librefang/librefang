...
Migrating to BossFang from other systems.

---

## Overview

BossFang provides the following migration support:
...
OpenFang and BossFang share the same directory structure and config format (BossFang is a community fork of OpenFang). Migration is a straightforward recursive copy of `~/.openfang` → `~/.librefang` with content rewriting in `.toml` and `.env` files.
...
> The guide below shows how to manually convert LangChain configurations to BossFang format.

### Agent Configuration Conversion

Convert LangChain agent configuration to BossFang format:
...
> The guide below shows how to manually convert AutoGPT configurations to BossFang format.
