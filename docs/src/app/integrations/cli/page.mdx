# BossFang CLI Reference

> **Sub-pages:** [Command Reference](/integrations/cli/commands) | [Examples & Guides](/integrations/cli/examples)

Complete command-line reference for `librefang`, the CLI tool for the BossFang Agent OS.

## Overview

The `librefang` binary is the primary interface for managing the BossFang Agent OS. It supports two modes of operation:
...
<Note>
AUR account registration is temporarily unavailable.
BossFang currently publishes signed Arch packages through its official pacman repository.
</Note>

Import and locally trust the BossFang package-signing key:
...
Install the CLI, daemon, HTTP API, and web dashboard:

```bash
sudo pacman -Syu librefang-bin
```

The standalone Desktop package does not require the CLI package and is documented separately in [Getting Started](/getting-started#desktop-app).
See the [Arch repository documentation](https://github.com/librefang/librefang/blob/main/packaging/arch-repo/README.md) for package details and aarch64 support.
...
Initialize the BossFang workspace. Creates `~/.librefang/` with subdirectories (`data/`, `workspaces/agents/`) and a default `config.toml`.
...
Start the BossFang daemon (kernel + API server).
...
- Boots the BossFang kernel (loads config, initializes SQLite database, loads agents, connects MCP servers, starts background tasks).
...
Run diagnostic checks on the BossFang installation.
...
1. **BossFang directory** -- `~/.librefang/` exists
...
BossFang supports Apple-style update channels. Set a persistent default in `config.toml`:
...
BossFang loads `~/.librefang/.env` into the process environment on every CLI invocation. System environment variables take priority over `.env` values.
