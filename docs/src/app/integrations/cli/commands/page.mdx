...
- Templates are discovered from: `~/.librefang/agent-types/` (pre-installed by the registry sync, plus any created via the dashboard, the `agent_type_create` tool, or `save-as-agent-type`), and `LIBREFANG_AGENTS_DIR` (env override).
- Each template is a `<name>.toml` manifest file.
