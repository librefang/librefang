...
- Templates are discovered from: `~/.librefang/workspaces/agents/` (pre-installed by the registry sync) and `LIBREFANG_AGENTS_DIR` (env override).
- Each template is a directory containing an `agent.toml` manifest.
  A flat `<name>.toml` dropped into one of those directories is skipped, because discovery filters on `is_dir()` before it looks at any file.
- Agent types authored through the dashboard, the `agent_type_create` tool or `save-as-agent-type` are written to `~/.librefang/agent-types/<name>.toml` instead (#7758), and this picker does not scan that directory.
  The daemon does resolve them — `agent_spawn` and a workflow step's `{"type": ...}` search `agent-types/` first — so spawn one of those through the daemon rather than through `agent new`.
...
Unlike `agent chat` this is a single request, meant for scripts and automation.

```
librefang message <AGENT> <TEXT> [--json] [--incognito] [--session-id <UUID>]
```
...
|---|---|---|
| `--json` | off | Print the raw JSON response instead of just the reply text. |
| `--incognito` | off | Suppress session writes and memory writes for this turn; memory reads still happen. |
| `--session-id <UUID>` | *(canonical session)* | Address one specific conversation. Also accepted as `--session`. |

**Session targeting:**

Without `--session-id` the daemon resolves the agent's *canonical* session — one conversation per agent.
That is fine for a personal agent and wrong for an agent serving several unrelated people: every caller's turns land in the same history, so each visitor's text is in the model context of the next visitor's turn, and the shared transcript is re-sent (and billed) on every turn until it hits the context limit.

Pass a per-conversation UUID and each conversation gets its own history.
The same UUID continues a conversation; a different UUID starts an isolated one.
This is the CLI equivalent of the `session_id` field on `POST /api/agents/{id}/message`.

`--incognito` is not a substitute: it suppresses session writes entirely, so continuity *inside* one conversation is lost too.

**Interaction with `session_mode`:**

An explicit `--session-id` sits at the top of the kernel's session-resolution ladder, so it wins over the agent's `session_mode` in `agent.toml` in both directions:

| `session_mode` | Without `--session-id` | With `--session-id` |
|---|---|---|
| `persistent` (default) | Every call reuses the agent's one canonical session. | The named session is used instead; the canonical session is left untouched. |
| `new` | Every call mints a fresh throwaway session. | The named session is reused across calls, so continuity is restored for that conversation. |

The session must belong to the agent being addressed.
A UUID owned by another agent is rejected rather than read, so one agent's id can never surface another's history.
A malformed UUID is rejected by the CLI before any request is sent.

**Examples:**

```bash
# Canonical session — unchanged behavior
librefang message coder "Fix the bug in main.rs"

# Two isolated conversations with the same agent
librefang message sales "My customer code is PINE-77." --session-id 11111111-1111-4111-8111-111111111111
librefang message sales "What is my customer code?"    --session-id 22222222-2222-4222-8222-222222222222

# Continue the first conversation
librefang message sales "And how many licences does that cover?" --session-id 11111111-1111-4111-8111-111111111111
```
...
| `--cooldown <SECS>` | none | Minimum seconds between consecutive fires of the same window. For task-board, memory-key and agent-lifecycle patterns the window is per subject, so a different task, key or agent fires immediately. |
...
# Fire on agent termination, max 5 times, 60s cooldown per terminated agent.
# The cooldown is per subject here, so five agents dying at once produce five
# fires and exhaust --max-fires immediately; raise it if you expect crash waves.
...
| `--cooldown <SECS>` | New cooldown in seconds, applied per window (per subject for task-board, memory-key and agent-lifecycle patterns). |
...
#### `--system` (macOS): start at boot instead of at login

All three defaults above register a *per-user* service, which starts once that user logs in.
A Mac that reboots and stops at the login window — or at the FileVault unlock screen — therefore does not start the daemon, which matters for an always-on install.

`--system` writes a LaunchDaemon to `/Library/LaunchDaemons/ai.librefang.daemon.plist` instead.
launchd loads that directory at boot, before any login.

```bash
sudo librefang service install --system
```

It requires root, and it requires being invoked through `sudo` rather than from a root login: the daemon has to run as a real account, and `SUDO_USER` is the only thing that identifies which one.
The generated job therefore sets `UserName` to your account, points `HOME` at your home directory and `LIBREFANG_HOME` at `~/.librefang`, and the install chowns that state directory and its `daemon.log` to you — launchd opens the log before dropping privileges, so without that step the daemon could not write to its own log.

Remove it with the same flag:

```bash
sudo librefang service uninstall --system
```

`librefang service status` reports both the per-user LaunchAgent and the boot-time LaunchDaemon, with no flag needed.

`--system` is macOS-only. On NixOS use the `services.librefang` module; on other Linux distributions either write a system unit to `/etc/systemd/system/` or keep the user service plus `loginctl enable-linger`.
...
|---|---|---|
| `librefang models` | LLM model catalog. | `list`, `aliases`, `providers`, `set`, `connect` |
...
| `librefang message <agent> <text>` | — | Send one-shot message; print the reply. `--json`, `--incognito`, `--session-id <UUID>` — see [librefang message](#librefang-message). |
