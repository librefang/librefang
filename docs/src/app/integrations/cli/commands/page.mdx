...
Detailed reference for all BossFang CLI command groups: agents, hands, workflows, triggers, skills, channels, and configuration.
...
Manage the system boot service so BossFang starts automatically on login or reboot.
...
- **Windows:** Adds a `BossFang` entry to `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`.
...
| `librefang uninstall` | Remove BossFang. | `--confirm`, `--keep-config` |
