...
Per-channel `output_format`, `dm_policy`, `group_policy`, and `threading` overrides are exposed on `[[sidecar_channels]]` as of #6445. Each is optional — an unset field inherits the bridge default and does not materialize a policy you did not write. Agent-level `[channel_overrides]` still takes precedence over these per-channel values.
