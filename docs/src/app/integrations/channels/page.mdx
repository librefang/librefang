...
# equivalent on the agent (`agent.toml [channel_overrides]`).
# DM / group gating is kernel-side via `[[users]]` channel bindings; among the sidecars only WhatsApp reads a `WHATSAPP_DM_POLICY` env var.
