...
BossFang ships an [Agent Client Protocol](https://agentclientprotocol.com/) adapter so editors like Zed, VS Code, and JetBrains can embed a BossFang agent natively. The editor renders approval modals, streams prompt deltas, and routes file references through its own UI rather than the BossFang dashboard.
...
BossFang exposes the ACP server as a CLI subcommand:
...
BossFang automatically selects between **in-process** and **daemon-attached** depending on whether a daemon is already running on the machine.
...
Zed exposes ACP-compatible agents through its `agent_servers` configuration. Add BossFang as an external agent in `~/.config/zed/settings.json`:
...
| `session/new` | client → agent | Mints a fresh ACP `SessionId`, allocates a backing BossFang session. |
| `session/load` | client → agent | Maps an existing ACP `SessionId` onto a fresh BossFang session. History replay across daemon restarts is a follow-up. |
...
1. Filters by the BossFang `SessionId` it owns.
...
- `session/load` / `session/resume` honour the supplied ACP session id by deriving the BossFang `SessionId` deterministically (`Uuid::new_v5` keyed on the ACP id), so a reconnecting editor rejoins the same kernel-side session and picks up its persisted history without an explicit replay step.
