...
| **Channels** | | |
| GET | `/api/channels` | List configured + discoverable channels (sidecar-only) |
| GET | `/api/channels/registry` | Channel metadata from the registry sync |
| POST | `/api/channels/sidecar/{name}/configure` | Save a `[[sidecar_channels]]` entry (schema-driven) |
| POST | `/api/channels/reload` | Hot-reload channels from disk |
| **Agent Types** | | |
| GET | `/api/agent-types` | List agent types (`/api/templates` is a deprecated alias) |
| GET | `/api/agent-types/{name}` | Get agent type (`/api/templates/{name}` is a deprecated alias) |
| **Sessions** | | |
...
| GET | `/api/usage/export` | Stream usage events as CSV for archival |
| **Budget** | | |
...
| GET | `/api/agent-types/{name}/toml` | Raw agent-type TOML (for editor use); `/api/templates/{name}/toml` is a deprecated alias |
| **Goals & Inbox** | | |
