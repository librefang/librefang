...
| **API meta / introspection** | | |
| GET | `/api/versions` | API version discovery (versioned vs unversioned route map) |
| GET | `/api/profiles` | List tool profiles (named tool sets used in templates) |
| GET | `/api/profiles/{name}` | Get one tool profile by name |
