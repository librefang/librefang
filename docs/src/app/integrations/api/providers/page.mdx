...
`vision_support` is the provenance of `supports_vision`: `supported` / `unsupported` when a source declared it, `unknown` when the flag was inferred from the model's name.
Only `unsupported` causes image attachments to be stripped from a request — an `unknown` model keeps them, so a guess never silently blinds a model that can see.
