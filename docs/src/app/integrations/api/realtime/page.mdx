...
BossFang exposes an OpenAI-compatible API for drop-in integration with tools that support the OpenAI API format (Cursor, Continue, Open WebUI, etc.).
...
**Model resolution** (the `model` field maps to an BossFang agent):
