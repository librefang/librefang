...
- `"unassigned"` — fire only for work nobody owns. Matches both spellings that reach the event: an absent assignee and an empty one, since neither the `task_post` tool nor `POST /api/tasks` normalises the field.
- `"any"` (default) — legacy behaviour; fires on every `task_posted` event regardless of assignee.

Note that `"unassigned"` reacts to a task being *posted* without an owner, not to one becoming unowned later. A claim released by the stuck-task sweeper publishes no event, so it does not wake this trigger — see #6728.
