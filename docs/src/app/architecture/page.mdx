...
LibreFang is organized as a Cargo workspace with 16 crates (15 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).
...
librefang-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    ...
    +-- librefang-channels     42 channel adapters, bridge, formatter, rate limiter
    ...
    +-- librefang-hands        Autonomous Hands system (14 built-in hand agents)
    |
...
| **librefang-hands** | Autonomous Hands system. 14 built-in hand agents that operate independently with specialized domain expertise: analyst, browser, coder, comms, files, researcher, scheduler, sysadmin, analytics, apitester, devops, linkedin, reddit, strategist, trader. Each hand exposes a structured tool surface and can be activated/deactivated at runtime via the `hand_activate`/`hand_deactivate` tools. |
...
5. Initialize model catalog
   - Build ModelCatalog with 130+ builtin models, 20+ aliases, 28 providers
   - Load additional provider definitions from ~/.librefang/providers/*.toml (dynamic catalog)
...
Three native driver implementations cover all 28 providers with 130+ models:
...
- **28 providers** with authentication status detection
- **Dynamic catalog loading**: Additional provider and model definitions can be placed in `~/.librefang/providers/*.toml` and are merged at boot time, allowing user-defined providers without recompilation
...
The channel system (`librefang-channels`) provides 42 adapters for messaging platform integration.
...
| **Wave 4 (10)** | Mumble, DingTalk, QQ, WeCom (WeChat Work), Discourse, Gitter, Ntfy, Gotify, Webhook, LinkedIn |

> Note: Webhook adapter in Wave 4 supports MQTT-over-webhook payloads via the sidecar_channels mechanism. Total: 42 adapters.
...
| Category | Tools |
|----------|-------|
| **File & Memory (12)** | file_read, file_write, file_list, file_delete, file_move, memory_store, memory_recall, memory_search, memory_delete, task_post, task_claim, task_complete |
| **Web & Search (6)** | web_search, web_fetch, web_read, image_search, news_search, location_get |
| **Code & Shell (5)** | code_run, shell_exec, code_format, apply_patch, docker_exec |
| **Agent Ops (5)** | agent_spawn, agent_message, agent_list, agent_kill, system_time |
...
| **Media & AI (5)** | media_describe, media_transcribe, image_generate, text_to_speech, speech_to_text |
| **Hands Control (4)** | hand_list, hand_activate, hand_status, hand_deactivate |
| **Scheduling & Comms (6)** | cron_create, cron_list, cron_cancel, channel_send, schedule_create, schedule_list |
...
The Hands system (`librefang-hands`) provides 14 built-in autonomous hand agents, each specialized for a particular domain.
...
| `comms` | Communication channel management |
| `files` | File system operations and management |
| `researcher` | Deep research and information gathering |
| `scheduler` | Task scheduling and cron management |
