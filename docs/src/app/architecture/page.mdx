...
BossFang is organized as a Cargo workspace with 15 crates (14 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).
...
| **librefang-channels** | Channel bridge layer with the in-process adapter set + the sidecar mechanism. Each in-process adapter implements the `ChannelAdapter` trait. Includes: Discord, Slack, WhatsApp, Signal, Matrix, Email, Webhook, Sidecar, Teams, Mattermost, IRC, Google Chat, Twitch, Rocket.Chat, Zulip, XMPP, LINE, Reddit, Bluesky, Feishu, Revolt, Nextcloud, Guilded, Keybase, Threema, Webex, Pumble, Flock, Twist, Mumble, DingTalk, QQ, WeCom (WeChat Work), Gitter. Telegram / ntfy / Gotify / Mastodon ship as Python sidecars via the `librefang-sdk` package. Features: `AgentRouter` for message routing, `BridgeManager` for lifecycle coordination, `ChannelRateLimiter` (per-user DashMap tracking), `formatter.rs` (Markdown to TelegramHTML/SlackMrkdwn/PlainText), `ChannelOverrides` (model/system_prompt/dm_policy/group_policy/rate_limit/threading/output_format), DM/group policy enforcement, sidecar_channels mechanism for auxiliary channel wiring. |
| **librefang-wire** | BossFang Protocol (OFP) for peer-to-peer agent communication. JSON-framed messages over TCP with HMAC-SHA256 mutual authentication (nonce + constant-time verify via `subtle`). `PeerNode` listens for connections and manages peers. `PeerRegistry` tracks known remote peers and their agents. |
...
BossFang implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
| **Wave 2 (6)** | LINE, Reddit, Mastodon, Bluesky, Feishu, Revolt |
| **Wave 3 (8)** | Nextcloud, Guilded, Keybase, Threema, Webex, Pumble, Flock, Twist |
| **Wave 4 (9)** | Mumble, DingTalk, QQ, WeCom (WeChat Work), Gitter, Ntfy, Gotify, Webhook, Sidecar |

> The Sidecar adapter enables external processes written in any language to act as channel adapters via JSON-RPC over stdin/stdout. Telegram / ntfy / Gotify / Mastodon ship as first-party Python sidecars in `librefang-sdk`.
...
- **FangHub**: Native BossFang marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-BossFang tool name mappings in `tool_compat.rs`.
...
BossFang implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes BossFang's 60 built-in tools via the MCP protocol. Enables external tools to use BossFang as a tool provider.
...
The BossFang Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`librefang-desktop`) wraps the full BossFang stack in a native Tauri 2.0 application.
