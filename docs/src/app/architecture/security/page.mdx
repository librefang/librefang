...
`Block` has a second, earlier trigger that the call counter alone cannot see: once the same parameters have produced the same result `outcome_block_threshold` (3) times, the next call with those parameters is refused without running.
A call that *declares* itself a poll — an explicit `"poll": true` parameter, or `shell_exec` running a recognised wait-loop command such as `docker ps` or `systemctl status` — has that threshold multiplied by `poll_multiplier` and is paced with a suggested backoff delay on its result rather than cut off, because an unchanged answer is what "not ready yet" looks like.
The broader test `check` uses for the call-count thresholds — any parameters mentioning `status` / `poll` / `wait`, whatever the tool — deliberately does not apply here: it would put "wait roughly 5s before checking again" on an ordinary `task_list {"status": "pending"}` and triple that call's outcome budget.
The backoff delay is keyed on the call rather than on the answer, so a declared poll gets the next step of the schedule on every repeat, including one whose result finally changed.
...
| `outcome_warn_threshold` | 2 | Identical results for identical parameters before an advisory |
| `outcome_block_threshold` | 3 | Identical results before the next such call is blocked |
| `poll_multiplier` | 3 | Multiplies the warn and block thresholds above. The call-count pair uses a broad poll test; the outcome pair and the backoff advisory use a narrow one (explicit `"poll": true`, or a recognised `shell_exec` wait-loop command). The circuit breaker is unaffected |
