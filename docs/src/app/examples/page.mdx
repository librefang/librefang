...
This page is meant to be executable, not aspirational. Every example below follows the current CLI, REST API, and config shapes in the source tree.
...
## What LibreFang Actually Does

Before looking at examples, it helps to separate product capability from config syntax.
...
So the right job for this page is not only to list “industry scenarios”, and not only to list raw commands. It should answer two questions:

1. What can LibreFang be used for?
2. How do those capabilities map to the current codebase and commands?

The examples below are written with that model in mind.
