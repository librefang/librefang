...
When BossFang completes an OAuth flow against an MCP server, the URL it sends the
...
BossFang pins the token-exchange target against the host the operator typed in
