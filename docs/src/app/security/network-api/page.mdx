...
The BossFang Wire Protocol (OFP) authenticates every TCP connection in
two layers:
...
BossFang uses the Generic Cell Rate Algorithm (GCRA) for cost-aware API
...
BossFang provides two health endpoints with different information levels.
...
BossFang supports optional username/password authentication for the web dashboard.
