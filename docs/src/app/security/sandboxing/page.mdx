...
against **42 regular-expression patterns** grouped into 13 categories of
dangerous operations.  This adds a semantic safety layer on top of the
capability and taint checks.

The screen is independent of `exec_policy.mode` and runs under `deny`,
`allowlist`, and `full` alike -- `full` disables allowlist *validation*, not
this denylist.
