...
LibreFang implements both the **Model Context Protocol (MCP)** and **Agent-to-Agent (A2A)** protocol, enabling deep interoperability with external tools, IDEs, and other agent frameworks.
...
- [Part 1: MCP (Model Context Protocol)](#part-1-mcp-model-context-protocol)
  ...
  - [MCP Server -- Exposing LibreFang via MCP](#mcp-server)
...
The Model Context Protocol (MCP) is a JSON-RPC 2.0 based protocol that standardizes how LLM applications discover and invoke tools. LibreFang supports MCP in both directions:

- **As a client**: LibreFang connects to external MCP servers (GitHub, filesystem, databases, Puppeteer, etc.) and makes their tools available to all agents.
- **As a server**: LibreFang exposes its own agents as MCP tools, so IDEs like Cursor, VS Code, and Claude Desktop can call LibreFang agents directly.

LibreFang implements MCP protocol version `2024-11-05`.

**Source files:**
- Client: `crates/librefang-runtime/src/mcp.rs`
- Server handler: `crates/librefang-runtime/src/mcp_server.rs`
- CLI server: `crates/librefang-cli/src/mcp.rs`
- Config types: `crates/librefang-types/src/config.rs` (`McpServerConfigEntry`, `McpTransportEntry`)

---

### MCP Client

The MCP client (`McpConnection` in `librefang-runtime`) allows LibreFang to connect to any MCP-compatible server and use its tools as if they were built-in.
...
Helper functions (exported from `librefang_runtime::mcp`):
...
LibreFang can also act as an MCP server, exposing its agents as callable tools to external MCP clients.

#### How It Works

Each LibreFang agent becomes an MCP tool named `librefang_agent_{name}` (with hyphens replaced by underscores). The tool accepts a single `message` string parameter and returns the agent's response.

For example, an agent named `code-reviewer` becomes the MCP tool `librefang_agent_code_reviewer`.

#### CLI: `librefang mcp`

The primary way to run the MCP server is the `librefang mcp` command, which starts a stdio-based MCP server:
...
This command:
1. Checks if an LibreFang daemon is running (via `find_daemon()`)
...
- `McpBackend::Daemon` -- forwards requests to a running LibreFang daemon via HTTP
- `McpBackend::InProcess` -- boots a full kernel when no daemon is available

#### HTTP MCP Endpoint

LibreFang also exposes an MCP endpoint over HTTP at `POST /mcp`. Unlike the stdio server (which only exposes agents), the HTTP endpoint exposes the full tool set (built-in + skills + MCP tools) and executes tools via the kernel's `execute_tool()` pipeline. This means the HTTP MCP endpoint supports:
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "librefang_agent_code_reviewer",
    "arguments": {
...
After configuration, all LibreFang agents appear as tools in the IDE. For example, you can ask Claude Desktop to "use the librefang code-reviewer agent to review this file."
...
LibreFang implements A2A in both directions:
...
**Source files:**
- Protocol types and logic: `crates/librefang-runtime/src/a2a.rs`
- API routes: `crates/librefang-api/src/routes.rs`
- Config types: `crates/librefang-types/src/config.rs` (`A2aConfig`, `ExternalAgent`)
...
pub struct AgentCapabilities {
    pub streaming: bool,                 // true -- LibreFang supports streaming
...
**AgentSkill** (not the same as LibreFang skills -- these are A2A capability descriptors):

```rust
pub struct AgentSkill {
    pub id: String,           // matches the LibreFang tool name
...
Agent Cards are built from LibreFang agent manifests via `build_agent_card()`. Each tool in the agent's capability list becomes an A2A skill descriptor. Example card:
...
LibreFang serves A2A requests through the REST API. The server-side implementation involves:
