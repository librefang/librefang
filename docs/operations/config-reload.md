...
| `task_board` | N | Shared task-board knobs. The sweeper re-reads `claim_ttl_secs` / `sweep_interval_secs` / `max_retries` on every tick, and `assignee_wake` is read per task post — all four take effect without a restart. |
