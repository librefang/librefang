...
3. The configured `[reload] mode` (`off` / `restart` / `hot` / `hybrid`) decides whether any of that is applied — see `should_store_config`, which is the gate `reload_config` actually uses.
   ...
   A plan whose only change is **R** counts as a change too: the new value is stored, so `GET /api/config` and anything that reads it live from the config see the file's value, while the boot-captured consumer it configures keeps the value boot wired until the restart. The response still reports `restart_required` for it.
   The response distinguishes applied from previewed: `config_applied` is `false` and `hot_actions_applied` is empty whenever the mode withheld the swap, with the reason in `warnings` and `status` reported as `partial`.
...
| `registry` | R/N | Registry sync config. `cache_ttl_secs` / `registry_mirror` / `registry_host` are **R**: they are read when the checkout is set up. `auto_sync` is **N**: the 24 h catalog task calls `config_snapshot()` at the top of each tick and passes the value into `sync_catalog_to`, so flipping it off freezes `~/.librefang/registry/` from the next tick on, with no restart. Splitting the section this way is what makes that true — while the whole section was classified R, a registry-only reload was reported restart-required even though the catalog task picks the new `auto_sync` up from the live config on its next tick (and before restart-required-only plans were stored too, the same classification discarded the new config outright, so the task kept reading the old value until the daemon restarted). Boot's own sync pass has of course already run by reload time, so `auto_sync = false` written *before* a start is still what prevents the boot-time fast-forward. |
...
| `tts.output_format` | N | Re-read per turn by `tool_text_to_speech`, through `LoopOptions.tts_config`. Carved out of the restart-required half deliberately: the tool reads it from the live `[tts]` per call, so **N** is the honest classification — classified R it would report a restart for an edit that is already effective on the next call (and, before restart-required-only plans were stored too, the swap itself was discarded and the value kept resolving to its boot-time value — #8272). |
