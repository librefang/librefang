...
- **Rich Markdown (primary).** Telegram parses the GFM itself, so tables, task lists, `_italic_`, `~~strikethrough~~`, `||spoiler||` and nested emphasis all work, and the size limit is 32768 characters rather than 4096.
  ...
  The cost is that a `<` inside a code sample renders as a literal `&lt;`, since Markdown does not decode entities inside code; `InputRichMessage.blocks`, where a preformatted block's text is a plain string Telegram never parses, is the fix for that and is tracked separately.
