...
- **Rich Markdown (primary).** Telegram parses the GFM itself, so tables, task lists, `_italic_`, `~~strikethrough~~`, `||spoiler||` and nested emphasis all work, and the size limit is 32768 characters rather than 4096.
  `sanitize_rich_markdown` runs first because Rich Markdown may contain arbitrary HTML: **every `<` is escaped, unconditionally**, so quoted untrusted content cannot render itself a `<tg-button>` whose `callback_data` would come back as a genuine button press.
  There are no exemptions for code spans, fenced blocks or well-formed tags: each exemption has to decide where a CommonMark construct ends, and five rounds of review found five inputs where that decision disagreed with a real parser and copied a live button through — two of them introduced by the fix for the previous one.
  The escape is a backslash rather than `&lt;`, because the Bot API documents that mechanism for the `markdown` field by example (`\#hashtag` in its own Rich Markdown sample) and says nothing about entity decoding there.
  The cost is that the escapes also land inside code spans and fenced blocks, where Markdown does not process them, so `Vec<String>` in a fence reads `Vec\<String>`; and that every Rich HTML construct is lost, since they all start with `<` — `<u>`, `<ins>`, `<sub>`, `<sup>`, `<br>`, `<details>`, `<aside>`, `<a name>`, `<tg-map>`, `<tg-collage>`, `<tg-slideshow>`, `<tg-emoji>`, `<tg-time>`.
  `InputRichMessage.blocks`, where a preformatted block's text is a plain string Telegram never parses, is the fix for both and is tracked separately.
  `[label](destination)` and `[label]: destination` are checked against the same scheme allowlist as the legacy sanitiser and `![](url)` is escaped so it stays inert text, but both are **best-effort**: locating a Markdown link exactly has the same problem as everything above. The guarantee this pass makes is the HTML one.
  Two escape hatches are known and left open, since closing either means reimplementing the parser: a link label longer than 999 characters is not scanned (CommonMark caps *reference* labels there but places no cap on inline link text), and the scanner works to a per-message budget that about a kilobyte of `("[" * 998 + "]")` exhausts, turning the check off for the rest of the message.
...
- **HTML sanitiser (fallback only).** Allowlist of `b`, `i`, `u`, `s`, `em`, `strong`, `a`, `code`, `pre`, `blockquote`, `tg-spoiler`, `tg-emoji` — matches Telegram's documented HTML subset.
  ...
  `rich_sanitize` enforces the same allowlist on the primary path, escaping the tag or link instead of dropping it.
