...
- **Rich blocks (preferred).** `rich_blocks::markdown_to_blocks` parses the agent's Markdown with `pulldown-cmark` and builds `InputRichBlock` values, so quoted markup arrives as a *string value* rather than as anything Telegram interprets, and a button can only exist as a `RichTextButton` object the converter never constructs.
  ...
  `||spoiler||` and `==highlight==` are Telegram's own Rich Markdown syntax rather than CommonMark, so they are found by scanning assembled inline runs. The pairing rules are Telegram's, read off the live parser by sending each shape as `rich_message.markdown` and diffing the parse it echoes back: an opener may not be followed by a space, a closer may not be preceded by one, the content may not be empty, a pair does not cross a line break, a delimiter that was not *written* as one is text, and the two nest in either order. Both spellings that put the character on the screen without writing it — an escape (`\|`) and a character reference (`&#124;`) — are honoured, and both are found by comparing each text event with the source range it came from rather than by guessing from the shape of the event stream. Guessing was tried and was wrong twice: reading escapes off "two text events in a row" broke on entities and on an escape at the start of a run, and the rule that replaced it removed an accidental protection against references that nobody had noticed was there. "Space" means ASCII — Telegram treats NBSP and the other Unicode spaces as content — and a rejected closer shifts by one character rather than by one delimiter, which is what makes `=====` a highlight over a single `=`.
  The battery of probed shapes is recorded in `src/format/telegram_oracle.json` and diffed by a test, so the claim is checkable rather than asserted: of 178 recorded forms 140 match byte for byte, and the other 38 are listed with a reason in `DIVERGENCES`. Twenty-nine of those are not content at all — Telegram writes the same characters with the same styles down differently, ordering styles that cover identical text its own way, normalising a bare host with a trailing slash, labelling list items, filling in table chrome — and the test *checks* that rather than taking the label's word for it. The remaining nine are named: three entity kinds Telegram detects and this converter does not, a tab it folds into a space, a relative link where GFM and Telegram disagree about what the text even is, a table row where GFM reads the pipes as cell separators, a backslash before a delimiter, where Telegram's own handling is neither CommonMark's nor self-consistent — it drops the backslash in `\||секрет||` and keeps it in `||секрет\||b||` — and a named character reference, which Telegram leaves as written while CommonMark decodes it.
  Nesting order is deliberately not modelled. It is not a property of the styles: `**==a==**` comes back with the highlight outside the bold and `==**a**==` comes back the other way round. An earlier version invented a fixed ranking for it, wrote it into this document as Telegram's, and was wrong on four of the six shapes it covered.
  Any count of the kind above describes the *sample*, not the converter: this document once read "22 of 23 probed shapes match exactly", and a wider battery then found five classes of divergence the sample had missed. The number is worth stating only next to the recording it came from.
  The scan cannot reach code — an inline span is a separate node by then, and a fenced block never passes through it.
