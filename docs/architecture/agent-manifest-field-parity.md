...
`AgentManifest` (`crates/librefang-types/src/agent.rs`) has 58 top-level fields.
...
| 4 | `author` | yes | no | |
| 5 | `module` | yes | no | Path-escape validated on every write (`validate_manifest_module_path`, #3533) |
| 6 | `schedule` | yes | no | Also `PATCH {"schedule"}`, which additionally restarts the background loop (#4984) |
| 7 | `session_mode` | yes | no | Takes effect on respawn |
| 8 | `model` | yes | no | Also `PATCH {"model"}` / `PATCH /config` |
| 9 | `fallback_models` | yes | no | Also `PATCH /config` |
| 10 | `resources` | yes | no | Quota cache refreshed in place |
| 11 | `priority` | yes | no | |
| 12 | `capabilities` | yes | no | Re-granted in place |
| 13 | `profile` | yes | no | |
| 14 | `tools` (per-tool params) | yes | no | |
| 15 | `skills` | yes | **yes** | `PUT /skills`; TUI detail, `s` |
| 16 | `skills_disabled` | yes | no | `PUT /skills` clears it as a side effect |
| 17 | `mcp_servers` | yes | **yes** | `PUT /mcp_servers`; TUI detail, `m` |
| 18 | `channels` | yes | **yes — added here** | `PUT /channels`; TUI detail, `n` |
| 19 | `mcp_disabled` | yes | no | |
| 20 | `metadata` | yes | no | |
| 21 | `tags` | **no** | no | The one API gap. In flight in #7866 — see *Known gaps* |
| 22 | `routing` | yes | no | |
| 23 | `autonomous` | yes | no | |
| 24 | `pinned_model` | yes | no | |
| 25 | `workspace` | yes, with a guard | no | Preserved when the incoming manifest omits it — see *Deliberately not writable* |
| 26 | `generate_identity_files` | yes | no | Read at spawn only |
| 27 | `workspaces` | yes | no | |
| 28 | `exec_policy` | yes | no | |
| 29 | `tool_allowlist` | yes | no | Also `PUT /tools` |
| 30 | `tool_blocklist` | yes | no | Also `PUT /tools` |
| 31 | `tools_disabled` | yes | no | `PUT /tools` cannot set it, and clears it — in flight in #7866 |
| 32 | `response_format` | yes | no | |
| 33 | `enabled` | yes | no | |
| 34 | `allowed_plugins` | yes | no | |
| 35 | `inherit_parent_context` | yes | no | |
| 36 | `thinking` | yes | no | |
| 37 | `context_injection` | yes | no | |
| 38 | `is_hand` | yes — arguably should not be | no | Provenance, and it is read. See *Known gaps* |
| 39 | `web_search_augmentation` | yes | no | Also `PATCH /config` |
| 40 | `auto_dream_enabled` | yes | no | |
| 41 | `auto_dream_min_hours` | yes | no | |
| 42 | `auto_dream_min_sessions` | yes | no | |
| 43 | `show_progress` | yes | no | |
| 44 | `auto_evolve` | yes | no | Also `PATCH {"auto_evolve"}` |
| 45 | `channel_overrides` | yes | no | |
| 46 | `max_history_messages` | yes | no | |
| 47 | `max_concurrent_invocations` | yes | no | Cap is sized once per spawn; needs kill + respawn |
| 48 | `assignee_wake` | yes | no | |
| 49 | `cache_context` | yes | no | |
| 50 | `tool_exec_backend` | yes | no | Picked up on respawn |
| 51 | `skill_workshop` | yes | no | |
| 52 | `proactive_memory` | yes | no | |
| 53 | `compaction` | yes | no | |
| 54 | `context_engine` | yes | no | Picked up on respawn |
| 55 | `rl_export` | yes | no | |
| 56 | `triggers` | yes | no | An entry with no `pattern` used to make the whole file unwritable — see below |
| 57 | `reconcile_orphans` | yes | no | |
| 58 | `async_tasks` | yes | no | |
...
**The TUI can edit three fields of 58 on a running agent.**
...
Today the whole-manifest path is write-only over HTTP: a client can `PATCH` 58 fields but cannot fetch the 58 it is about to overwrite.
...
TOML has no null.
`persist_full_manifest_at` serializes the entire `AgentManifest` with `toml::to_string_pretty`, so one such entry failed the write for all 58 fields at once — and the failure was a swallowed log, so `update_manifest` still answered `200 OK`.
