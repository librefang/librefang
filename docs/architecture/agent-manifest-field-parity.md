...
`AgentManifest` (`crates/librefang-types/src/agent.rs`) has 60 top-level fields.
...
The field count is guarded by `librefang_types` test `the_populated_fixture_covers_every_serialized_manifest_key`, so a 60th field cannot be added without someone updating this page.
...
| 6 | `source_template` | yes | no | Name of the agent type the agent was spawned from (#8018); set by `POST /api/agents` only — `step_agent.rs` and `assistant_routing.rs` instantiate from a template without recording it |
| 7 | `module` | yes | no | Path-escape validated on every write (`validate_manifest_module_path`, #3533) |
| 8 | `schedule` | yes | no | Also `PATCH {"schedule"}`, which additionally restarts the background loop (#4984) |
| 9 | `session_mode` | yes | no | Takes effect on respawn |
| 10 | `model` | yes | no | Also `PATCH {"model"}` / `PATCH /config` |
| 11 | `fallback_models` | yes | no | Also `PATCH /config` |
| 12 | `resources` | yes | no | Quota cache refreshed in place |
| 13 | `priority` | yes | no | |
| 14 | `capabilities` | yes | no | Re-granted in place |
| 15 | `profile` | yes | no | |
| 16 | `tools` (per-tool params) | yes | no | |
| 17 | `skills` | yes | **yes** | `PUT /skills`; TUI detail, `s` |
| 18 | `skills_disabled` | yes | no | `PUT /skills` clears it as a side effect |
| 19 | `mcp_servers` | yes | **yes** | `PUT /mcp_servers`; TUI detail, `m` |
| 20 | `channels` | yes | **yes — added here** | `PUT /channels`; TUI detail, `n` |
| 21 | `mcp_disabled` | yes | no | |
| 22 | `metadata` | yes | no | |
| 23 | `tags` | **no** | no | The one API gap. In flight in #7866 — see *Known gaps* |
| 24 | `routing` | yes | no | |
| 25 | `autonomous` | yes | no | |
| 26 | `pinned_model` | yes | no | |
| 27 | `workspace` | yes, with a guard | no | Preserved when the incoming manifest omits it — see *Deliberately not writable* |
| 28 | `generate_identity_files` | yes | no | Read at spawn only |
| 29 | `workspaces` | yes | no | |
| 30 | `exec_policy` | yes | no | |
| 31 | `tool_allowlist` | yes | no | Also `PUT /tools` |
| 32 | `tool_blocklist` | yes | no | Also `PUT /tools` |
| 33 | `tools_disabled` | yes | no | `PUT /tools` cannot set it, and clears it — in flight in #7866 |
| 34 | `response_format` | yes | no | |
| 35 | `enabled` | yes | no | |
| 36 | `allowed_plugins` | yes | no | |
| 37 | `inherit_parent_context` | yes | no | |
| 38 | `thinking` | yes | no | |
| 39 | `context_injection` | yes | no | |
| 40 | `is_hand` | yes — arguably should not be | no | Provenance, and it is read. See *Known gaps* |
| 41 | `web_search_augmentation` | yes | no | Also `PATCH /config` |
| 42 | `auto_dream_enabled` | yes | no | |
| 43 | `auto_dream_min_hours` | yes | no | |
| 44 | `auto_dream_min_sessions` | yes | no | |
| 45 | `show_progress` | yes | no | |
| 46 | `auto_evolve` | yes | no | Also `PATCH {"auto_evolve"}` |
| 47 | `channel_overrides` | yes | no | |
| 48 | `max_history_messages` | yes | no | |
| 49 | `max_concurrent_invocations` | yes | no | Cap is sized once per spawn; needs kill + respawn |
| 50 | `assignee_wake` | yes | no | |
| 51 | `cache_context` | yes | no | |
| 52 | `tool_exec_backend` | yes | no | Picked up on respawn |
| 53 | `skill_workshop` | yes | no | |
| 54 | `proactive_memory` | yes | no | |
| 55 | `compaction` | yes | no | |
| 56 | `context_engine` | yes | no | Picked up on respawn |
| 57 | `rl_export` | yes | no | |
| 58 | `triggers` | yes | no | An entry with no `pattern` used to make the whole file unwritable — see below |
| 59 | `reconcile_orphans` | yes | no | |
| 60 | `async_tasks` | yes | no | |
...
**The TUI can edit three fields of 60 on a running agent.**
...
Today the whole-manifest path is write-only over HTTP: a client can `PATCH` 60 fields but cannot fetch the 60 it is about to overwrite.
...
TOML has no null.
`persist_full_manifest_at` serializes the entire `AgentManifest` with `toml::to_string_pretty`, so one such entry failed the write for every other field at once — and the failure was a swallowed log, so `update_manifest` still answered `200 OK`.
