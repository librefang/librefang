...
# The daemon resolves its config file through LIBREFANG_CONFIG_PATH first and
# only then falls back to $LIBREFANG_HOME/config.toml — see
# `default_config_path` in crates/librefang-kernel/src/config.rs. Deriving it
# as "$DATA_DIR/config.toml" here made this script test, initialise, and
# rewrite a file the daemon never reads whenever the config was relocated. The
# case that matters is a Kubernetes ConfigMap mount (#6695): /data/config.toml
# is absent on a fresh PVC, so `librefang init` ran, took the upgrade branch
# because the *mounted* file exists, and exited 1 trying to merge new default
# sections into a read-only mount — a permanent CrashLoopBackOff.
...
if [ ! -f "$CONFIG" ]; then
  # Managed mode means the deployment owns this file, so a missing one is a
  # broken mount rather than a first boot. `librefang init` is not gated by
  # managed mode (it is a CLI write, and managed mode governs the API surface),
  # so it would happily author the file the manifest is supposed to supply and
  # the daemon would come up on configuration nobody declared. Refuse with the
  # path, which is the one fact needed to fix the mount.
...
# The three rewrites below edit config.toml in place. `sed -i` writes a
# temporary file into the config's own directory, so on a read-only mount they
# fail and take the whole script down with `set -e` — a crash loop caused by a
# rewrite that usually has nothing to change. A relocated config is frequently
# read-only (a Kubernetes ConfigMap volume always is), the deployment owns it
# by definition, and LIBREFANG_LISTEN overrides `api_listen` after load
# anyway (see `boot_with_config_at` in librefang-kernel), so skipping is the
# correct behaviour rather than merely the safe one. Test writability rather
# than the mode: this is about whether the rewrite can succeed, and a mutable
# deployment that mounts its config read-only wants the same answer.
if [ -w "$CONFIG" ]; then
  ...
  # Silence here would bind the wrong port or run the wrong model with nothing
  # on screen to explain it, so say so even though it is not fatal.
