...
#: Wire-protocol version this SDK implements, carried on every ``ready``
#: frame via :attr:`librefang.sidecar.runtime.SidecarAdapter.protocol_version`.
#:
#: The daemon's ``librefang_channels::sidecar::SIDECAR_PROTOCOL_VERSION`` is
#: the source of truth for the number; this constant mirrors it and
#: ``crates/librefang-channels/tests/sidecar_version_contract.rs`` fails the
...
def sdk_version() -> str:
    ...
    Imported lazily: :mod:`librefang.sidecar.protocol` is imported by every
    adapter at startup, and resolving the version touches the filesystem or
...
class Schema:
    ...
    def to_dict(self):
        # `sdk_version` is emitted unconditionally rather than declared per
        # adapter: #7140 was a deployment running a four-month-old SDK against
        # a current daemon, and the daemon had no path by which the adapter's
        # own version could reach it. `--describe` runs the same interpreter,
        # with the same PYTHONPATH resolution, as the eventual spawn — so the
        # version it reports is the version that will actually serve traffic.
        return {
            "name": self.name,
