...
/// Render a best-effort command failure for logging. Debug-formats the error so embedded control
/// characters (newlines, ANSI escapes) are escaped rather than able to forge extra log lines,
/// matching `file_lookup_error_log` / `callback_ack_error_log` in `translator.rs`.
