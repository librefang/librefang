...
- Honour `agent.toml: [model] context_window` on the paths that actually run a turn, so the field the runtime's own warning tells operators to set is no longer inert.
  ...
  The chain (manifest override, then catalog, then persisted session value) now lives in one `manifest_helpers::resolve_context_window` used by all four call sites plus the compaction gate.
