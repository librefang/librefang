...
- **Codify the error-contract target shape** (#3576). New `docs/architecture/error-contracts.md` captures the four-layer rule that the #3541 / #3711 / #3745 / #3743 work has been migrating toward: crate-local `thiserror` enums for single-domain libraries, `LibreFangError` (in `librefang-types`) for cross-domain orchestrators, typed errors at trait boundaries (`KernelHandle`, `LlmDriver`), and `anyhow::Result` only in binaries. Enumerates the three remaining sanctioned `anyhow` sites (`librefang-runtime-wasm/sandbox.rs`, `librefang-api/terminal_tmux.rs`, the `From<anyhow::Error>` shim in `routes/memory.rs`) and the reasons they cannot trivially migrate today (wasmtime returns `anyhow::Result`; `librefang-types` cannot depend on `librefang-llm-driver` without inverting the DAG). Documents migration-time adapters on `LibreFangError` (`From<String>`, `From<&str>`, `From<serde_json::Error>`) as conveniences, not endorsed long-term. Out of scope: per-variant CLI exit codes, promoting `LlmError` into `librefang-types`, promoting `Result<_, String>`-typed inner fns (`compact_session`, workflow `execute_run` / `dry_run`, `router::load_template_manifest`) to typed errors — those need separate follow-up issues per the #3711 slice-5 audit. `CONTRIBUTING.md` `Code Style` references the new RFC. Refs #3576. (@houko)
