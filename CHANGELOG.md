...
- Drive the dashboard's per-channel status indicator from the sidecar supervisor's real liveness instead of from message traffic, and surface the fields it reads on `GET /api/channels`.
  ...
  The `librefang channel list` table gains CONNECTED and IN/OUT columns plus a per-channel error footnote, fed the raw supervisor fields rather than a second copy of the state mapping (#6606) (@nevgenov)
- Stop presenting the channels page's 24h message count as per-bot traffic when it is a per-channel-type aggregate.
  ...
  `UsageStore::channels_msgs_24h_bulk` is renamed to `channel_type_msgs_24h_bulk` so the grouping is not misread again at the call site (#6606) (@nevgenov)
- Render a failed `GET /api/channels` as an error on the channels page instead of the "no channels configured" empty state, which made an unreachable daemon look like a clean install on the page whose whole purpose is now health signalling (#6606) (@nevgenov)
