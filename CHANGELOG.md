...
- Expose every config field on `GET /api/config` that `POST /api/config/set` accepts, so a setting the dashboard just saved stops reading back as "not configured".
  The response body is hand-enumerated section by section, and forty-odd writable fields had never been added to it — among them `browser.enabled` / `browser.cdp_endpoint`, `media.image_model` / `media.custom_stt`, `tts.custom`, `approval.totp_grace_period_secs`, `web.timeout_secs`, `exec_policy.allowed_env_vars`, the `[channels]` file-transfer limits, and the whole `[terminal]` section, which the dashboard declares a page for but could never populate.
