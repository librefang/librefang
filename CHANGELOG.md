...
- **Daemon log lines now carry `agent.id` / `session.id` for correlation across `run_agent_loop` and supervised background tasks.** Two changes: (1) `run_agent_loop`'s `#[instrument]` span now sets `session.id` alongside the existing `agent.id`, so every event emitted inside the agent loop (e.g. `WARN Shell exec in full mode — no restrictions ...`) includes both fields as inline span suffix tokens with the compact formatter; (2) `librefang_kernel::supervised_spawn` now wraps the spawned future with `.instrument(Span::current())` — `tokio::spawn` does NOT propagate the current tracing span, so every supervised background task (channel bridges, cron tickers, inbox pumps, persist loops; ~48 call sites in the kernel) was starting in a bare span context and its logs could not be correlated to the originating agent. Symptom: `docker logs jarvis | grep "Shell exec"` showed no agent context. Now the same grep yields `WARN run_agent_loop{agent.name=… agent.id="…" session.id="…"}: shell exec full mode …`. New regression tests pin both behaviours: `instrument_span_fields::warn_inside_agent_span_includes_agent_and_session_ids` (runtime), `with_trace_id_compact_format_carries_agent_and_session_ids_from_span` (cli WithTraceId wrapper), and `supervised_spawn::tests::supervised_task_inherits_caller_span` (kernel). (@vigneshjagadeesh)
