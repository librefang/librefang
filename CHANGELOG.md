...
- Correct the channel docs' claim that all five sidecars read `*_DM_POLICY` / `*_GROUP_POLICY` env vars: only the WhatsApp sidecar reads `WHATSAPP_DM_POLICY` (`WHATSAPP_GROUP_POLICY` is read but inert), the Telegram / Discord / Slack / Teams sidecars read neither, and DM / group authorization is actually enforced kernel-side via `[[users]]` channel bindings (#6405) (@houko)
