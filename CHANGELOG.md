...
- **MCP OAuth: extend the same eTLD+1 loosening to the discovery-time metadata host pin** (#4665, follow-up to #4779). #4779 fixed the API-layer callback gate but left the discovery-layer gate in `librefang-runtime-mcp::mcp_oauth::validate_metadata_endpoints` strict — Slack's AS metadata was rejected at Tier 1/2 with `endpoint domain mismatch` BEFORE any auth flow could begin, and librefang fell back to a synthesized `https://<server>/authorize` URL that Slack errored on. The discovery gate now mirrors #4779's policy on `authorization_endpoint`, `token_endpoint`, and `registration_endpoint`: exact-origin match accepts via the strict #3713 fast path; otherwise both hosts must be DNS names with the same registrable domain per the PSL (same `psl::domain_str` + IP-literal carve-out as #4779). Refusal text is unchanged (`"OAuth metadata endpoint domain mismatch: …"`) so operator dashboards and grep patterns continue to fire. A `tracing::info!` audit line is emitted on the relaxed-accept path with `endpoint`, `endpoint_host`, `server_url`, `server_host`, `registrable_domain`, `label`. Four new unit tests pin Slack-shape acceptance, deeper-subdomain acceptance, PSL private-suffix rejection, and IPv4 false-match rejection; the five pre-existing `validate_metadata_endpoints_*` tests use cross-registrable-domain hosts and pass unchanged. `docs/architecture/mcp-oauth-host-pinning.md` extended with a discovery-layer-gate section. (@neo-wanderer)
