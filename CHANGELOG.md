...
- **runtime(sandbox): WASI Preview 2 host integration for Component plugins (@gqadonis)** — `wasmtime-wasi` and `wasmtime-wasi-http` are now wired into the Component plugin linker (always-on, deny-by-default WasiCtx). Language runtimes (CPython, StarlingMonkey, cargo-component's wasi-rt, TinyGo/WASI P1 adapter) can call `wasi:cli/environment`, `wasi:io/poll`, `wasi:http/types`, etc. during init and get back empty-but-valid responses rather than a link-time trap. The librefang capability list remains the real gate; plugins reach host capabilities through `librefang:plugin/*`, not through WASI. All 6 `librefang:plugin/*` interfaces are now unconditionally bound at link time (capability check deferred to dispatch), resolving the StarlingMonkey preview2-shim auto-import issue that blocked `js-kv-counter`. 3 of 5 Phase-6 integration tests now run end-to-end (c-noop, rust-fs-cat, python-hello-time); 2 remain `#[ignore]`d pending Phase-8 fixture rebuilds (WASI version mismatch in StarlingMonkey + TinyGo reactor adapter). (@gqadonis)

- **runtime(sandbox): `SandboxConfig::max_memory_bytes` enforced on Component path (@gqadonis)** — `store.limiter(|s| s.guest.limiter_mut())` is now wired in `execute_component`, closing the Phase-5 TODO. `GuestState::limiter_mut()` is `pub(crate)` to bridge `sandbox.rs` ↔ `sandbox_component.rs`; `MemoryLimiter` is `pub(crate)` for the same reason. Memory bound enforcement is verified by the new `component_respects_max_memory_bytes` test in `sandbox_component::tests`. (@gqadonis)

- **runtime(examples,docs): phase-6 walking plugin examples (@gqadonis)** — five per-language minimal plugins exercising the Component Model host: `c-noop` (C, no caps, 1 KB), `rust-fs-cat` (Rust, fs, 55 KB), `python-hello-time` (Python, time, 18 MB CPython), `js-kv-counter` (JavaScript, kv, 12 MB StarlingMonkey), `go-env-greet` (Go, env, 405 KB). All under [`examples/plugins/`](examples/plugins/) with a checked-in `pre-built/plugin.wasm`; regenerate via `cargo xtask plugins-rebuild <name>`. Integration tests at `crates/librefang-runtime/tests/plugin_example_*.rs` — c-noop runs end-to-end today; the other four are `#[ignore]`d with precise reasons pointing at the missing WASI 0.2 host imports (CPython init, StarlingMonkey preview2-shim, TinyGo runtime, cargo-component's wasi-rt). Wiring `wasmtime-wasi` into `sandbox_component.rs` is the Phase-7 follow-on that lights the four skips green.
...
- **runtime(sandbox): Component Model `execute_component` now seeds `Store::set_fuel` + `Store::set_epoch_deadline` (@gqadonis)** — the shared `engine_config()` enables `consume_fuel(true)` and `epoch_interruption(true)`, but the Component path was instantiating stores without seeding either. Result: fuel = 0, every component trapped on its first wasm instruction with an opaque "wasm function N" error. Parity with the core-module `execute()` path at `sandbox.rs:483`. The Phase-6 c-noop integration test surfaced this — Phase-5's smoke test passed only because it loaded the component without invoking `run()`.
