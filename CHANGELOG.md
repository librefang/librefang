...
- **Workflow lifecycle (#4906) follow-ups: persist `Cancelled` runs across restart, reject `total_timeout_secs = 0`** (#4908). Three review findings on top of the merged #4906, plus a fourth defect surfaced while landing the test fixtures: (0) The v37 `workflow_runs` CHECK constraint enumerated only `pending/running/paused/completed/failed`, so when #4906 added the `WorkflowRunState::Cancelled` variant **every SQLite write of a Cancelled row was silently rejected at the schema layer** — `upsert_run_to_store` got a WARN log per cancel and the row never landed on disk. New migration v39 swaps `workflow_runs` to a new table whose CHECK admits `cancelled`, preserving every existing row (table-swap pattern: `CREATE TABLE workflow_runs_new (..., CHECK(state IN (..., 'cancelled')))`, `INSERT INTO workflow_runs_new SELECT * FROM workflow_runs`, `DROP TABLE workflow_runs`, `ALTER ... RENAME`, recreate the three indexes — all inside the migration transaction). Companion fix in `row_to_workflow_run` at `workflow.rs:3498` which never had a `"cancelled"` arm, so even if the row had landed it would have been silently dropped on reload with a `warn!`. (1) The JSON persist filter in `WorkflowEngine::persist_runs_to_json` (`crates/librefang-kernel/src/workflow.rs:828`) only matched `Completed | Failed | Paused`, so `WorkflowEngine::cancel_run` flushed the row through `persist_runs_async` but the filter **dropped Cancelled before writing** — every cancellation on JSON-mode installs was silently lost on restart and `recover_stale_running_runs` would flip the formerly-Running row to `Failed` after the stale window, losing the user-cancellation signal entirely. Symmetric defect in the SQLite path at `workflow.rs:2890` — `upsert_run_to_store` after `cancel_run` wrote the row but the WAL-checkpoint match arm omitted `Cancelled`, so on SIGKILL / OOM / power loss the row could be left in WAL-only state and `recover_stale_running_runs` would similarly flip it to Failed. Both call sites now include `WorkflowRunState::Cancelled`, closing the load-bearing "persists immediately so a crash after the call does not revert state" promise from the original PR body. (2) `total_timeout_secs = 0` was accepted at every input layer (`POST /api/workflows`, `PUT /api/workflows/{id}`, `KernelConfig.workflow_default_total_timeout_secs`) and mapped to `Duration::ZERO` in `WorkflowEngine::execute_run`, firing the timeout before the inner future is polled once — every workflow run failing instantly with `"workflow exceeded total_timeout of 0s"`. Worst case via the kernel-default field: a single bad value in `config.toml` bricks every workflow on the daemon at once. The API layer now rejects `Some(0)` with HTTP 400 in both `create_workflow` and `update_workflow` (clients that want unbounded must omit the field or set it to `null`); `boot.rs` log-warns and treats `Some(0)` as `None` so a typo in `config.toml` doesn't take the whole daemon down silently; the kernel resolver in `execute_run` carries the same defensive filter as a final belt-and-braces guard. (3) Docstring on `WorkflowEngine::cancel_run` now explicitly documents the between-step semantics — cancel is observed at step boundaries; an in-flight `send_message`, FanOut `join_all`, or any other step body continues to completion (or its per-step `timeout_secs`) before cancellation lands. The retry-sleep `tokio::select!` is the one place inside a step where cancel pre-empts an in-progress wait; hard Ctrl-C-style preemption of an in-flight LLM call is out of scope and tracked as a separate follow-up. New tests in `crates/librefang-kernel/src/workflow.rs::tests`: `cancelled_run_survives_json_persist_round_trip` writes a cancelled run to disk and reloads it, `cancelled_run_persists_to_sqlite_with_wal_checkpoint` covers the WAL path, `total_timeout_secs_zero_is_treated_as_unset_in_resolver` pins the kernel-side defensive filter. New tests in `crates/librefang-api/tests/workflow_lifecycle_test.rs`: `create_workflow_rejects_total_timeout_secs_zero` and `update_workflow_rejects_total_timeout_secs_zero` pin the 400 responses. Closes #4908. (@houko)
