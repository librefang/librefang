...
- Refresh the EveryAPI catalog from the channel model picker, which was the one read path #6583 left unwired.
  `list_models_by_provider` in `channel_bridge.rs` refreshes for `openrouter` and fell through for `everyapi`, so a Telegram or Slack user picking a model saw whatever snapshot the catalog last happened to hold while every `providers.rs` read path refreshed correctly (#6602) (@houko)
