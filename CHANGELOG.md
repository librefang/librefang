...
- Request `/json/new` with PUT when attaching to an `http://` `cdp_endpoint`, so HTTP target discovery works against current Chrome.
  ...
  A 405 on PUT falls back to GET rather than failing, since older Chromium builds and third-party CDP proxies may route only the older verb, and there is no version handshake to branch on.
  `/json/close/{target_id}` is the same class of state-mutating endpoint and needs the same PUT-with-405-fallback handling, so the tab cleanup in `BrowserManager::close_session` now goes through it too — otherwise every session opened against current Chrome would attach fine and then leak its tab on close (@nevgenov)
