...
- **`ChannelSender` / `ApprovalGate` migrated to `KernelOpError`** (#3541 5/N): bundles the two largest remaining default-impl traits — 11 methods total. `ChannelSender` covers `send_channel_message`, `send_channel_media`, `send_channel_file_data`, `send_channel_poll`, `roster_upsert`, `roster_members`, `roster_remove_member`. `ApprovalGate` covers `request_approval`, `submit_tool_approval`, `resolve_tool_approval`, `get_approval_status`. Default impls return `KernelOpError::Unavailable { capability: "Channel … send" / "Approval system" }` so callers get the same structured error category instead of bespoke strings. 8 of 14 role traits typed. (#3541 5/N) (@houko)
- **`PromptStore` migrated to `KernelOpError`** (#3541 6/N): all 13 methods (`get_running_experiment`, `record_experiment_request`, `get_prompt_version`, `list_prompt_versions`, `create_prompt_version`, `delete_prompt_version`, `set_active_prompt_version`, `list_experiments`, `create_experiment`, `get_experiment`, `update_experiment_status`, `get_experiment_metrics`, `auto_track_prompt_version`) flipped from `Result<_, String>` to `Result<_, KernelOpError>`. Default impls return `KernelOpError::Unavailable { capability: "Prompt store" }`. Kernel impls preserve their existing `format!()` error messages by wrapping in `KernelOpError::Other(...)` for now; finer NotFound/Invalid classification is left to a follow-up. 9 of 14 role traits typed. (#3541 6/N) (@houko)
- **`AgentControl` migrated to `KernelOpError`** (#3541 7/N): the hot-path role trait — `spawn_agent`, `spawn_agent_checked`, `send_to_agent`, `send_to_agent_as`, `kill_agent`, `run_forked_agent_oneshot` — flipped from `Result<_, String>` to `Result<_, KernelOpError>`. `LibreFangKernel::spawn_agent_checked` now emits `KernelOpError::Invalid { field: "manifest", reason }` for TOML parse failures and `Other(...)` for capability-inheritance violations; `kill_agent` preserves the historical `"Kill failed: …"` message via `Other`. Test stubs across 4 kernel-handle test files plus runtime tool_runner inline stubs all migrated. Existing `assert_eq!(result, Ok(...))` test helpers switched to `result.unwrap()` since `KernelOpError` doesn't impl `PartialEq` (the `Serialize(serde_json::Error)` variant blocks the derive). 10 of 14 role traits typed — only `A2ARegistry` (already typed via `Option`) and `ToolPolicy` (infallible) remain. (#3541 7/N) (@houko)
