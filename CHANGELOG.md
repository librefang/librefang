...
- Stop applying the credential-word secret-name heuristic (`KEY` / `TOKEN` / `PASSWORD` / …) to the operator's own `exec_policy.allowed_env_vars` when scrubbing a sandboxed child's environment: the #6439 re-check filtered the operator's explicit allowlist through `is_blocked_env_var`, so a secret-shaped name (e.g. a CLI's keyring-unlock password variable) never reached a `shell_exec` / `process_start` child even when the operator had explicitly granted it in their own agent.toml — essentially every credential name matches the heuristic, so the allowlist could not carry a credential at all, and the only signal was a daemon-side WARN; `sandbox_command` now takes the allowlist's provenance (`EnvAllowlistSource`: operator-authored config vs hand-declared `[[requires]]` / hand-authored manifest, decided by `manifest.is_hand`) and applies the heuristic only to hand-declared sources, while the reserved daemon secrets (`LIBREFANG_VAULT_KEY`, provider keys) stay unconditionally blocked for every source, and the provenance is persisted on deferred approvals so a resumed execution filters identically (#6458) (@neo-wanderer)
