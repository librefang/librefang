...
- Per-user LLM provider credentials (#6460, initial/WIP slice): a human user can store their own upstream provider API key encrypted in the existing credential vault (`CredentialVault`, AES-256-GCM, keyed by `LIBREFANG_VAULT_KEY` / OS keyring) under a per-user, per-provider namespace via the new `LibreFangKernel::{set,get,remove,list}_user_provider_key` methods, and `resolve_driver_for_owner` resolves credentials with the precedence user-scoped key > daemon-global key (env var / credential pool); global-only behaviour is byte-identical when no user-scoped key exists, the plaintext value is never returned through the API (listing surfaces provider names only), and per-owner spend is already queryable via the existing `/api/budget/users` rollup — threading the turn's human owner into resolution from request context and the HTTP/dashboard management surface are deferred follow-ups (#6460) (@houko)
