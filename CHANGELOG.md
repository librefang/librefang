...
- Require an explicit declaration before a hand's agent gets a wake-up cycle, instead of inferring autonomous ticking from `max_iterations`.
  ...
  **Hands that were implicitly ticking will stop, and that is a behaviour change, not only a bug fix.**
  ...
  Three further roles (`devteam`'s `pm` / `engineer` / `qa`) inherit from agent templates that live outside the in-repo fixtures, so whether they were ticking depends on those templates and has to be checked in the registry repository.
  ...
  `frequency = "reactive"` also stops being a TOML parse error — #6595 reports an operator reaching for that spelling while hunting the ticks — though as catalog metadata it has no effect on scheduling either way (#6595) (@nevgenov)
