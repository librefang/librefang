...
- Stop the three streaming LLM paths from silently swallowing a provider error delivered mid-stream over an already-HTTP-200 SSE body, which turned a recoverable overload / rate-limit into a truncated-or-empty "successful" turn with no retry or failover. The Anthropic driver now handles an `event: error` frame (e.g. `overloaded_error` under load) by returning the typed `LlmError::Overloaded` / `RateLimited` / `Api` it maps from `error.type` instead of letting the frame fall into the catch-all match arm and ending the stream `Ok` with partial content; the OpenAI-compatible driver (which OpenRouter and Groq route through) now surfaces a terminal `data: {"error": …}` frame as an `LlmError::Api` — classified via a new `openai_stream_error_code` helper so a rate-limit still retries the same provider rather than skipping it — instead of `continue`-ing past the choice-less frame and returning empty content, while a bare `"error": null` on an otherwise-valid chunk is still ignored (#6512) (@houko)
- Stop `FallbackDriver::stream` from corrupting the caller's output when a provider fails over mid-stream: once a provider has forwarded observable content (text / tool / thinking deltas) to the caller and then errors, advancing to the next driver on the same caller channel concatenated a second full response onto the partial output already delivered, producing garbled text. `FallbackDriver::stream` now wraps the wrapped driver's stream in the same content-emitted intercept relay that `FallbackChain::stream` already uses, and propagates the error instead of failing over once any content has reached the caller (#6512) (@houko)
