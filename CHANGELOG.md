...
- Exempt `read_artifact` results from the post-tool artifact re-spill chokepoint, so paging in a spilled tool result larger than `spill_threshold_bytes` returns the actual bytes instead of minting a fresh artifact stub that again points at `read_artifact` — previously any read in the 16–64 KiB range could never make progress (#6406) (@houko)
