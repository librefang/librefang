...
- `LIBREFANG_HOME` env var overrides the home directory used by the daemon AND any CLI it spawns (e.g. `claude` via the `claude-code` driver). Containers that drop privileges to a numeric uid without a matching passwd entry inherit a placeholder home (`/nonexistent` on glibc, `/var/empty` on BSD/Alpine, `/dev/null` on some hardened images) — set `LIBREFANG_HOME` in the orchestrator manifest / kernel-boot wrapper so the spawned CLI can find `~/.claude/.credentials.json`. The override is unused when the inherited platform home (`$HOME` on Unix, `%USERPROFILE%` on Windows) already resolves to a real on-disk directory.
