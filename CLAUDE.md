...
## Architecture Notes
- **OFP wire is plaintext by design**: `librefang-wire` ships HMAC-SHA256 mutual auth + per-message HMAC + nonce replay protection but **does not encrypt frames**. Confidentiality is delegated to the deployment layer (WireGuard / Tailscale / SSH tunnel / service-mesh mTLS), not re-implemented here. Closed [#3874](https://github.com/librefang/librefang/issues/3874) and closed [#4001](https://github.com/librefang/librefang/pull/4001) record the decision; full rationale in `docs/architecture/ofp-wire-security.md`. **Do not add TLS termination inside `librefang-wire`** without re-litigating that decision in an issue first — the HMAC framing intentionally covers active-attacker threats and overlays cover passive-observer threats.
